...
import (
	...
	"github.com/khulnasoft/shipyard/modules/pprof"
	"github.com/khulnasoft/shipyard/modules/private"
	"github.com/khulnasoft/shipyard/modules/process"
...
func setup(ctx context.Context, debug bool) {
	...
	// `[repository]` `ROOT` is a relative path and $SHIPYARD_WORK_DIR isn't passed to the SSH connection.
	if _, err := os.Stat(setting.RepoRootPath); err != nil {
		if os.IsNotExist(err) {
			_ = fail(ctx, "Incorrect configuration, no repository directory.", "Directory `[repository].ROOT` %q was not found, please check if $SHIPYARD_WORK_DIR is passed to the SSH connection or make `[repository].ROOT` an absolute value.", setting.RepoRootPath)
		} else {
...
func fail(ctx context.Context, userMessage, logMsgFmt string, args ...any) error {
	...
	_, _ = fmt.Fprintln(os.Stderr, "Shipyard:", userMessage)

	if logMsgFmt != "" {
		logMsg := fmt.Sprintf(logMsgFmt, args...)
		if !setting.IsProd {
			_, _ = fmt.Fprintln(os.Stderr, "Shipyard:", logMsg)
		}
		if userMessage != "" {
...
func runServ(c *cli.Context) error {
	...
	if setting.SSH.Disabled {
		println("Shipyard: SSH has been disabled")
		return nil
	}
	...
	if len(cmd) == 0 {
		...
		case asymkey_model.KeyTypeDeploy:
			println("Hi there! You've successfully authenticated with the deploy key named " + key.Name + ", but Shipyard does not provide shell access.")
		case asymkey_model.KeyTypePrincipal:
			println("Hi there! You've successfully authenticated with the principal " + key.Content + ", but Shipyard does not provide shell access.")
		default:
			println("Hi there, " + user.Name + "! You've successfully authenticated with the key named " + key.Name + ", but Shipyard does not provide shell access.")
		}
		println("If this is unexpected, please log in with password and setup Shipyard under another user.")
		return nil
	...
	if len(words) < 2 {
		if git.CheckGitVersionAtLeast("2.29") == nil {
			// for AGit Flow
			if cmd == "ssh_info" {
				fmt.Print(`{"type":"shipyard","version":1}`)
				return nil
			}
		}
	...
	// to avoid breaking, here only use the minimal environment variables for the "shipyard serv" command.
