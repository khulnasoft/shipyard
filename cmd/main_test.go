...
func TestCliCmd(t *testing.T) {
	...
	for _, s := range os.Environ() {
		if strings.HasPrefix(s, "SHIPYARD_") && strings.Contains(s, "=") {
			envBackup = append(envBackup, s)
		}
	}
	clearShipyardEnv := func() {
		for _, s := range os.Environ() {
			if strings.HasPrefix(s, "SHIPYARD_") {
				_ = os.Unsetenv(s)
			}
		}
	}
...
func TestCliCmdError(t *testing.T) {
	...
	r, err := runTestApp(app, "./shipyard", "test-cmd")
	assert.Error(t, err)
	...
	r, err = runTestApp(app, "./shipyard", "test-cmd", "--no-such")
	assert.Error(t, err)
	assert.Equal(t, 1, r.ExitCode)
