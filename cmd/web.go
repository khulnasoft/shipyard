...
import (
	...
	"github.com/khulnasoft/shipyard/routers/install"
...
var PIDFile = "/run/shipyard.pid"

// CmdWeb represents the available web sub-command.
var CmdWeb = &cli.Command{
	Name:  "web",
	Usage: "Start Shipyard web server",
	Description: `Shipyard web server is the only thing you need to run,
...
func showWebStartupMessage(msg string) {
	log.Info("Shipyard version: %s%s", setting.AppVer, setting.AppBuiltWith)
...
func serveInstall(ctx *cli.Context) error {
	...
	err := listen(c, false)
	if err != nil {
		log.Critical("Unable to open listener for installer. Is Shipyard already running?")
		graceful.GetManager().DoGracefulShutdown()
	}
	select {
	case <-graceful.GetManager().IsShutdown():
		<-graceful.GetManager().Done()
		log.Info("PID: %d Shipyard Web Finished", os.Getpid())
...
func serveInstalled(ctx *cli.Context) error {
	...
	// now, Shipyard only serves pre-defined files in the "custom/public" folder basing on the web root, the user should move their custom files to "custom/public/assets"
...
func runWeb(ctx *cli.Context) error {
	...
	if os.Getppid() > 1 && len(os.Getenv("LISTEN_FDS")) > 0 {
		log.Info("Restarting Shipyard on PID: %d from parent PID: %d", os.Getpid(), os.Getppid())
	} else {
		log.Info("Starting Shipyard on PID: %d", os.Getpid())
	}
...
func listen(m http.Handler, handleRedirector bool) error {
	...
	_, _, finished := process.GetManager().AddTypedContext(graceful.GetManager().HammerContext(), "Web: Shipyard Server", process.SystemProcessType, true)
	defer finished()
