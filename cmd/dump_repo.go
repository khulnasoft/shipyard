...
var CmdDumpRepository = &cli.Command{
	Name:        "dump-repo",
	Usage:       "Dump the repository from git/github/shipyard/gitlab",
	...
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:  "git_service",
			Value: "",
			Usage: "Git service, git, github, shipyard, gitlab. If clone_addr could be recognized, this could be ignored.",
		},
		&cli.StringFlag{
			Name:    "repo_dir",
			Aliases: []string{"r"},
		...
		&cli.StringFlag{
			Name:  "clone_addr",
			Value: "",
			Usage: "The URL will be clone, currently could be a git/github/shipyard/gitlab http/https URL",
		},
		&cli.StringFlag{
			Name:  "auth_username",
			Value: "",
...
func runDumpRepository(ctx *cli.Context) error {
	...
	// migrations.ShipyardLocalUploader depends on git module
	...
	if strings.HasPrefix(strings.ToLower(cloneAddr), "https://github.com/") {
		...
		serviceStr = "gitlab"
	} else if strings.HasPrefix(strings.ToLower(cloneAddr), "https://shipyard.khulnasoft.com/") {
		serviceStr = "shipyard"
	}
	if serviceStr == "" {
