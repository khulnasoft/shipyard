...
var CmdDump = &cli.Command{
	Name:  "dump",
	Usage: "Dump Shipyard files and database",
	Description: `Dump compresses all related files and database into zip file.
It can be used for backup and capture Shipyard server image to send to maintainer`,
	Action: runDump,
	Flags: []cli.Flag{
		&cli.StringFlag{
			Name:    "file",
			Aliases: []string{"f"},
			Value:   fmt.Sprintf("shipyard-dump-%d.zip", time.Now().Unix()),
...
func runDump(ctx *cli.Context) error {
	...
	if !setting.InstallLock {
		...
		return fmt.Errorf("shipyard is not initialized")
	}
	...
	dbDump, err := os.CreateTemp(tmpDir, "shipyard-db.sql")
	if err != nil {
	...
	if err := addFile(w, "shipyard-db.sql", dbDump.Name(), verbose); err != nil {
		fatal("Failed to include shipyard-db.sql: %v", err)
	}
