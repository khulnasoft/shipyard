...
func runHookPreReceive(c *cli.Context) error {
	...
	if len(os.Getenv("SSH_ORIGINAL_COMMAND")) == 0 {
		if setting.OnlyAllowPushIfShipyardEnvironmentSet {
			return fail(ctx, `Rejecting changes as Shipyard environment not set.
If you are pushing over SSH you must push with a key managed by
Shipyard or set your environment appropriately.`, "")
		}
		return nil
	}
