...
func TestShipyardUploadRepo(t *testing.T) {
	...
	var (
		...
		uploader   = NewShipyardLocalUploader(graceful.GetManager().HammerContext(), user, user.Name, repoName)
	)
...
func TestShipyardUploadRemapLocalUser(t *testing.T) {
	unittest.PrepareTestEnv(t)
	doer := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 1})
	user := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 2})

	repoName := "migrated"
	uploader := NewShipyardLocalUploader(context.Background(), doer, doer.Name, repoName)
	// call remapLocalUser
...
func TestShipyardUploadRemapExternalUser(t *testing.T) {
	unittest.PrepareTestEnv(t)
	doer := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: 1})

	repoName := "migrated"
	uploader := NewShipyardLocalUploader(context.Background(), doer, doer.Name, repoName)
	uploader.gitServiceType = structs.ShipyardService
	...
	externalLoginUser := &user_model.ExternalLoginUser{
		...
		LoginSourceID: 0,
		Provider:      structs.ShipyardService.Name(),
	}
...
func TestShipyardUploadUpdateGitForPullRequest(t *testing.T) {
	unittest.PrepareTestEnv(t)
	...
	toRepoName := "migrated"
	uploader := NewShipyardLocalUploader(context.Background(), fromRepoOwner, fromRepoOwner.Name, toRepoName)
	uploader.gitServiceType = structs.ShipyardService
	assert.NoError(t, uploader.CreateRepo(&base.Repository{
		...
		GitServiceType: structs.ShipyardService,
		Private:        false,
		Mirror:         true,
	}))
