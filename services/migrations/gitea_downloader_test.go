...
func TestShipyardDownloadRepo(t *testing.T) {
	// Skip tests if Shipyard token is not found
	shipyardToken := os.Getenv("SHIPYARD_TOKEN")
	if shipyardToken == "" {
		t.Skip("skipped test because SHIPYARD_TOKEN was not in the environment")
	}

	resp, err := http.Get("https://shipyard.khulnasoft.com/shipyard")
	if err != nil || resp.StatusCode != http.StatusOK {
		t.Skipf("Can't reach https://shipyard.khulnasoft.com, skipping %s", t.Name())
	}

	downloader, err := NewShipyardDownloader(context.Background(), "https://shipyard.khulnasoft.com", "shipyard/test_repo", "", "", shipyardToken)
	if downloader == nil {
	...
	assertRepositoryEqual(t, &base.Repository{
		...
		Owner:         "shipyard",
		IsPrivate:     false,
		Description:   "Test repository for testing migration from shipyard to shipyard",
		CloneURL:      "https://shipyard.khulnasoft.com/shipyard/test_repo.git",
		OriginalURL:   "https://shipyard.khulnasoft.com/shipyard/test_repo",
	...
	assert.NoError(t, err)
	sort.Strings(topics)
	assert.EqualValues(t, []string{"ci", "shipyard", "migration", "test"}, topics)
	...
	assertCommentsEqual(t, []*base.Comment{
		{
			...
			Content:     "a really good question!\n\nIt is the used as TESTSET for shipyard2shipyard repo migration function",
		},
		{
			IssueIndex:  4,
			PosterID:    -1,
	...
	assertPullRequestEqual(t, &base.PullRequest{
		...
		Head: base.PullRequestBranch{
			CloneURL:  "https://shipyard.khulnasoft.com/6543-forks/test_repo.git",
		...
		PatchURL:       "https://shipyard.khulnasoft.com/shipyard/test_repo/pulls/12.patch",
	}, prs[1])
			...
			Comments: []*base.ReviewComment{
				{
					...
					Content:   "is one `\\newline` to less?",
					TreePath:  "README.md",
					DiffHunk:  "@@ -2,3 +2,3 @@\n \n-Test repository for testing migration from shipyard 2 shipyard\n\\ No newline at end of file\n+Test repository for testing migration from shipyard 2 shipyard",
					Position:  0,
					Line:      4,
					CommitID:  "187ece0cb6631e2858a6872e5733433bb3ca3b03",
					PosterID:  689,
