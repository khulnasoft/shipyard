...
func SendTestMail(email string) error {
	...
	return gomail.Send(Sender, NewMessage(email, "Shipyard Test Email!", "Shipyard Test Email!").ToMessage())
}
...
func generateAdditionalHeaders(ctx *mailCommentContext, reason string, recipient *user_model.User) map[string]string {
	...
	return map[string]string{
		...
		"X-Mailer":                     "Shipyard",
		"X-Shipyard-Reason":            reason,
		"X-Shipyard-Sender":            ctx.Doer.DisplayName(),
		"X-Shipyard-Recipient":         recipient.DisplayName(),
		"X-Shipyard-Recipient-Address": recipient.Email,
		"X-Shipyard-Repository":        repo.Name,
		"X-Shipyard-Repository-Path":   repo.FullName(),
		"X-Shipyard-Repository-Link":   repo.HTMLURL(),
		"X-Shipyard-Issue-ID":          strconv.FormatInt(ctx.Issue.Index, 10),
		"X-Shipyard-Issue-Link":        ctx.Issue.HTMLURL(),
