...
func TestDingTalkPayload(t *testing.T) {
	...
	t.Run("Package", func(t *testing.T) {
		...
		assert.Equal(t, "Package created: ShipyardContainer:latest by user1", pl.(*DingtalkPayload).ActionCard.Text)
		assert.Equal(t, "Package created: ShipyardContainer:latest by user1", pl.(*DingtalkPayload).ActionCard.Title)
		assert.Equal(t, "view package", pl.(*DingtalkPayload).ActionCard.SingleTitle)
		assert.Equal(t, "http://localhost:3000/user1/-/packages/container/ShipyardContainer/latest", parseRealSingleURL(pl.(*DingtalkPayload).ActionCard.SingleURL))
	})
