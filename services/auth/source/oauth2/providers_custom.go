...
func init() {
	...
	RegisterGothProvider(NewCustomProvider(
		"shipyard", "Shipyard", &CustomURLSettings{
			TokenURL:   requiredAttribute(shipyard.TokenURL),
			AuthURL:    requiredAttribute(shipyard.AuthURL),
			ProfileURL: requiredAttribute(shipyard.ProfileURL),
		},
		func(clientID, secret, callbackURL string, custom *CustomURLMapping, scopes []string) (goth.Provider, error) {
			return shipyard.NewCustomisedURL(clientID, secret, callbackURL, custom.AuthURL, custom.TokenURL, custom.ProfileURL, scopes...), nil
		}))
