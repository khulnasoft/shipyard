...
const packageUrl = 'https://raw.githubusercontent.com/khulnaSoft/shipyard/master/package.json';
...
const makeMsg = (latestVersion) => {
  ...
  let msg = '';
  if (difference <= 0) {
    msg = '\x1b[1m\x1b[32m✅ Shipyard is Up-to-Date\x1b[0m\n';
  } else {
...
axios.get(packageUrl).then((response) => {
  if (response && response.data && response.data.version) {
    logToConsole(`\nUsing Shipyard V-${currentVersion}. Update Check Complete`);
