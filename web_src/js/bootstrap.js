...
function processWindowErrorEvent(e) {
  ...
  if (!e.error && e.lineno === 0 && e.colno === 0 && e.filename === '' && window.navigator.userAgent.includes('FxiOS/')) {
    // At the moment, Firefox (iOS) (10x) has an engine bug. See https://github.com/go-shipyard/shipyard/issues/20240
...
function initGlobalErrorHandler() {
  ...
  if (!window.config) {
    showGlobalErrorMessage(`Shipyard JavaScript code couldn't run correctly, please check your custom templates`);
  }
