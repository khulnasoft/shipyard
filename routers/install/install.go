...
import (
	...
	db_install "github.com/khulnasoft/shipyard/models/db/install"
	"github.com/khulnasoft/shipyard/models/migrations"
	system_model "github.com/khulnasoft/shipyard/models/system"
	user_model "github.com/khulnasoft/shipyard/models/user"
	"github.com/khulnasoft/shipyard/modules/auth/password/hash"
	"github.com/khulnasoft/shipyard/modules/base"
	"github.com/khulnasoft/shipyard/modules/context"
	"github.com/khulnasoft/shipyard/modules/generate"
	...
	"github.com/khulnasoft/shipyard/modules/user"
...
func checkDatabase(ctx *context.Context, form *forms.InstallForm) bool {
	...
	if err = db.InitEngine(ctx); err != nil {
		if strings.Contains(err.Error(), `Unknown database type: sqlite3`) {
			...
			ctx.RenderWithErr(ctx.Tr("install.sqlite3_not_available", "https://docs.shipyard.khulnasoft.com/installation/install-from-binary"), tplInstall, form)
		} else {
	...
	if hasPostInstallationUser && dbMigrationVersion > 0 {
		log.Error("The database is likely to have been used by Shipyard before, database migration version=%d", dbMigrationVersion)
		...
		log.Info("User confirmed re-installation of Shipyard into a pre-existing database")
	}

	if hasPostInstallationUser || dbMigrationVersion > 0 {
		log.Info("Shipyard will be installed in a database with: hasPostInstallationUser=%v, dbMigrationVersion=%v", hasPostInstallationUser, dbMigrationVersion)
	}
...
func SubmitInstall(ctx *context.Context) {
	...
	// Prepare AppDataPath, it is very important for Shipyard
	...
	go func() {
		...
		// What if this duration is not long enough? That's impossible -- if the user can't load the simple page in time, how could they install or use Shipyard in the future ....
