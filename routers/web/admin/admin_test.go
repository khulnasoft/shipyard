...
func TestShadowPassword(t *testing.T) {
	kases := []struct {
		...
		{
			Provider: "redis",
			CfgItem:  "network=tcp,addr=:6379,password=shipyard,db=0,pool_size=100,idle_timeout=180",
			Result:   "network=tcp,addr=:6379,password=******,db=0,pool_size=100,idle_timeout=180",
		},
		{
			Provider: "mysql",
			CfgItem:  "root:@tcp(localhost:3306)/shipyard?charset=utf8",
			Result:   "root:******@tcp(localhost:3306)/shipyard?charset=utf8",
		},
		{
			Provider: "mysql",
			CfgItem:  "/shipyard?charset=utf8",
			Result:   "/shipyard?charset=utf8",
		},
		{
			Provider: "mysql",
