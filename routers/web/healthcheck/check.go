...
// database checks shipyard database status
func checkDatabase(ctx context.Context, checks checks) status {
	...
	if setting.Database.Type.IsSQLite3() && st.Status == pass {
		if !setting.EnableSQLite3 {
			st.Status = fail
			st.Time = getCheckTime()
			log.Error("SQLite3 health check failed with error: %v", "this Shipyard binary is built without SQLite3 enabled")
		} else {
...
// cache checks shipyard cache status
