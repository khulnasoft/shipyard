...
import (
	...
	admin_model "github.com/khulnasoft/shipyard/models/admin"
	...
	issue_model "github.com/khulnasoft/shipyard/models/issues"
	repo_model "github.com/khulnasoft/shipyard/models/repo"
	unit_model "github.com/khulnasoft/shipyard/models/unit"
	user_model "github.com/khulnasoft/shipyard/models/user"
	"github.com/khulnasoft/shipyard/modules/actions"
	...
	"github.com/khulnasoft/shipyard/modules/lfs"
...
func findReadmeFileInEntries(ctx *context.Context, entries []*git.TreeEntry, tryWellKnownDirs bool) (string, *git.TreeEntry, error) {
	...
	docsEntries := make([]*git.TreeEntry, 3) // (one of docs/, .shipyard/ or .github/)
	for _, entry := range entries {
		if tryWellKnownDirs && entry.IsDir() {
			// as a special case for the top-level repo introduction README,
			// fall back to subfolders, looking for e.g. docs/README.md, .shipyard/README.zh-CN.txt, .github/README.txt, ...
			...
			case ".shipyard":
				if entry.Name() == ".shipyard" || docsEntries[1] == nil {
					docsEntries[1] = entry
				}
			case ".github":
...
func renderFile(ctx *context.Context, entry *git.TreeEntry, treeLink, rawLink string) {
	...
	} else if slices.Contains([]string{"CODEOWNERS", "docs/CODEOWNERS", ".shipyard/CODEOWNERS"}, ctx.Repo.TreePath) {
	...
	case isRepresentableAsText:
		...
		if markupType != "" && !shouldRenderSource {
			...
			// Shipyard uses the definition (like most modern editors):
