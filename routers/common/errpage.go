...
func RenderPanicErrorPage(w http.ResponseWriter, req *http.Request, err any) {
	...
	// This recovery handler could be called without Shipyard's web context, so we shouldn't touch that context too much.
	...
	if err != nil {
		...
		_, _ = w.Write([]byte("Internal server error, please collect error logs and report to Shipyard issue tracker"))
	}
}
