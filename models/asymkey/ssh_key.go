...
func AddPublicKeysBySource(ctx context.Context, usr *user_model.User, s *auth.Source, sshPublicKeys []string) bool {
	...
	loop:
		for len(keys) > 0 && err == nil {
			...
			// We ignore options as they are not relevant to Shipyard
...
func SynchronizePublicKeys(ctx context.Context, usr *user_model.User, s *auth.Source, sshPublicKeys []string) bool {
	...
	for _, v := range keys {
		shipyardKeys = append(shipyardKeys, v.OmitEmail())
	}
	...
	if util.SliceSortedEqual(shipyardKeys, providedKeys) {
		log.Trace("synchronizePublicKeys[%s]: Public Keys are already in sync for %s (Source:%v/DB:%v)", s.Name, usr.Name, len(providedKeys), len(shipyardKeys))
		return false
	}
	log.Trace("synchronizePublicKeys[%s]: Public Key needs update for user %s (Source:%v/DB:%v)", s.Name, usr.Name, len(providedKeys), len(shipyardKeys))

	// Add new Public SSH Keys that doesn't already exist in DB
	var newKeys []string
	for _, key := range providedKeys {
		if !util.SliceContainsString(shipyardKeys, key) {
	...
	var shipyardKeysToDelete []string
	for _, shipyardKey := range shipyardKeys {
		if !util.SliceContainsString(providedKeys, shipyardKey) {
			log.Trace("synchronizePublicKeys[%s]: Marking Public SSH Key for deletion for user %s: %v", s.Name, usr.Name, shipyardKey)
			shipyardKeysToDelete = append(shipyardKeysToDelete, shipyardKey)
		}
	}

	// Delete keys from DB that no longer exist in the source
	needUpd, err := deleteKeysMarkedForDeletion(ctx, shipyardKeysToDelete)
	if err != nil {
