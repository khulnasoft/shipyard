...
import (
	...
	"github.com/khulnasoft/shipyard/models/migrations/v1_10"
	"github.com/khulnasoft/shipyard/models/migrations/v1_11"
	"github.com/khulnasoft/shipyard/models/migrations/v1_12"
	"github.com/khulnasoft/shipyard/models/migrations/v1_13"
	"github.com/khulnasoft/shipyard/models/migrations/v1_14"
	"github.com/khulnasoft/shipyard/models/migrations/v1_15"
	"github.com/khulnasoft/shipyard/models/migrations/v1_16"
	"github.com/khulnasoft/shipyard/models/migrations/v1_17"
	"github.com/khulnasoft/shipyard/models/migrations/v1_18"
	"github.com/khulnasoft/shipyard/models/migrations/v1_19"
	"github.com/khulnasoft/shipyard/models/migrations/v1_20"
	"github.com/khulnasoft/shipyard/models/migrations/v1_21"
	"github.com/khulnasoft/shipyard/models/migrations/v1_22"
	"github.com/khulnasoft/shipyard/models/migrations/v1_6"
	"github.com/khulnasoft/shipyard/models/migrations/v1_7"
	"github.com/khulnasoft/shipyard/models/migrations/v1_8"
	"github.com/khulnasoft/shipyard/models/migrations/v1_9"
...
const minDBVersion = 70 // Shipyard 1.5.3
...
func EnsureUpToDate(x *xorm.Engine) error {
	...
	if minDBVersion > currentDB {
		return fmt.Errorf("DB version %d (<= %d) is too old for auto-migration. Upgrade to Shipyard 1.6.4 first then upgrade to this version", currentDB, minDBVersion)
	}

	expected := ExpectedVersion()

	if currentDB != expected {
		return fmt.Errorf(`Current database version %d is not equal to the expected version %d. Please run "shipyard [--config /path/to/app.ini] migrate" to update the database version`, currentDB, expected)
	}
...
func Migrate(x *xorm.Engine) error {
	// Set a new clean the default mapper to GonicMapper as that is the default for Shipyard.
	...
	if minDBVersion > v {
		log.Fatal(`Shipyard no longer supports auto-migration from your previously installed version.
Please try upgrading to a lower version first (suggested v1.6.4), then upgrade to this version.`)
		return nil
	}

	// Downgrading Shipyard's database version not supported
	if int(v-minDBVersion) > len(migrations) {
		msg := fmt.Sprintf("Your database (migration version: %d) is for a newer Shipyard, you can not use the newer database for this old Shipyard release (%d).", v, minDBVersion+len(migrations))
		msg += "\nShipyard will exit to keep your database safe and unchanged. Please use the correct Shipyard release, do not change the migration version manually (incorrect manual operation may lose data)."
		if !setting.IsProd {
