...
When a Shipyard action is running, the Shipyard action server should collect and store all the logs.

The requirements are:
* The running logs must be stored across the cluster if the Shipyard servers are deployed as a cluster.
* The logs will be archived to Object Storage (S3/MinIO, etc.) after a period of time.
* The Shipyard action UI should be able to render the running logs and the archived logs.

Some possible solutions for the running logs:
* [Not ideal] Using local temp file: it can not be shared across the cluster.
* [Not ideal] Using shared file in the filesystem of git repository: although at the moment, the Shipyard cluster's
	git repositories must be stored in a shared filesystem, in the future, Shipyard may need a dedicated Git Service Server
...
* In the future, when Shipyard action needs to limit the log size (other CI/CD services also do so), it's easier to calculate the log file size.
