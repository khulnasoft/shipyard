...
import (
	...
	"github.com/khulnasoft/shipyard/models/system"
	...
	"github.com/khulnasoft/shipyard/modules/setting/config"
...
// shipyardRoot a path to the shipyard root
var (
	shipyardRoot string
	fixturesDir  string
)
...
func MainTest(m *testing.M, testOpts ...*TestOptions) {
	...
	for searchDir != "" {
		if _, err := os.Stat(filepath.Join(searchDir, "go.mod")); err == nil {
			break // The "go.mod" should be the one for Shipyard repository
		}
	...
	if searchDir == "" {
		panic("The tests should run in a Shipyard repository, there should be a 'go.mod' in the root")
	}

	shipyardRoot = searchDir
	setting.CustomPath = filepath.Join(shipyardRoot, "custom")
	InitSettings()

	fixturesDir = filepath.Join(shipyardRoot, "models", "fixtures")
	...
	setting.AppURL = "https://try.shipyard.io/"
	setting.RunUser = "runuser"
	setting.SSH.User = "sshuser"
	setting.SSH.BuiltinServerUser = "builtinuser"
	setting.SSH.Port = 3000
	setting.SSH.Domain = "try.shipyard.io"
	...
	setting.AppWorkPath = shipyardRoot
	setting.StaticRootPath = shipyardRoot
	...
	if err = CopyDir(filepath.Join(shipyardRoot, "tests", "shipyard-repositories-meta"), setting.RepoRootPath); err != nil {
...
func CreateTestEngine(opts FixturesOptions) error {
	...
	switch os.Getenv("SHIPYARD_UNIT_TESTS_LOG_SQL") {
	case "true", "1":
		x.ShowSQL(true)
	}
...
func PrepareTestEnv(t testing.TB) {
	...
	metaPath := filepath.Join(shipyardRoot, "tests", "shipyard-repositories-meta")
	...
	base.SetupShipyardRoot() // Makes sure SHIPYARD_ROOT is set
}
