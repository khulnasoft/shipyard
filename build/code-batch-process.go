...
import (
	...
	"github.com/khulnasoft/shipyard/build/codeformat"
)

// Windows has a limitation for command line arguments, the size can not exceed 32KB.
// So we have to feed the files to some tools (like gofmt) batch by batch

// We also introduce a `shipyard-fmt` command, it does better import formatting than gofmt/goimports. `shipyard-fmt` calls `gofmt` internally.
...
func newFileCollector(fileFilter string, batchSize int) (*fileCollector, error) {
	...
	if fileFilter == "go-own" {
		...
		co.excludePatterns = append(co.excludePatterns, regexp.MustCompile(`tests/shipyard-repositories-meta`))
...
func shipyardFormatGoImports(files []string, doWriteFile bool) error {
	...
	for _, files := range fileBatches {
		...
		case "shipyard-fmt":
			if containsString(subArgs, "-d") {
				log.Print("the -d option is not supported by shipyard-fmt")
			}
			cmdErrors = append(cmdErrors, shipyardFormatGoImports(files, containsString(subArgs, "-w")))
