...
Adding a new theme is really easy. There're two things you need to do: Pass the theme name to Shipyard, so that it can be added to the theme selector dropdown menu, and then write some styles!
...
For full docs about Shipyard's multi-language support, see [Multi-Language Support](./multi-language-support.md)
...
When Shipyard is deployed to Netlify, it is effectively running as a static app, and therefore the server-side code for the Node.js endpoints is not available. However Netlify now supports serverless cloud lambda functions, which can be used to replace most functionality.

### 1. Run Netlify Dev Server

First off all, install the Netlify CLI: `npm install netlify-cli -g`
Then, from within the root of Shipyard's directory, start the server, by running: `netlify dev`
