...
By default Shipyard comes with 25+ built-in themes, which can be applied from the dropdown menu in the UI.

![Built-in Themes](https://i.ibb.co/GV3wRss/Shipyard-Themes.png)

You can also add your own themes, apply custom styles, and modify colors.

You can customize Shipyard by writing your own CSS, which can be loaded either as an external stylesheet, set directly through the UI, or specified in the config file. Most styling options can be set through CSS variables, which are outlined below.
