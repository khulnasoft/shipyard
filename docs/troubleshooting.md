...
This is not an issue with Shipyard, but instead caused by the target app preventing direct access through embedded elements.
...
```text
Header set X-Frame-Options: "ALLOW-FROM http://[shipyard-location]/"
```

### LightHttpd

```text
Content-Security-Policy: frame-ancestors 'self' https://[shipyard-location]/
```
...
If you're seeing Shipyard's 404 page on initial load/ refresh, and then the main app when you go back to Home, then this is likely caused by the Vue router, and if so can be fixed in one of two ways.
...
```text
Access to XMLHttpRequest at 'https://example.com/raw/my-config.yml' from origin 'http://shipyard.local' has been blocked by CORS policy:
No 'Access-Control-Allow-Origin' header is present on the requested resource.
```

The solution is to add the appropriate headers onto the target server, to allow it to accept requests from the origin where you're running Shipyard.
...
```yaml
volumes:
- /srv/shipyard/conf.yml:/app/user-data/conf.yml
- /srv/shipyard/item-icons:/app/public/item-icons
```
...
You need to allow Shipyard to make requests to Keycloak, and Keycloak to redirect to Shipyard. The way you do this depends on how you're hosting these applications / which proxy you are using, and examples can be found in the [Management Docs](/docs/management.md#setting-headers).

For example, add the access control header to Keycloak, like:

`Access-Control-Allow-Origin [URL-of Shipyard]`
...
```text
nginx.ingress.kubernetes.io/cors-allow-origin: "https://shipyard.example.com"
...
If you find that your styles and other visual assets work when visiting `ip:port` by not `shipyard.domain.com`, then this is usually caused by caching. In your browser, do a hard-refresh (<kbd>Ctrl</kbd> + <kbd>F5</kbd>). If you use Cloudflare, then you can clear the cache through the management console, or set the cache level to Bypass for certain files, under the Rules tab.
...
```text
Continuing execution. Pulling image khulnasoft/shipyard:release-1.6.0
...
- Shipyard is also available through GHCR, which at present does not have any hard limits. Just use `docker pull ghcr.io/khulnasoft/shipyard:latest` to fetch the image
- You can also build the image from source, by cloning the repo, and running `docker build -t shipyard .` or use the pre-made docker compose
...
The most common widget issue is a CORS error. This is a browser security mechanism which prevents the client-side app (Shipyard) from from accessing resources on a remote origin, without that server's explicit permission (e.g. with headers like Access-Control-Allow-Origin). See the MDN Docs for more info: [Cross-Origin Resource Sharing](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS).
...
You can route requests through Shipyard's built-in CORS proxy. Instructions and more details can be found [here](/docs/widgets.md#proxying-requests). If you don't have control over the target origin, and you are running Shipyard either through Docker, with the Node server or on Netlify, then this solution will work for you.
...
A 401 error means your API key is invalid, it is not an issue with Shipyard.
...
You can also view any and all data that Shipyard is storing, using the developer tools. Open your browser's dev tools (usually <kbd>F12</kbd>), in Chromium head to the Application tab, or in Firefox go to the Storage tab. Select Local Storage, then scroll down the the URL Shipyard is running on. You should now see all data being stored, and you can select and delete any fields you wish.
...
Complete the fields, asking for your environment info and version of Shipyard.
