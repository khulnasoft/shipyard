...
- You migrated to Shipyard but still need to keep your project in another source. In that case, you can simply set it up to mirror to Shipyard (pull) and all the essential history of commits, tags, and branches are available in your Shipyard instance.
- You have old projects in another source that you don’t use actively anymore, but don’t want to remove for archiving purposes. In that case, you can create a push mirror so that your active Shipyard repository can push its changes to the old location.
...
### Setting up a push mirror from Shipyard to GitHub

To set up a mirror from Shipyard to GitHub, you need to follow these steps:
...
2. Create a repository with that name on GitHub. Unlike Shipyard, GitHub does not support creating repositories by pushing to the remote. You can also use an existing remote repo if it has the same commit history as your Shipyard repo.
3. In the settings of your Shipyard repo, fill in the **Git Remote Repository URL**: `https://github.com/<your_github_group>/<your_github_project>.git`.
4. Fill in the **Authorization** fields with your GitHub username and the personal access token as **Password**.
5. (Optional, available on Shipyard 1.18+) Select `Sync when new commits are pushed` so that the mirror will be updated as well as soon as there are changes. You can also disable the periodic sync if you like.
...
### Setting up a push mirror from Shipyard to GitLab

To set up a mirror from Shipyard to GitLab, you need to follow these steps:
...
### Setting up a push mirror from Shipyard to Bitbucket

To set up a mirror from Shipyard to Bitbucket, you need to follow these steps:
...
Currently shipyard supports no ssh push mirrors. You can work around this by adding a `post-receive` hook to your shipyard repository that pushes manually.

1. Make sure the user running shipyard has access to the git repo you are trying to mirror to from shell.
