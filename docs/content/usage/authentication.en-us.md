...
- Username attribute (optional)
  ...
  - The attribute of the user's LDAP record containing the user name. Given
    attribute value will be used for new Shipyard account user name after first
  ...
  - This is useful when supplied login name is matched against multiple
    attributes, but only single specific attribute should be used for Shipyard
...
- Enable user synchronization
  - This option enables a periodic task that synchronizes the Shipyard users with
    ...
    the [sample
    app.ini](https://github.com/go-shipyard/shipyard/blob/main/custom/conf/app.example.ini)
    for detailed comments about that section. The _User Search Base_ and _User
    Filter_ settings described above will limit which users can use Shipyard and
...
- Verify group membership in LDAP (optional)
  ...
  - Example: `(|(cn=shipyard_users)(cn=admins))`
...
authentication.  To work with normal Linux passwords, the user running Shipyard
must also have read access to `/etc/shadow` in order to check the validity of
the account when logging in using a public key.

**Note**: If a user has added SSH public keys into Shipyard, the use of these
keys _may_ bypass the login check system.  Therefore, if you wish to disable a user who
authenticates with PAM, you _should_ also manually disable the account in Shipyard using the
...
1. Fill out the field as follows:
    ...
    field is set to `mail.com`, then Shipyard will expect the `user email` field
...
user will log into the Shipyard web interface as `gituser` and not `gituser@mail.com`

## SMTP (Simple Mail Transfer Protocol)

This option allows Shipyard to log in to an SMTP host as a Shipyard user. To
...
- Allowed Domains
  ...
  - Example: `shipyard.io,mydomain.com,mydomain2.com`
...
- In order to log in to Shipyard using FreeIPA credentials, a bind account needs to
  be created for Shipyard:

- On the FreeIPA server, create a `shipyard.ldif` file, replacing `dc=example,dc=com`
  with your DN, and provide an appropriately secure password:

  ```sh
  dn: uid=shipyard,cn=sysaccounts,cn=etc,dc=example,dc=com
  changetype: add
  objectclass: account
...
- Import the LDIF (change localhost to an IPA server if needed). A prompt for
  ...
  "cn=Directory Manager" -W -f shipyard.ldif
  ```

- Add an IPA group for shipyard_users :

  ```sh
  ipa group-add --desc="Shipyard Users" shipyard_users
  ```

- Note: For errors about IPA credentials, run `kinit admin` and provide the
  domain admin account password.

- Log in to Shipyard as an Administrator and click on "Authentication" under Admin Panel.
...
Shipyard supports SPNEGO single sign-on authentication (the scheme defined by RFC4559) for the web part of the server via the Security Support Provider Interface (SSPI) built in Windows. SSPI works only in Windows environments - when both the server and the clients are running Windows.

Before activating SSPI single sign-on authentication (SSO) you have to prepare your environment:

- Create a separate user account in active directory, under which the `shipyard.exe` process will be running (eg. `user` under domain `domain.local`):

- Create a service principal name for the host where `shipyard.exe` is running with class `HTTP`:
...
- Start the web server (`shipyard.exe web`)
...
- Sign in to a client computer in the same domain with any domain user (client computer, different from the server running `shipyard.exe`)
...
- Start Chrome or Edge and navigate to the FQDN URL of Shipyard (eg. `http://host.domain.local:3000`)
...
- If it does not work, make sure that:
  - You are not running the web browser on the same server where Shipyard is running. You should be running the web browser on a domain joined computer (client) that is different from the server. If both the client and server are running on the same computer NTLM will be preferred over Kerberos.
...
Shipyard supports Reverse Proxy Header authentication, it will read headers as a trusted login user name or user email address. This hasn't been enabled by default, you can enable it with
