...
If a password were later to be compromised, logging into Shipyard will not be possible without the additional credentials and the account would remain secure.
Shipyard supports both TOTP (Time-based One-Time Password) tokens and FIDO-based hardware keys using the Webauthn API.
...
The Shipyard API supports providing the relevant TOTP password in the `X-Shipyard-OTP` header, as described in [API Usage](development/api-usage.md).
