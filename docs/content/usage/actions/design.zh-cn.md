...
Shipyard Actions由多个组件组成。本文档将对它们进行逐个描述。

## Act

[nektos/act](https://github.com/nektos/act) 项目是一个优秀的工具，允许你在本地运行GitHub Actions。
我们受到了它的启发，并思考它是否可能为Shipyard运行Actions。

然而，尽管[nektos/act](https://github.com/nektos/act)被设计为一个命令行工具，但我们实际上需要的是一个专为Shipyard修改的Go库。
因此，我们在[shipyard/act](https://shipyard.khulnasoft.com/shipyard/act)基础上进行了分叉。

这是一个软分叉，将定期跟进上游。
虽然添加了一些自定义提交，但我们会尽力避免对原始代码进行太多更改。

分叉的 act 只是Shipyard特定用途的桥接或适配器。
还添加了一些额外的提交，例如：

- 将执行日志输出到日志记录器钩子，以便报告给Shipyard
- 禁用 GraphQL URL，因为Shipyard不支持它
...
Shipyard的Runner被称为act runner，因为它基于act。

与其他CIRunner一样，我们将其设计为Shipyard的外部部分，这意味着它应该在与Shipyard不同的服务器上运行。

为了确保Runner连接到正确的Shipyard实例，我们需要使用令牌注册它。
此外，Runner通过声明自己的标签向Shipyard报告它可以运行的Job类型。
...
由于act runner是Shipyard的独立部分，我们需要一种协议让Runner与Shipyard实例进行通信。
然而，我们不认为让Shipyard监听一个新端口是个好主意。
相反，我们希望重用HTTP端口，这意味着我们需要一个与HTTP兼容的协议。
因此，我们选择使用基于HTTP的gRPC。

我们使用[actions-proto-def](https://shipyard.khulnasoft.com/shipyard/actions-proto-def) 和 [actions-proto-go](https://shipyard.khulnasoft.com/shipyard/actions-proto-go) 进行连接。
...
### 连接 1，act runner到Shipyard实例

act runner 必须能够连接到Shipyard以接收任务并发送执行结果回来。

### 连接 2，Job容器到Shipyard实例

即使Job容器位于同一台机器上，它们的网络命名空间与Runner不同。
举个例子，如果工作流中包含 `actions/checkout@v4`，Job容器需要连接到Shipyard来获取代码。
获取代码并不总是运行某些Job所必需的，但在大多数情况下是必需的。

如果您使用回环地址注册Runner，当Runner与Shipyard在同一台机器上时，Runner可以连接到Shipyard。
然而，如果Job容器尝试从本地主机获取代码，它将失败，因为Shipyard不在同一个容器中。

### 连接 3，act runner到互联网

当您使用诸如 `actions/checkout@v4` 的一些Actions时，act runner下载的是脚本，而不是Job容器。
默认情况下，它从[shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/)下载，因此需要访问互联网。
它还默认从Docker Hub下载一些Docker镜像，这也需要互联网访问。

然而，互联网访问并不是绝对必需的。
您可以配置您的Shipyard实例从您的内部网络设施中获取 Actions 或镜像。

实际上，您的Shipyard实例可以同时充当 Actions 市场和镜像注册表。
您可以将GitHub上的Actions仓库镜像到您的Shipyard实例，并将其用作普通Actions。
而 [Shipyard 容器注册表](usage/packages/container.md) 可用作Docker镜像注册表。
...
使用Shipyard Actions只需要确保Runner能够连接到Shipyard实例。
互联网访问是可选的，但如果没有互联网访问，将需要额外的工作。
换句话说：当Runner能够自行查询互联网时，它的工作效果最好，但您不需要将其暴露给互联网（无论是单向还是双向）。

如果您在使用Shipyard Actions时遇到任何网络问题，希望上面的图片能够帮助您进行故障排除。
