...
本页面将详细介绍[Act Runner](https://shipyard.khulnasoft.com/shipyard/act_runner)，这是Shipyard Actions的Runner。
...
您可以从[发布页面](https://shipyard.khulnasoft.com/shipyard/act_runner/releases)下载二进制文件。
然而，如果您想使用最新的夜间构建版本，可以从[下载页面](https://dl.shipyard.khulnasoft.com/act_runner/)下载。
...
您可以使用[docker hub](https://hub.docker.com/r/shipyard/act_runner/tags)上的Docker镜像。
...
在运行Act Runner之前，我们需要首先将其注册到您的Shipyard实例中。

## 注册

在运行Act Runner之前，需要进行注册，因为Runner需要知道从哪里获取Job，并且对于Shipyard实例来说，识别Runner也很重要。
...
注册令牌也可以通过 Shipyard 的 [命令行](administration/command-line.md#actions-generate-runner-token) 获得:
...
- Shipyard 实例的 URL，例如 `https://shipyard.khulnasoft.com/` 或 `http://192.168.8.8:3000/`。
- 注册令牌。
...
从 Shipyard 1.21 开始，您可以通过修改 runner 的配置文件中的 `container.labels` 来更改标签（如果没有配置文件，请参考 [配置教程](#配置)），通过执行 `./act_runner daemon --config config.yaml` 命令重启 runner 之后，这些新定义的标签就会生效。
...
Runner将从Shipyard实例获取Job并自动运行它们。
...
- 变量名称不能以 `GITHUB_` 和 `SHIPYARD_` 前缀开头。
