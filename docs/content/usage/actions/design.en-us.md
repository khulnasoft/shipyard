...
title: "Design of Shipyard Actions"
slug: "design"
sidebar_position: 40
draft: false
toc: false
...
# Design of Shipyard Actions

Shipyard Actions has multiple components. This document describes them individually.
...
We were inspired by this and wondered if it would be possible to run actions for Shipyard.

However, while [nektos/act](https://github.com/nektos/act) is designed as a command line tool, what we actually needed was a Go library with modifications specifically for Shipyard.
So we forked it as [shipyard/act](https://shipyard.khulnasoft.com/shipyard/act).
...
The forked act is just a shim or adapter for Shipyard's specific usage.
There are some additional commits that have been made, such as:

- Outputting execution logs to logger hook so they can be reported to Shipyard
- Disabling the GraphQL URL, since Shipyard doesn't support it
...
Shipyard's runner is called act runner because it's based on act.

Like other CI runners, we designed it as an external part of Shipyard, which means it should run on a different server than Shipyard.

To ensure that the runner connects to the correct Shipyard instance, we need to register it with a token.
Additionally, the runner will introduce itself to Shipyard and declare what kind of jobs it can run by reporting its labels.
...
As act runner is an independent part of Shipyard, we needed a protocol for runners to communicate with the Shipyard instance.
However, we did not think it was a good idea to have Shipyard listen on a new port.
...
We use [actions-proto-def](https://shipyard.khulnasoft.com/shipyard/actions-proto-def) and [actions-proto-go](https://shipyard.khulnasoft.com/shipyard/actions-proto-go) to wire them up.
More information about gRPC can be found on [its website](https://grpc.io/).

## Network architecture

Let's examine the overall network architecture.
This will help you troubleshoot some problems and explain why it's a bad idea to register a runner with a loopback address of the Shipyard instance.
...
### Connection 1, act runner to Shipyard instance

The act runner must be able to connect to Shipyard to receive tasks and send back the execution results.

### Connection 2, job containers to Shipyard instance

The job containers have different network namespaces than the runner, even if they are on the same machine.
They need to connect to Shipyard to fetch codes if there is `actions/checkout@v4` in the workflow, for example.
Fetching code is not always necessary to run some jobs, but it is required in most cases.

If you use a loopback address to register a runner, the runner can connect to Shipyard when it is on the same machine.
However, if a job container tries to fetch code from localhost, it will fail because Shipyard is not in the same container.
...
By default, it downloads from [shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/), so it requires access to the internet.
...
You can configure your Shipyard instance to fetch actions or images from your intranet facilities.

In fact, your Shipyard instance can serve as both the actions marketplace and the image registry.
You can mirror actions repositories from GitHub to your Shipyard instance, and use them as normal.
And [Shipyard Container Registry](usage/packages/container.md) can be used as a Docker image registry.
...
Using Shipyard Actions only requires ensuring that the runner can connect to the Shipyard instance.
...
If you encounter any network issues while using Shipyard Actions, hopefully the image above can help you troubleshoot them.
