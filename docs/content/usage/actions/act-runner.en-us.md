...
This page will introduce the [act runner](https://shipyard.khulnasoft.com/shipyard/act_runner) in detail, which is the runner of Shipyard Actions.
...
You can download the binary from the [release page](https://shipyard.khulnasoft.com/shipyard/act_runner/releases).
However, if you want to use the latest nightly build, you can download it from the [download page](https://dl.shipyard.khulnasoft.com/act_runner/).
...
You can use the docker image from the [docker hub](https://hub.docker.com/r/shipyard/act_runner/tags).
...
```bash
docker pull shipyard/act_runner:latest # for the latest stable release
docker pull shipyard/act_runner:nightly # for the latest nightly build
```
...
```bash
docker run --entrypoint="" --rm -it shipyard/act_runner:latest act_runner generate-config > config.yaml
```
...
Before running the act runner, we need to register it to your Shipyard instance first.

## Registration

Registration is required before running the act runner, because the runner needs to know where to get jobs from.
And it is also important to Shipyard instance to identify the runner.
...
A registration token can also be obtained from the shipyard [command-line interface](administration/command-line.md#actions-generate-runner-token):

```
shipyard --config /etc/shipyard/app.ini actions generate-runner-token
```
...
- The Shipyard instance URL, like `https://shipyard.khulnasoft.com/` or `http://192.168.8.8:3000/`.
...
docker run \
    ...
    -e SHIPYARD_INSTANCE_URL=<instance_url> \
    -e SHIPYARD_RUNNER_REGISTRATION_TOKEN=<registration_token> \
    -e SHIPYARD_RUNNER_NAME=<runner_name> \
    -e SHIPYARD_RUNNER_LABELS=<runner_labels> \
    --name my_runner \
    -d shipyard/act_runner:nightly
```
...
services:
  runner:
    image: shipyard/act_runner:nightly
    environment:
      CONFIG_FILE: /config.yaml
      SHIPYARD_INSTANCE_URL: "${INSTANCE_URL}"
      SHIPYARD_RUNNER_REGISTRATION_TOKEN: "${REGISTRATION_TOKEN}"
      SHIPYARD_RUNNER_NAME: "${RUNNER_NAME}"
      SHIPYARD_RUNNER_LABELS: "${RUNNER_LABELS}"
    volumes:
...
Starting with Shipyard 1.21, you can change labels by modifying `container.labels` in the runner configuration file (if you don't have a configuration file, please refer to [configuration tutorials](#configuration)).
...
The runner will fetch jobs from the Shipyard instance and run them automatically.
...
```ini
[Unit]
Description=Shipyard Actions runner
Documentation=https://shipyard.khulnasoft.com/shipyard/act_runner
...
- Variable names must not start with the `GITHUB_` and `SHIPYARD_` prefix.
