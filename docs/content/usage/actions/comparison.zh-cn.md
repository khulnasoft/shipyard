...
尽管Shipyard Actions旨在与GitHub Actions兼容，但它们之间存在一些差异。

## 额外功能

### Action URL绝对路径

Shipyard Actions支持通过URL绝对路径定义actions，这意味着您可以使用来自任何Git存储库的Actions。
例如，`uses: https://github.com/actions/checkout@v4`或`uses: http://your_shipyard.khulnasoft.com/owner/repo@branch`。

### 使用Go编写Actions

Shipyard Actions支持使用Go编写Actions。
请参阅[创建Go Actions](https://blog.shipyard.khulnasoft.com/creating-go-actions/)。
...
Shipyard Actions目前只支持`runs-on: xyz`或`runs-on: [xyz]`。
...
Shipyard Actions目前不支持此功能，如果使用它，结果将始终为空字符串。

作为解决方法，您可以使用[go-hashfiles](https://shipyard.khulnasoft.com/actions/go-hashfiles)。
...
当 `[actions].DEFAULT_ACTIONS_URL` 保持默认值为 `github` 时，Shipyard将会从 https://github.com 下载相对路径的actions。比如：
如果你使用 `uses: actions/checkout@v4`，Shipyard将会从 https://github.com/actions/checkout.git 下载这个 actions 项目。
如果你想要从另外一个 Git服务下载actions，你只需要使用绝对URL `uses: https://shipyard.khulnasoft.com/actions/checkout@v4` 来下载。

如果你的 Shipyard 实例是部署在一个互联网限制的网络中，有可以使用绝对地址来下载 actions。你也可以讲配置项修改为 `[actions].DEFAULT_ACTIONS_URL = self`。这样所有的相对路径的actions引用，将不再会从 github.com 去下载，而会从这个 Shipyard 实例自己的仓库中去下载。例如： `uses: actions/checkout@v4` 将会从 `[server].ROOT_URL`/actions/checkout.git 这个地址去下载 actions。
