...
Even though Shipyard Actions is designed to be compatible with GitHub Actions, there are some differences between them.

## Additional features

### Absolute action URLs

Shipyard Actions supports defining actions via absolute URL, which means that you can use actions from any git repository.
Like `uses: https://github.com/actions/checkout@v4` or `uses: http://your_shipyard.khulnasoft.com/owner/repo@branch`.

### Actions written in Go

Shipyard Actions supports writing actions in Go.
See [Creating Go Actions](https://blog.shipyard.khulnasoft.com/creating-go-actions/).
...
It's ignored by Shipyard Actions now.
...
Shipyard Actions only supports `runs-on: xyz` or `runs-on: [xyz]` now.
...
Shipyard Actions doesn't support it now, if you use it, the result will always be empty string.

As a workaround, you can use [go-hashfiles](https://shipyard.khulnasoft.com/actions/go-hashfiles) instead.
...
Previously (Pre 1.21.0), `[actions].DEFAULT_ACTIONS_URL` defaulted to `https://shipyard.khulnasoft.com`.
We have since restricted this option to only allow two values (`github` and `self`).
When set to `github`, the new default, Shipyard will download non-fully-qualified actions from `https://github.com`.
...
If you want to download an action from another git hoster, you can use an absolute URL, e.g. `uses: https://shipyard.khulnasoft.com/actions/checkout@v4`.

If your Shipyard instance is in an intranet or a restricted area, you can set the URL to `self` to only download actions from your own instance by default.
