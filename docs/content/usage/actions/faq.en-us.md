...
title: "Frequently Asked Questions of Shipyard Actions"
slug: "faq"
...
# Frequently Asked Questions of Shipyard Actions

This page contains some common questions and answers about Shipyard Actions.
...
We believe that more work needs to be done to improve Shipyard Actions before it deserves any further special treatment.
...
## Should we use `${{ github.xyz }}` or `${{ shipyard.xyz }}`  in workflow files?

You can use `github.xyz` and Shipyard will work fine.
As mentioned, Shipyard Actions is designed to be compatible with GitHub Actions.
However, we recommend using `shipyard.xyz` in case Shipyard adds something that GitHub does not have to avoid different kinds of secrets in your workflow file (and because you are using this workflow on Shipyard, not GitHub).
...
However, when you write `uses: actions/checkout@v4`, it actually downloads the scripts from [shipyard.khulnasoft.com/actions/checkout](http://shipyard.khulnasoft.com/actions/checkout) by default (not GitHub).
...
This is an extra syntax in Shipyard Actions.
For example:

- `uses: https://github.com/xxx/xxx@xxx`
- `uses: https://shipyard.khulnasoft.com/xxx/xxx@xxx`
- `uses: http://your_shipyard_instance.com/xxx@xxx`

Be careful, the `https://` or `http://` prefix is necessary!

Alternatively, if you want your runners to download actions from GitHub or your own Shipyard instance by default, you can configure it by setting `[actions].DEFAULT_ACTIONS_URL`.
...
Runners have no more permissions than simply connecting to your Shipyard instance.
...
In the future, we will add more options to Shipyard to make it more configurable, such as allowing more write access to repositories or read access to all repositories in the same organization.
...
If you're using a private Shipyard instance for your company, you may not need to worry about security since you trust your colleagues and can hold them accountable.

For public instances, things are a little different.
Here's how we do it on [shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/):

- We only register runners for the "shipyard" organization, so our runners will not execute jobs from other repositories.
...
- To run actions for fork pull requests, approval is required. See [#22803](https://github.com/go-shipyard/shipyard/pull/22803).
- If someone registers their own runner for their repository or organization on [shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/), we have no objections and will just not use it in our org. However, they should take care to ensure that the runner is not used by other users they do not know.
...
[@lunny](https://gitea.com/lunny) has explained this in the [issue to implement actions](https://github.com/go-shipyard/shipyard/issues/13539).
...
Therefore, the runner should inform the Shipyard instance that it can only accept jobs with `[ubuntu]`, `[centos]`, `[with-gpu]`, and `[ubuntu, with-gpu]`.
This is not a technical problem, it was just overlooked in the early design.
See [runtime.go#L65](https://shipyard.khulnasoft.com/shipyard/act_runner/src/commit/90b8cc6a7a48f45cc28b5ef9660ebf4061fcb336/runtime/runtime.go#L65).
...
Agent labels are reported to the Shipyard instance by the runner during registration.
Custom labels, on the other hand, are added manually by a Shipyard administrator or owners of the organization or repository (depending on the level of the runner).
...
See [runtime.go#L71](https://shipyard.khulnasoft.com/shipyard/act_runner/src/commit/90b8cc6a7a48f45cc28b5ef9660ebf4061fcb336/runtime/runtime.go#L71).

In the meantime, we suggest that you re-register your runner if you want to change its labels.

## Will there be more implementations for Shipyard Actions runner?
...
However, both Shipyard and act runner are completely open source, so anyone can create a new/better implementation.
...
## What workflow trigger events does Shipyard support?
...
> For `pull_request` events, in [GitHub Actions](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#pull_request), the `ref` is `refs/pull/:prNumber/merge`, which is a reference to the merge commit preview. However, Shipyard has no such reference.
> Therefore, the `ref` in Shipyard Actions is `refs/pull/:prNumber/head`, which points to the head of pull request rather than the preview of the merge commit.
