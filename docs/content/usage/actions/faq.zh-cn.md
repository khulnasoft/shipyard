...
# Shipyard Actions常见问题解答

本页面包含一些关于Shipyard Actions的常见问题和答案。

## 为什么默认情况下不启用Actions？

我们知道为整个实例和每个仓库启用Actions可能很麻烦，但并不是每个人都喜欢或需要此功能。
在我们认为Shipyard Actions值得被特别对待之前，我们认为还需要做更多的工作来改进它。
...
## 在工作流文件中应该使用`${{ github.xyz }}`还是`${{ shipyard.xyz }}`？

您可以使用`github.xyz`，Shipyard将正常工作。
如前所述，Shipyard Actions的设计是与GitHub Actions兼容的。
然而，我们建议在工作流文件中使用`shipyard.xyz`，以防止在工作流文件中出现不同类型的密钥（因为您在Shipyard上使用此工作流，而不是GitHub）。
...
然而，当您编写`uses: actions/checkout@v4`时，它实际上默认从[shipyard.khulnasoft.com/actions/checkout](http://shipyard.khulnasoft.com/actions/checkout)下载脚本（而不是从GitHub下载）。
...
另外，如果您希望您的Runner默认从GitHub或您自己的Shipyard实例下载Actions，可以通过设置 `[actions].DEFAULT_ACTIONS_URL`进行配置。
...
在未来，我们将添加更多选项以使Shipyard更可配置，例如允许对仓库进行更多写访问或对同一组织中的所有仓库进行读访问。
...
后者要复杂一些。
如果您为公司使用私有的Shipyard实例，您可能不需要担心安全问题，因为您信任您的同事，并且可以追究他们的责任。

对于公共实例，情况略有不同。
以下是我们在 [shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/)上的做法：

- 我们仅为 "shipyard" 组织注册Runner，因此我们的Runner不会执行来自其他仓库的Job。
- 我们的Runner始终在隔离容器中运行Job。虽然可以直接在主机上进行这样的操作，但出于安全考虑，我们选择不这样做。
- 对于 fork 的拉取请求，需要获得批准才能运行Actions。参见[#22803](https://github.com/go-shipyard/shipyard/pull/22803)。
- 如果有人在[shipyard.khulnasoft.com](http://shipyard.khulnasoft.com/)为其仓库或组织注册自己的Runner，我们不会反对，只是不会在我们的组织中使用它。然而，他们应该注意确保该Runner不被他们不认识的其他用户使用。
...
[@lunny](https://gitea.com/lunny)在实现Actions的[问题](https://github.com/go-shipyard/shipyard/issues/13539)中已经解释过这个问题。
...
因此，Runner应该通知Shipyard实例它只能接受具有 `[ubuntu]`、`[centos]`、`[with-gpu]` 和 `[ubuntu, with-gpu]`的Job。
这不是一个技术问题，只是在早期设计中被忽视了。
参见[runtime.go#L65](https://shipyard.khulnasoft.com/shipyard/act_runner/src/commit/90b8cc6a7a48f45cc28b5ef9660ebf4061fcb336/runtime/runtime.go#L65)。
...
代理标签是由Runner在注册过程中向Shipyard实例报告的。
而自定义标签则是由Shipyard的管理员或组织或仓库的所有者手动添加的（取决于Runner所属的级别）。
...
这个默认值可能与用户的期望不符。
参见[runtime.go#L71](https://shipyard.khulnasoft.com/shipyard/act_runner/src/commit/90b8cc6a7a48f45cc28b5ef9660ebf4061fcb336/runtime/runtime.go#L71)。

与此同时，如果您想更改Runner的标签，我们建议您重新注册Runner。

## Shipyard Actions runner会有更多的实现吗？

虽然我们希望提供更多的选择，但由于我们有限的人力资源，act runner将是唯一受支持的官方Runner。
然而，无论您如何决定，Shipyard 和act runner都是完全开源的，所以任何人都可以创建一个新的/更好的实现。
我们支持您的选择，无论您如何决定。
如果您选择分支act runner来创建自己的版本，请在您认为您的更改对其他人也有帮助的情况下贡献这些更改。

## Shipyard 支持哪些工作流触发事件？
...
> 对于 `pull_request` 事件，在 [GitHub Actions](https://docs.github.com/en/actions/using-workflows/events-that-trigger-workflows#pull_request) 中 `ref` 是 `refs/pull/:prNumber/merge`，它指向这个拉取请求合并提交的一个预览。但是 Shipyard 没有这种 reference。
> 因此，Shipyard Actions 中 `ref` 是 `refs/pull/:prNumber/head`，它指向这个拉取请求的头分支而不是合并提交的预览。
