...
Shipyard (starting with version `1.11.0`) supports creating template repositories
...
template except for the `.shipyard/template` file. The `.shipyard/template` file tells
Shipyard which files are subject to the variable expansion when creating a
repository from the template.

Shipyard uses [gobwas/glob](https://github.com/gobwas/glob) for its glob syntax. It closely resembles a traditional `.gitignore`, however there may be slight differences.

## Example `.shipyard/template` file
...
Shipyard `1.12.0` adds a few transformers to some of the applicable variables above.
