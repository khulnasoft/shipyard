...
Shipyard supports permissions for repository so that you can give different access for different people. At first, we need to know about `Unit`.

## Unit

In Shipyard, we call a sub module of a repository `Unit`. Now we have following possible units.
