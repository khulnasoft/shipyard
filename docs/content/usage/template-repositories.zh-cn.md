...
Shipyard `1.11.0` 及以上版本引入了模板仓库，并且其中一个实现的功能是自动展开模板文件中的特定变量。

要告诉 Shipyard 哪些文件需要展开，您必须在模板仓库的 `.shipyard` 目录中包含一个 `template` 文件。

Shipyard 使用 [gobwas/glob](https://github.com/gobwas/glob) 作为其 glob 语法。它与传统的 `.gitignore` 语法非常相似，但可能存在细微的差异。

## `.shipyard/template` 文件示例
...
**注意：** 当从模板生成仓库时，`.shipyard` 目录中的 `template` 文件将被删除。
...
Shipyard `1.12.0` 添加了一些转换器以应用于上述适用的变量。
