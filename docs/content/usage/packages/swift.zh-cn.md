...
```shell
swift package-registry set https://shipyard.example.com/api/packages/{owner}/swift -login {username} -password {password}
```
...
| `password` | 您的 Shipyard 密码。如果您使用两步验证或 OAuth，请使用[个人访问令牌](development/api-usage.md#通过-api-认证)代替密码。 |
...
curl -X PUT --user {username}:{password} \
	 ...
	 -F metadata={metadata} \
	 https://shipyard.example.com/api/packages/{owner}/swift/{scope}/{name}/{version}
```
