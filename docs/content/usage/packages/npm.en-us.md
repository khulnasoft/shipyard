...
```shell
npm config set {scope}:registry https://shipyard.example.com/api/packages/{owner}/npm/
npm config set -- '//shipyard.example.com/api/packages/{owner}/npm/:_authToken' "{token}"
```
...
```shell
npm config set @test:registry https://shipyard.example.com/api/packages/testuser/npm/
npm config set -- '//shipyard.example.com/api/packages/testuser/npm/:_authToken' "personal_access_token"
```

or without scope:

```shell
npm config set registry https://shipyard.example.com/api/packages/testuser/npm/
npm config set -- '//shipyard.example.com/api/packages/testuser/npm/:_authToken' "personal_access_token"
```
...
The registry supports [searching](https://docs.npmjs.com/cli/v7/commands/npm-search/) but does not support special search qualifiers like `author:shipyard`.
