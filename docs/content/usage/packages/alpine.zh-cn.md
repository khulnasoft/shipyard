...
```
https://shipyard.example.com/api/packages/{owner}/alpine/<branch>/<repository>
```
...
```
https://{username}:{your_password_or_token}@shipyard.example.com/api/packages/{owner}/alpine/<branch>/<repository>
```

Alpine 注册表文件使用 RSA 密钥进行签名，apk 必须知道该密钥。下载公钥并将其存储在 `/etc/apk/keys/` 目录中：

```shell
curl -JO https://shipyard.example.com/api/packages/{owner}/alpine/key
```
...
```
PUT https://shipyard.example.com/api/packages/{owner}/alpine/{branch}/{repository}
```
...
curl --user your_username:your_password_or_token \
     ...
     https://shipyard.example.com/api/packages/testuser/alpine/v3.17/main
```
...
```
DELETE https://shipyard.example.com/api/packages/{owner}/alpine/{branch}/{repository}/{architecture}/{filename}
```
...
curl --user your_username:your_token_or_password -X DELETE \
     https://shipyard.example.com/api/packages/testuser/alpine/v3.17/main/test-package-1.0.0.apk
```
