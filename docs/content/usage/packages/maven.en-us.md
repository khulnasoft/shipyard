...
<repositories>
  <repository>
    <id>shipyard</id>
    <url>https://shipyard.example.com/api/packages/{owner}/maven</url>
  </repository>
</repositories>
...
When you plan to add some packages from Shipyard instance in your project, you should add it in repositories section:

```groovy
repositories {
    // other repositories
    maven { url "https://shipyard.example.com/api/packages/{owner}/maven" }
}
```
...
publishing {
    ...
    repositories {
        maven {
            name = "Shipyard"
            url = uri("https://shipyard.example.com/api/packages/{owner}/maven")
...
Or call `gradle` with task `publishAllPublicationsToShipyardRepository` in case you are using gradle:

```groovy
./gradlew publishAllPublicationsToShipyardRepository
```
...
```shell
mvn deploy:deploy-file -Durl=https://shipyard.example.com/api/packages/{owner}/maven -DrepositoryId=shipyard -Dfile=/path/to/package.jar
```
