...
```shell
dotnet nuget add source --name {source_name} --username {username} --password {password} https://shipyard.example.com/api/packages/{owner}/nuget/index.json
```

| 参数          | 描述                                                                                                                                   |
| ------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `source_name` | 所需源名称                                                                                                                             |
| `username`    | 您的Shipyard用户名                                                                                                                        |
| `password`    | 您的Shipyard密码。如果您使用2FA或OAuth，请使用[个人访问令牌](development/api-usage.md#通过-api-认证)代替密码。 |
| `owner`       | 软件包的所有者                                                                                                                         |

例如：

```shell
dotnet nuget add source --name shipyard --username testuser --password password123 https://shipyard.example.com/api/packages/testuser/nuget/index.json
```
...
```shell
dotnet nuget push --source shipyard test_package.1.0.0.nupkg
```
...
```
https://shipyard.example.com/api/packages/{owner}/nuget/symbols
```
...
```
https://shipyard.example.com/api/packages/testuser/nuget/symbols
```
...
```shell
dotnet add package --source shipyard --version 1.0.0 test_package
```
