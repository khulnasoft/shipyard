...
Cargo stores information about the available packages in a package index stored in a git repository.
In Shipyard this repository has the special name `_cargo-index`.
...
This can be useful if for example the Shipyard instance domain was changed.

If the case arises where the packages stored in Shipyard and the information in the index repository are out of sync, the settings page allows to rebuild the index repository.
...
[registries.shipyard]
index = "sparse+https://shipyard.example.com/api/packages/{owner}/cargo/" # Sparse index
# index = "https://shipyard.example.com/{owner}/_cargo-index.git" # Git
