...
Shipyard supports these methods of API authentication:
...
Shipyard parses queries and headers to find the token in
[modules/auth/auth.go](https://github.com/go-shipyard/shipyard/blob/6efdcaed86565c91a3dc77631372a9cc45a58e89/modules/auth/auth.go#L47).
...
```sh
$ curl -H "Content-Type: application/json" -d '{"name":"test"}' -u username:password https://shipyard.your.host/api/v1/users/<username>/tokens
...
```sh
$ curl --url https://yourusername:password@shipyard.your.host/api/v1/users/<username>/tokens
...
An example of the header is `X-Shipyard-OTP: 123456` where `123456`
is where you'd place the code from your authenticator.
Here is how the request would look like in curl:

```sh
$ curl -H "X-Shipyard-OTP: 123456" --url https://yourusername:yourpassword@shipyard.your.host/api/v1/users/yourusername/tokens
```

You can also create an API key token via your Shipyard installation's web
interface: `Settings | Applications | Generate New Token`.

## OAuth2 Provider

Access tokens obtained from Shipyard's [OAuth2 provider](development/oauth2-provider.md) are accepted by these methods:
...
For historical reasons, Shipyard needs the word `token` included before
...
`https://shipyard.your.host/api/swagger`
or on the
[Shipyard demo instance](https://try.shipyard.io/api/swagger)

The OpenAPI document is at:
`https://shipyard.your.host/swagger.v1.json`
...
- [Official go-sdk](https://shipyard.khulnasoft.com/shipyard/go-sdk)
- [more](https://shipyard.khulnasoft.com/shipyard/awesome-shipyard#user-content-sdk)
