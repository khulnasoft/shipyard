...
title: "Hacking on Shipyard"
slug: "hacking-on-shipyard"
sidebar_position: 10
toc: false
draft: false
aliases:
  - /en-us/hacking-on-shipyard
menu:
  sidebar:
    parent: "development"
    name: "Hacking on Shipyard"
    sidebar_position: 10
    identifier: "hacking-on-shipyard"
---
...
[![Open in Gitpod](/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/go-shipyard/shipyard)
...
`make watch-backend`, Shipyard will automatically download and build these as
necessary. To be able to use these you must have the `"$GOPATH"/bin` directory
on the executable path. If you don't add the go bin directory to the
executable path you will have to manage this yourself.

**Note 2**: Go version @minGoVersion@ or higher is required.
Shipyard uses `gofmt` to format source code. However, the results of
...
Shipyard makes heavy use of Make to automate tasks and improve development. This
...
- [MinGW-w64](https://www.mingw-w64.org) / [MSYS2](https://www.msys2.org/).
  ...
  - To compile Shipyard with CGO_ENABLED (eg: SQLite3), you might need to use [tdm-gcc](https://jmeubank.github.io/tdm-gcc/) instead of MSYS2 gcc, because MSYS2 gcc headers lack some Windows-only CRT functions like `_beginthread`.
...
## Downloading and cloning the Shipyard source code
...
Download the main Shipyard source code as above. Then, fork the
[Shipyard repository](https://github.com/go-shipyard/shipyard) on GitHub,
and either switch the git remote origin for your fork or add your fork as another remote:

```bash
# Rename original Shipyard origin to upstream
git remote rename origin upstream
git remote add origin "git@github.com:$GITHUB_USERNAME/shipyard.git"
git fetch --all --prune
```
...
git remote add "$FORK_NAME" "git@github.com:$GITHUB_USERNAME/shipyard.git"
git fetch --all --prune
```

To be able to create pull requests, the forked repository should be added as a remote
to the Shipyard sources. Otherwise, changes can't be pushed.

## Building Shipyard (Basic)
...
See `make help` for all available `make` targets. Also see [`.drone.yml`](https://github.com/go-shipyard/shipyard/blob/main/.drone.yml) to see how our continuous integration works.
...
The PNG and SVG versions of the Shipyard logo are built from a single SVG source file `assets/logo.svg` using the `TAGS="shipyard" make generate-images` target. To run it, Node.js and npm must be available.

The same process can also be used to generate custom logo PNGs from a SVG source file by updating `assets/logo.svg` and running `make generate-images`. Omitting the `shipyard` tag will update only the user-designated logo files.
...
or compare with a previous PR adding a new API endpoint, e.g. [PR #5483](https://github.com/go-shipyard/shipyard/pull/5843/files#diff-2e0a7b644cf31e1c8ef7d76b444fe3aaR20)
...
When changing the Shipyard logo SVG, you will need to run and commit the results
of:
...
This will create the necessary Shipyard favicon and others.
...
There are two types of test run by Shipyard: Unit tests and Integration Tests.

### Unit Tests

Unit tests are covered by `*_test.go` in `go test` system.
You can set the environment variable `SHIPYARD_UNIT_TESTS_LOG_SQL=1` to display all SQL statements when running the tests in verbose mode (i.e. when `GOTESTFLAGS=-v` is set).
...
Unit tests will not and cannot completely test Shipyard alone. Therefore, we
...
Take a look at [`tests/integration/README.md`](https://github.com/go-shipyard/shipyard/blob/main/tests/integration/README.md)
...
Migration from several recent versions of Shipyard will also be tested.
...
```bash
# from the docs directory within Shipyard
...
[`contrib/ide/README.md`](https://github.com/go-shipyard/shipyard/blob/main/contrib/ide/README.md)
for more information.

## GoLand

Clicking the `Run Application` arrow on the function `func main()` in `/main.go`
can quickly start a debuggable Shipyard instance.

The `Output Directory` in `Run/Debug Configuration` MUST be set to the
shipyard project directory (which contains `main.go` and `go.mod`),
otherwise, the started instance's working directory is a GoLand's temporary directory
and prevents Shipyard from loading dynamic resources (eg: templates) in a development environment.
...
is recommended that you allow Shipyard Managers and Owners to modify your PR
branches as we will need to update it to main before merging and/or may be
able to help fix issues directly.

Any PR requires two approvals from the Shipyard maintainers and needs to pass the
continuous integration. Take a look at our
[`CONTRIBUTING.md`](https://github.com/go-shipyard/shipyard/blob/main/CONTRIBUTING.md)
document.

If you need more help pop on to [Discord](https://discord.gg/shipyard) #Develop
and chat there.

That's it! You are ready to hack on Shipyard.
