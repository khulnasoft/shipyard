...
Shipyard 调用解析查询参数以及头部信息来获取 token 的代码可以在 [modules/auth/auth.go](https://github.com/go-shipyard/shipyard/blob/6efdcaed86565c91a3dc77631372a9cc45a58e89/modules/auth/auth.go#L47) 中找到。

您可以通过您的 shipyard web 界面来创建 apiKey token：
`Settings | Applications | Generate New Token`.

### 关于 `Authorization:` header

由于一些历史原因，Shipyard 需要在 header 的 apiKey token 里引入前缀 `token`，类似于如下形式：
...
[#3842](https://github.com/go-shipyard/shipyard/issues/3842#issuecomment-397743346) 有所提及，使用方法如下所示：

### 使用 Basic authentication 认证：

```
$ curl --url https://yourusername:yourpassword@shipyard.your.host/api/v1/users/yourusername/tokens
