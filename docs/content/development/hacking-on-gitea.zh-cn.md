...
[![在 Gitpod 中打开](/open-in-gitpod.svg)](https://gitpod.io/#https://github.com/go-shipyard/shipyard)
...
`make watch-backend`，Shipyard 会自动下载并构建这些必要的组件。为了能够使用这些，你必须
将 `"$GOPATH"/bin` 目录加入到可执行路径上。如果你不把go bin目录添加到可执行路径你必须手动
指定可执行程序路径。

**注意2** ：Go版本 @minGoVersion@ 或更高版本是必须的。Shipyard 使用 `gofmt` 来
格式化源代码。然而，`gofmt` 的结果可能因 `go` 的版本而有差异。因此推荐安装我们持续集成使用
的 Go版本。截至上次更新，Go 版本应该是 @goVersion@。

## 安装 Make

Shipyard 大量使用 `Make` 来自动化任务和改进开发。本指南涵盖了如何安装 Make。
...
- [MinGW-w64](https://www.mingw-w64.org) / [MSYS2](https://www.msys2.org/)。
  ...
  - 要使用 CGO_ENABLED（例如：SQLite3）编译 Shipyard，您可能需要使用 [tdm-gcc](https://jmeubank.github.io/tdm-gcc/) 而不是 MSYS2 gcc，因为 MSYS2 gcc 标头缺少一些 Windows -只有 CRT 函数像 _beginthread 一样。
...
如上所述下载主要的 Shipyard 源代码。然后，派生 [Shipyard 仓库](https://github.com/go-shipyard/shipyard)，
并为您的本地仓库切换 git 远程源，或添加另一个远程源：

```bash
# 将原来的 Shipyard origin 重命名为 upstream
...
为了能够创建合并请求，应将分叉存储库添加为 Shipyard 本地仓库的远程，否则无法推送更改。
...
有关所有可用的 `make` 目标，请参阅 `make help`。另请参阅 [`.drone.yml`](https://github.com/go-shipyard/shipyard/blob/main/.drone.yml) 以了解我们的持续集成是如何工作的。
...
Shipyard Logo的 PNG 和 SVG 版本是使用 `TAGS="shipyard" make generate-images` 目标从单个 SVG 源文件 assets/logo.svg 构建的。要运行它，Node.js 和 npm 必须可用。

通过更新 `assets/logo.svg` 并运行 `make generate-images`，同样的过程也可用于从 SVG 源文件生成自定义 Logo PNG。忽略 shipyard 编译选项将仅更新用户指定的 LOGO 文件。
...
或与添加新 API 端点的先前 PR 进行比较，例如 [PR #5483](https://github.com/go-shipyard/shipyard/pull/5843/files#diff-2e0a7b644cf31e1c8ef7d76b444fe3aaR20)
...
更改 Shipyard Logo SVG 时，您将需要运行并提交结果的：

```bash
make generate-images
```

这将创建必要的 Shipyard 图标和其他图标。
...
Shipyard 运行两种类型的测试：单元测试和集成测试。

### 单元测试

`go test` 系统中的`*_test.go` 涵盖了单元测试。
您可以设置环境变量 `SHIPYARD_UNIT_TESTS_LOG_SQL=1` 以在详细模式下运行测试时显示所有 SQL 语句（即设置`GOTESTFLAGS=-v` 时）。
...
单元测试不会也不能完全单独测试 Shipyard。因此，我们编写了集成测试；但是，这些依赖于数据库。
...
看看 [`tests/integration/README.md`](https://github.com/go-shipyard/shipyard/blob/main/tests/integration/README.md) 有关更多信息以及如何运行单个测试。

### 测试 PR

我们的持续集成将测试代码是否通过了单元测试，并且所有支持的数据库都将在 Docker 环境中通过集成测试。
还将测试从几个最新版本的 Shipyard 迁移。
...
[`contrib/ide/README.md`](https://github.com/go-shipyard/shipyard/blob/main/contrib/ide/README.md) 了解更多信息。

## Goland

单击 `/main.go` 中函数 `func main()` 上的 `Run Application` 箭头
可以快速启动一个可调试的 Shipyard 实例。

`Run/Debug Configuration` 中的 `Output Directory` 必须设置为
shipyard 项目目录（包含 `main.go` 和 `go.mod`），
否则，启动实例的工作目录是 GoLand 的临时目录
并防止 Shipyard 在开发环境中加载动态资源（例如：模板）。
...
对更改感到满意后，将它们推送并打开拉取请求。它建议您允许 Shipyard Managers 和 Owners 修改您的 PR
分支，因为我们需要在合并之前将其更新为 main 和/或可能是能够直接帮助解决问题。

任何 PR 都需要 Shipyard 维护者的两次批准，并且需要通过持续集成。看看我们的
[CONTRIBUTING.md](https://github.com/go-shipyard/shipyard/blob/main/CONTRIBUTING.md)
文档。

如果您需要更多帮助，请访问 [Discord](https://discord.gg/shipyard) #Develop 频道
并在那里聊天。

现在，您已准备好 Hacking Shipyard。
