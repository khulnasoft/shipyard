...
- `--help`，`-h`：显示帮助文本并退出。可选。
- `--version`，`-v`：显示版本信息并退出。可选。 (示例：`Shipyard version 1.1.0+218-g7b907ed built with: bindata, sqlite`)。
- `--custom-path path`，`-C path`：Shipyard 自定义文件夹的路径。可选。 (默认值：`AppWorkPath`/custom 或 `$SHIPYARD_CUSTOM`)。
- `--config path`，`-c path`：Shipyard 配置文件的路径。可选。 (默认值：`custom`/conf/app.ini)。
- `--work-path path`，`-w path`：Shipyard 的 `AppWorkPath`。可选。 (默认值：LOCATION_OF_SHIPYARD_BINARY 或 `$SHIPYARD_WORK_DIR`)
...
- 注意：
  - Shipyard 不应以 root 用户身份运行。要绑定到低于 1024 的端口，您可以在 Linux 上使用 setcap 命令：`sudo setcap 'cap_net_bind_service=+ep' /path/to/shipyard`。每次更新 Shipyard 都需要重新执行此操作。
...
- 命令：
  - `user`：
    ...
    - `create`：
      - 选项：
        - `--name value`：用户名。必填。自 Shipyard 1.9.0 版本起，请改用 `--username` 标志。
        - `--username value`：用户名。必填。Shipyard 1.9.0 新增。
...
将所有文件和数据库导出到一个zip文件中。输出文件将保存在当前目录下，类似于`shipyard-dump-1482906742.zip`。

- 选项：
  - `--file name`，`-f name`：指定要创建的导出文件的名称。可选。（默认值：shipyard-dump-[timestamp].zip）。
...
```ini
...
# -e 的值和 AuthorizedKeysCommandUser 应与运行 Shipyard 的用户名匹配
...
注意: opensshd 要求 Shipyard 程序由 root 拥有，并且不可由组或其他人写入。程序必须使用绝对路径指定。
注意: Shipyard 必须在运行此命令时处于运行状态才能成功。
...
对 Shipyard 实例进行诊断，可以修复一些可修复的问题。
默认只运行部分检查，额外的检查可以参考：

- `shipyard doctor check --list` - 列出所有可用的检查
- `shipyard doctor check --all` - 运行所有可用的检查
- `shipyard doctor check --default` - 运行默认的检查
- `shipyard doctor check --run [check(s),]...` - 运行指定的名字的检查
...
您可以通过以下方式让 Shipyard 重新创建这些表，并将旧数据复制到新表中，并适当设置默认值：

```
shipyard doctor recreate-table user
```

您可以使用以下方式让 Shipyard 重新创建多个表：

```
shipyard doctor recreate-table table1 table2 ...
```

如果您希望 Shipyard 重新创建所有表，请直接调用：
...
- 命令:
    ...
    - 命令:
      - `pause`: 暂停日志记录
        - 注意:
          ...
          - Shipyard将在一定程度上缓冲日志，并在超过该点后丢弃日志。
      - `resume`: 恢复日志记录
      - `release-and-reopen`: 使Shipyard释放和重新打开用于日志记录的文件和连接（相当于向Shipyard发送SIGUSR1信号）。
  ...
  - `processes`: 显示 Shipyard 进程和 Goroutine 信息
    - 选项:
...
`dump-repo` 从 Git/GitHub/Shipyard/GitLab 中转储存储库数据：

- 选项：
  - `--git_service service`：Git 服务，可以是 `git`、`github`、`shipyard`、`gitlab`。如果 `clone_addr` 可以被识别，则可以忽略此选项。
  - `--repo_dir dir`，`-r dir`：存储数据的存储库目录路径。
  - `--clone_addr addr`：将被克隆的 URL，目前可以是 git/github/shipyard/gitlab 的 http/https URL。例如：https://github.com/lunny/tango.git
