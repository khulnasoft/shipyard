...
Shipyard 具有邮件功能，用于发送事务性邮件（例如注册确认邮件）。它可以配置为使用 Sendmail（或兼容的 MTA，例如 Postfix 和 msmtp）或直接使用 SMTP 服务器。

## 使用 Sendmail

使用 `sendmail` 命令作为邮件传输代理（mailer）。

注意：对于在官方Shipyard Docker镜像中使用，请使用SMTP版本进行配置（请参考下一节）。
...
重启 Shipyard 以使配置更改生效。

要发送测试邮件以验证设置，请转到 Shipyard > 站点管理 > 配置 > SMTP 邮件配置。
...
[mailer]
ENABLED        = true
HOST           = smtp.gmail.com:465 ; 对于 Shipyard >= 1.18.0，删除此行
