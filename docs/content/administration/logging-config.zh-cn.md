...
在 Shipyard 中，日志设施的配置在 `[log]` 部分及其子部分。

在顶层的 `[log]` 部分，可以放置以下配置项：

- `ROOT_PATH`：（默认值：**%(SHIPYARD_WORK_DIR)/log**）：日志文件的基本路径。
...
将默认日志（>=Warn）记录到 `shipyard.log` 中，将错误日志记录到 `file-error.log` 中：
...
; 默认情况下，"file" 模式会将日志记录到 %(log.ROOT_PATH)/shipyard.log，因此我们不需要设置它
; [log.file]
...
在此模式下，日志记录器将将日志消息转发到 Shipyard 进程附加的 stdout 和 stderr 流。
...
- `FILE_NAME`：要将日志事件写入的文件，相对于 `ROOT_PATH`，默认为 `%(ROOT_PATH)/shipyard.log`。异常情况：访问日志默认为 `%(ROOT_PATH)/access.log`。
...
在 v1.17.3 版本时的确切行为可以在[这里](https://github.com/go-shipyard/shipyard/blob/v1.17.3/modules/setting/log.go#L185)中查看。
...
当 Shipyard 的路由处理程序工作时，Router 日志记录器记录以下消息类型：
...
"Access" 日志记录器是自 Shipyard 1.9 版本以来的新日志记录器。它提供了符合 NCSA Common Log 标准的日志格式。虽然它具有高度可配置性，但在更改其模板时应谨慎。此日志记录器的主要好处是，Shipyard 现在可以使用标准日志格式记录访问日志，因此可以使用标准工具进行分析。
...
可以通过向运行中的进程发送 `SIGUSR1` 信号或运行 `shipyard manager logging release-and-reopen` 命令来强制 Shipyard 释放并重新打开其日志文件和连接。

或者，您可能希望暂停和恢复日志记录 - 可以通过使用 `shipyard manager logging pause` 和 `shipyard manager logging resume` 命令来实现。请注意，当日志记录暂停时，低于 INFO 级别的日志事件将不会存储，并且只会存储有限数量的事件。在暂停时，日志记录可能会阻塞，尽管是暂时性的，但会大大减慢 Shipyard 的运行速度，因此建议仅暂停很短的时间。

### 在 Shipyard 运行时添加和删除日志记录

可以使用 `shipyard manager logging add` 和 `remove` 子命令在 Shipyard 运行时添加和删除日志记录。
此功能只能调整正在运行的日志系统，不能用于启动未初始化的访问或路由日志记录器。如果您希望启动这些系统，建议调整 app.ini 并（优雅地）重新启动 Shipyard 服务。

这些命令的主要目的是在运行中的系统上轻松添加临时日志记录器，以便调查问题，因为重新启动可能会导致问题消失。

## 使用 `logrotate` 而不是内置的日志轮转

Shipyard 包含内置的日志轮转功能，对于大多数部署来说应该已经足够了。但是，如果您想使用 `logrotate` 工具：

- 在 `app.ini` 中将 `LOG_ROTATE` 设置为 `false`，禁用内置的日志轮转。
- 安装 `logrotate`。
- 根据部署要求配置 `logrotate`，有关配置语法细节，请参阅 `man 8 logrotate`。
  在 `postrotate/endscript` 块中通过 `kill -USR1` 或 `kill -10` 向 `shipyard` 进程本身发送 `USR1` 信号，
  或者运行 `shipyard manager logging release-and-reopen`（使用适当的环境设置）。
  确保配置适用于由 Shipyard 日志记录器生成的所有文件，如上述部分所述。
...
- 如果您正在使用 Docker 并从容器外部运行，您可以使用
  ...
  或 `docker exec $CONTAINER_NAME sh -c '/bin/s6-svc -1 /etc/s6/shipyard/'`，或直接向 Shipyard 进程本身发送 `USR1` 信号。
