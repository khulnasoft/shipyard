...
Shipyard supports custom file renderings (i.e., Jupyter notebooks, asciidoc, etc.) through external binaries,
it is just a matter of:

- installing external binaries
- add some configuration to your `app.ini` file
- restart your Shipyard instance

This supports rendering of whole files. If you want to render code blocks in markdown you would need to do something with javascript. See some examples on the [Customizing Shipyard](administration/customizing-shipyard.md) page.
...
```docker
FROM shipyard/shipyard:@version@
[...]

COPY custom/app.ini /data/shipyard/conf/app.ini
[...]
...
If your external markup relies on additional classes and attributes on the generated HTML elements, you might need to enable custom sanitizer policies. Shipyard uses the [`bluemonday`](https://godoc.org/github.com/microcosm-cc/bluemonday) package as our HTML sanitizer. The example below could be used to support server-side [KaTeX](https://katex.org/) rendering output from [`pandoc`](https://pandoc.org/).
...
Once your configuration changes have been made, restart Shipyard to have changes take effect.

**Note**: Prior to Shipyard 1.12 there was a single `markup.sanitiser` section with keys that were redefined for multiple rules, however,
