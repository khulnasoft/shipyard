...
如果是从发行版本完成的安装，
配置文件的路径为`/etc/shipyard/conf/app.ini`。

所有默认值可以通过 [app.example.ini](https://github.com/go-shipyard/shipyard/blob/main/custom/conf/app.example.ini) 查看到。
如果你发现 `%(X)s` 这样的内容，请查看
...
**注意:** 修改完配置文件后，需要重启 Shipyard 服务才能生效。

## 默认配置 (非`app.ini`配置文件)

这些值取决于环境，但构成了许多值的基础。当运行 `shipyard help`或启动时，它们将
作为默认配置的一部分进行报告。它们在那里发出的顺序略有不同，但我们将按照设置的顺序在这里列出。

- _`AppPath`_: Shipyard二进制可执行文件的绝对路径
- _`AppWorkPath`_: Shipyard可执行文件的工作目录。 该配置可以通过以下几种方式设置，优先级依次递减:
  - `app.ini`中的`WORK_PATH`配置项
  - 启动Shipyard时的`--work-path`命令行参数
  - `$SHIPYARD_WORK_DIR`环境变量
  - 在编译时设置的内置值（参见从源代码编译）
  - 默认为 _`AppPath`_ 的目录
...
- `APP_NAME`: **Shipyard: Git with a cup of tea** 应用名称，在网页的标题中显示。
- `RUN_USER`: **_current OS username_/`$USER`/`$USERNAME` e.g. git**: 运行Shipyard的用户，
  应当是一个专用的系统账户(非用户使用，推荐创建一个专用的`git`用户). 如果在你自己的个人电脑使用改成你自己的用户名。
  该配置如果设置不正确，Shipyard可能崩溃。
...
- `ROOT`: **%(APP_DATA_PATH)s/shipyard-repositories**: 存放git工程的根目录，建议填绝对路径。
...
- `ENABLE_PUSH_CREATE_USER`:  **false**: 允许用户将本地存储库推送到Shipyard，并为用户自动创建它们。
- `ENABLE_PUSH_CREATE_ORG`:  **false**:  允许用户将本地存储库推送到Shipyard，并为组织自动创建它们。
...
- `TEMP_PATH`: **data/tmp/uploads**: 文件上传的临时保存路径(在Shipyard重启的时候该目录会被清空)。
...
- `DEFAULT_TRUST_MODEL`: **collaborator**: \[collaborator, committer, collaboratorcommitter\]: 用于验证提交的默认信任模型。
  ...
  - `committer`: 信任与提交者匹配的签名（这与GitHub匹配，并会强制Shipyard签名的提交具有Shipyard作为提交者）。
...
- `LOCAL_COPY_PATH`: **tmp/local-repo**:临时本地仓库副本的路径。默认为 tmp/local-repo（内容在 Shipyard 重新启动时被删除）
...
- `DEFAULT_THEME`: **shipyard-auto**: \[shipyard-auto, shipyard-light, shipyard-dark\]: 在Shipyard安装时候设置的默认主题。
- `SHOW_USER_EMAIL`: **true**: 用户的电子邮件是否应该显示在`Explore Users`页面中。
- `THEMES`:  **shipyard-auto,shipyard-light,shipyard-dark**: 所有可用的主题。允许用户选择个性化的主题，
...
- `CUSTOM_EMOJIS`: **shipyard, codeberg, gitlab, git, github, gogs**: 不在utf8标准中定义的额外表情符号。
    默认情况下，我们支持 Shipyard 表情符号（:shipyard:）。要添加更多表情符号，请将它们复制到 public/assets/img/emoji/ 目录下，
    并将其添加到此配置中。
...
- `AUTHOR`: **Shipyard - Git with a cup of tea**: 主页的作者元标签。
- `DESCRIPTION`: **Shipyard (Git with a cup of tea) is a painless self-hosted Git service written in Go**: 主页的描述元标签。
- `KEYWORDS`: **go,git,self-hosted,shipyard**: 首页关键词元标签。
...
- `STATIC_URL_PREFIX`: **_empty_**:
   ...
   静态文件位于Shipyard源代码仓库的`public/`目录中。
   您可以将`STATIC_URL_PREFIX`请求代理到 Shipyard 服务器以提供静态资源，或者将手动构建的 Shipyard 资源从 `$SHIPYARD_BUILD/public`复制到静态位置，例如`/var/www/assets`。确保`$STATIC_URL_PREFIX/assets/css/index.css`指向`/var/www/assets/css/index.css`。

- `HTTP_ADDR`: **0.0.0.0**：HTTP 监听地址。
  - 如果 `PROTOCOL` 设置为 `fcgi`，Shipyard 将在由
...
- `HTTP_PORT`: **3000**：HTTP 监听端口。
  - 如果 `PROTOCOL` 设置为 `fcgi`，Shipyard 将在由 `HTTP_ADDR` 和 `HTTP_PORT`
  配置设置定义的 TCP 套接字上监听 FastCGI 请求。
- `UNIX_SOCKET_PERMISSION`: **666**：Unix 套接字的权限。
- `LOCAL_ROOT_URL`: **%(PROTOCOL)s://%(HTTP_ADDR)s:%(HTTP_PORT)s/**：
  用于访问网络服务的 Shipyard 工作器（例如 SSH 更新）的本地（DMZ）URL。
  在大多数情况下，您不需要更改默认值。
...
- `SSH_CREATE_AUTHORIZED_KEYS_FILE`: **true**：当 Shipyard 不使用内置 SSH 服务器时，默认情况下 Shipyard 会创建一个 authorized_keys 文件。如果您打算使用 AuthorizedKeysCommand 功能，您应该关闭此选项。
...
- `SSH_TRUSTED_USER_CA_KEYS_FILENAME`: **`RUN_USER`/.ssh/shipyard-trusted-user-ca-keys.pem**：Shipyard 将管理的 `TrustedUserCaKeys` 文件的绝对路径。如果您正在运行自己的 SSH 服务器，并且想要使用 Shipyard 管理的文件，您还需要修改您的 `sshd_config` 来指向此文件。官方的 Docker 映像将自动工作，无需进一步配置。
...
- `SSH_CREATE_AUTHORIZED_PRINCIPALS_FILE`: **false/true**：当 Shipyard 不使用内置 SSH 服务器且 `SSH_AUTHORIZED_PRINCIPALS_ALLOW` 不为 `off` 时，默认情况下 Shipyard 会创建一个 authorized_principals 文件。
...
- `SSH_SERVER_HOST_KEYS`: **ssh/shipyard.rsa, ssh/gogs.rsa**：对于内置 SSH 服务器，选择要提供为主机密钥的密钥对。私钥应在 `SSH_SERVER_HOST_KEY` 中，公钥在 `SSH_SERVER_HOST_KEY.pub` 中。相对路径会相对于 `APP_DATA_PATH` 转为绝对路径。如果不存在密钥，将为您创建一个 4096 位的 RSA 密钥。
- `SSH_KEY_TEST_PATH`: **/tmp**：在使用 `ssh-keygen` 测试公共 SSH 密钥时要在其中创建临时文件的目录，默认为系统临时目录。
- `SSH_KEYGEN_PATH`: **_empty_**：使用 `ssh-keygen` 解析公共 SSH 密钥。该值将传递给 shell。默认情况下，Shipyard 会自行进行解析。
...
- `APP_DATA_PATH`: **data**（在 Docker 上为 **/data/shipyard**）：应用程序数据的默认路径。相对路径会相对于 _`AppWorkPath`_ 转为绝对路径。
...
- `PPROF_DATA_PATH`: **_`AppWorkPath`_/data/tmp/pprof**：`PPROF_DATA_PATH`，当您将 Shipyard 作为服务启动时，请使用绝对路径。
...
- `LFS_JWT_SECRET_URI`: **_empty_**：代替在配置中定义 LFS_JWT_SECRET，可以使用此配置选项为 Shipyard 提供包含密钥的文件的路径（示例值：`file:/etc/shipyard/lfs_jwt_secret`）。
...
- `REDIRECT_OTHER_PORT`: **false**：如果为 true 并且 `PROTOCOL` 为 https，则允许将 http 请求重定向到 Shipyard 监听的 https 端口的 `PORT_TO_REDIRECT`。
...
- `ENABLE_ACME`: **false**：通过 ACME 能力的证书颁发机构（CA）服务器（默认为 Let's Encrypt）启用自动证书管理的标志。如果启用，将忽略 `CERT_FILE` 和 `KEY_FILE`，并且 CA 必须将 `DOMAIN` 解析为此 Shipyard 服务器。确保设置了 DNS 记录，并且端口 `80` 或端口 `443` 可以被 CA 服务器访问（默认情况下是公共互联网），并重定向到相应的端口 `PORT_TO_REDIRECT` 或 `HTTP_PORT`。
...
- `SSL_MODE`: **disable**：MySQL 或 PostgreSQL 数据库是否启用 SSL 模式，仅适用于 MySQL 和 PostgreSQL。
  - MySQL 的有效值：
    - `true`：启用 TLS，并针对数据库服务器证书根证书进行验证。选择此选项时，请确保用于验证数据库服务器证书的根证书（例如 CA 证书）位于数据库服务器和 Shipyard 服务器的系统证书存储中。有关如何将 CA 证书添加到证书存储的说明，请参阅系统文档。
    - `false`：禁用 TLS。
...
- `PATH`：**data/shipyard.db**：仅适用于 SQLite3 的数据库文件路径。
...
- `ISSUE_INDEXER_NAME`：**shipyard_issues**：工单索引器名称，在 ISSUE_INDEXER_TYPE 为 elasticsearch 或 meilisearch 时可用。
...
- `REPO_INDEXER_NAME`：**shipyard_codes**：代码索引器名称，在 `REPO_INDEXER_TYPE` 为 elasticsearch 时可用。
...
- `SECRET_KEY_URI`: **_empty_**：与定义 `SECRET_KEY` 不同，此选项可用于使用存储在文件中的密钥（示例值：`file:/etc/shipyard/secret_key`）。它不应该像 `SECRET_KEY` 一样容易丢失。
- `LOGIN_REMEMBER_DAYS`: **7**：Cookie 保存时间，单位为天。
- `COOKIE_REMEMBER_NAME`: **shipyard\_incredible**：保存自动登录信息的 Cookie 名称。
...
- `DISABLE_GIT_HOOKS`: **true**：设置为 `false` 以允许具有 Git 钩子权限的用户创建自定义 Git 钩子。
   警告：自定义 Git 钩子可用于在主机操作系统上执行任意代码。这允许用户访问和修改此配置文件和 Shipyard 数据库，并中断 Shipyard 服务。
   通过修改 Shipyard 数据库，用户可以获得 Shipyard 管理员权限。
   它还使他们可以访问正在运行 Shipyard 实例的操作系统上用户可用的其他资源，并以 Shipyard 操作系统用户的名义执行任意操作。
   这可能对您的网站或操作系统造成危害。
   在必要之前，请在更改现有 git 存储库中的钩子之前进行调整。
- `DISABLE_WEBHOOKS`: **false**：设置为 `true` 以禁用 Webhooks 功能。
- `ONLY_ALLOW_PUSH_IF_SHIPYARD_ENVIRONMENT_SET`: **true**：设置为 `false` 以允许本地用户在未设置 Shipyard 环境的情况下推送到 Shipyard 存储库。不建议这样做，如果您希望本地用户推送到 Shipyard 存储库，应该适当地设置环境。
- `IMPORT_LOCAL_PATHS`: **false**：设置为 `false` 以防止所有用户（包括管理员）从服务器上导入本地路径。
- `INTERNAL_TOKEN`: **\<每次安装时随机生成，如果未设置 URI\>**：用于验证 Shipyard 二进制文件内部通信的密钥。
- `INTERNAL_TOKEN_URI`: **_empty_**：与在配置中定义 `INTERNAL_TOKEN` 不同，此配置选项可用于将包含内部令牌的文件的路径提供给 Shipyard（示例值：`file:/etc/shipyard/internal_token`）。
...
- `ALLOW_ONLY_INTERNAL_REGISTRATION`: **false**：设置为True以强制仅通过Shipyard进行注册。
...
⚠️ 此部分适用于 Shipyard 1.18 及更高版本。如果您使用的是 Shipyard 1.17 或更早版本，请阅读以下链接获取更多信息：
[Shipyard 1.17 app.ini 示例](https://github.com/go-shipyard/shipyard/blob/release/v1.17/custom/conf/app.example.ini)
和
[Shipyard 1.17 配置文档](https://github.com/go-shipyard/shipyard/blob/release/v1.17/docs/content/doc/advanced/config-cheat-sheet.en-us.md)

- `ENABLED`: **false**：是否启用邮件服务。
- `PROTOCOL`: **_empty_**：邮件服务协议，可选择 "smtp"、"smtps"、"smtp+starttls"、"smtp+unix"、"sendmail"、"dummy"。在 Shipyard 1.18 之前，邮件服务协议由 `MAILER_TYPE` 和 `IS_TLS_ENABLED` 两个配置共同决定。
...
- `COOKIE_NAME`: **i\_like\_shipyard**：用于会话 ID 的 cookie 名称。
...
- `REPOSITORY_AVATAR_FALLBACK`: **none**：Shipyard 处理缺少仓库头像的方式
  - none = 不显示任何头像
  - random = 生成随机头像
...
- `MINIO_BUCKET`: **shipyard**: Minio 存储附件的存储桶，仅当 STORAGE_TYPE 为 `minio` 时可用。
...
- `FILE_NAME`: 设置此记录器的文件名。默认为 `shipyard.log`（例外：访问日志默认为 `access.log`）。如果是相对路径，将相对于 `ROOT_PATH`。
...
#### Cron - 使用 Shipyard SSH 密钥更新 '.ssh/authorized_keys' 文件 (`cron.resync_all_sshkeys`)
...
- `PATH`: **""**: Git可执行文件的路径。如果为空，Shipyard将在PATH环境中搜索。
- `HOME_PATH`: **%(APP_DATA_PATH)s/home**: Git的HOME目录。
   此目录将用于包含Shipyard的git调用将使用的`.gitconfig`和可能的`.gnupg`目录。如果您可以确认Shipyard是在此环境中唯一运行的应用程序，您可以将其设置为Shipyard用户的正常主目录。
...
- `ENABLED_ISSUE_BY_LABEL`: **false**: 启用按标签统计问题，格式为`shipyard_issues_by_label{label="bug"} 2`。
- `ENABLED_ISSUE_BY_REPOSITORY`: **false**: 启用按存储库统计问题，格式为`shipyard_issues_by_repository{repository="org/repo"} 5`。
...
- `JWT_SECRET_URI`：**_empty_**：可以使用此配置选项，而不是在配置中定义`JWT_SECRET`，以向Shipyard提供包含密钥的文件的路径（示例值：`file:/etc/shipyard/oauth2_jwt_secret`）。
...
shipyard支持外部渲染工具，你可以配置你熟悉的文档渲染工具. 比如一下将新增一个名字为 asciidoc 的渲染工具。
...
- `SHIPYARD_PREFIX_SRC`，其中包含`src`路径树中的当前URL前缀。用作链接的前缀。
- `SHIPYARD_PREFIX_RAW`，其中包含`raw`路径树中的当前URL前缀。用作图像路径的前缀。

如果`RENDER_CONTENT_MODE`为`sanitized`，Shipyard支持自定义用于呈现的HTML的清理策略。下面的示例将支持来自pandoc的KaTeX输出。
...
- Shipyard 将使用 `.gitattributes` 文件中的 `linguist-language` 或 `gitlab-language` 属性来对文件进行高亮显示，如果可用。
...
- `MINIO_BUCKET`：**shipyard**：用于存储 lfs 的 Minio 桶，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
...
- `MINIO_ACCESS_KEY_ID`：Minio 的 accessKeyID，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
- `MINIO_SECRET_ACCESS_KEY`：Minio 的 secretAccessKey，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
- `MINIO_BUCKET`：**shipyard**：用于存储数据的 Minio 桶，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
- `MINIO_LOCATION`：**us-east-1**：创建桶的 Minio 位置，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
- `MINIO_USE_SSL`：**false**：Minio 启用 ssl，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
- `MINIO_INSECURE_SKIP_VERIFY`：**false**：Minio 跳过 SSL 验证，仅在 `STORAGE_TYPE` 为 `minio` 时可用。
...
- `MINIO_BUCKET`: **shipyard**：用于存储归档的Minio存储桶，仅在`STORAGE_TYPE`为`minio`时可用。
...
- `DEFAULT_ACTIONS_URL`: **github**：获取操作插件的默认平台，`github`表示`https://github.com`，`self`表示当前的 Shipyard 实例。
...
`DEFAULT_ACTIONS_URL` 指示 Shipyard 操作运行程序应该在哪里找到带有相对路径的操作。
例如，`uses: actions/checkout@v4` 表示 `https://github.com/actions/checkout@v4`，因为 `DEFAULT_ACTIONS_URL` 的值为 `github`。
它可以更改为 `self`，以使其成为 `root_url_of_your_shipyard/actions/checkout@v4`。

请注意，对于大多数情况，不建议使用 `self`，因为它可能使名称在全局范围内产生歧义。
此外，它要求您将所有所需的操作镜像到您的 Shipyard 实例，这可能不值得。
因此，请仅在您了解自己在做什么的情况下使用 `self`。

在早期版本（`<= 1.19`）中，`DEFAULT_ACTIONS_URL` 可以设置为任何自定义 URL，例如 `https://shipyard.khulnasoft.com` 或 `http://your-git-server,https://shipyard.khulnasoft.com`，默认值为 `https://shipyard.khulnasoft.com`。
然而，后来的更新删除了这些选项，现在唯一的选项是 `github` 和 `self`，默认值为 `github`。
但是，如果您想要使用其他 Git 服务器中的操作，您可以在 `uses` 字段中使用完整的 URL，Shipyard 支持此功能（GitHub 不支持）。
例如 `uses: https://shipyard.khulnasoft.com/actions/checkout@v4` 或 `uses: http://your-git-server/actions/checkout@v4`。

## 其他 (`other`)

- `SHOW_FOOTER_VERSION`: **true**: 在页面底部显示Shipyard的版本。
