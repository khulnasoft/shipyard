...
To craft the e-mail subject and contents for certain operations, Shipyard can be customized by using templates. The templates
for these functions are located under the [`custom` directory](administration/customizing-shipyard.md).
Shipyard has an internal template that serves as default in case there's no custom alternative.

Custom templates are loaded when Shipyard starts. Changes made to them are not recognized until Shipyard is restarted again.
...
The only mandatory template is action type `issue`, action name `default`, which is already embedded in Shipyard
...
did not include a subject part), Shipyard's **internal default** will be used.
...
Shipyard's default subject can also be found in the template _metadata_ as `.FallbackSubject` from any of
...
Please check [Shipyard's logs](administration/logging-config.md) for error messages in case of trouble.
...
<body>
    ...
    <hr>
    <p>
        <a href="{{.Link}}">View it on Shipyard</a>.
    </p>
</body>
</html>
```
...
| ---------------- | ----------- | --------- | --------------------------------------------------------------------------- |
| `AppUrl`         | -           | Any       | Shipyard's URL                                                                 |
| `AppName`        | -           | Any       | Set from `app.ini`, usually "Shipyard"                                         |
| `AppDomain`      | -           | Any       | Shipyard's host name                                                           |
