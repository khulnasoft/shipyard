...
`shipyard [global options] command [command or global options] [arguments...]`
...
- `--version`, `-v`: Show version and exit. Optional. (example: `Shipyard version 1.1.0+218-g7b907ed built with: bindata, sqlite`).
- `--work-path path`, `-w path`: Shipyard's work path. Optional. (default: the binary's path or `$SHIPYARD_WORK_DIR`)
- `--custom-path path`, `-C path`: Shipyard's custom folder path. Optional. (default: `WorkPath`/custom or `$SHIPYARD_CUSTOM`).
- `--config path`, `-c path`: Shipyard configuration file path. Optional. (default: `CustomPath`/conf/app.ini).
...
- Examples:
  - `shipyard web`
  - `shipyard web --port 80`
  - `shipyard web --config /etc/shipyard.ini --pid /some/custom/shipyard.pid`
- Notes:
  - Shipyard should not be run as root. To bind to a port below 1024, you can use setcap on
    Linux: `sudo setcap 'cap_net_bind_service=+ep' /path/to/shipyard`. This will need to be
    redone every time you update Shipyard.
...
- Commands:
  - `user`:
    ...
    - `delete`:
      ...
      - Examples:
        - `shipyard admin user delete --id 1`
    - `create`:
      - Options:
        - `--name value`: Username. Required. As of Shipyard 1.9.0, use the `--username` flag instead.
        - `--username value`: Username. Required. New in Shipyard 1.9.0.
      ...
      - Examples:
        - `shipyard admin user create --username myname --password asecurepassword --email me@example.com`
    - `change-password`:
      - Options:
        - `--username value`, `-u value`: Username. Required.
        - `--password value`, `-p value`: New password. Required.
      - Examples:
        - `shipyard admin user change-password --username myname --password asecurepassword`
    ...
    - `generate-access-token`:
      ...
      - Examples:
        - `shipyard admin user generate-access-token --username myname --token-name mytoken`
        - `shipyard admin user generate-access-token --help`
  - `regenerate`
    - Options:
      - `hooks`: Regenerate Git Hooks for all repositories
      - `keys`: Regenerate authorized_keys file
    - Examples:
      - `shipyard admin regenerate hooks`
      - `shipyard admin regenerate keys`
  - `auth`:
    - `list`:
      - Description: lists all external authentication sources that exist
      - Examples:
        - `shipyard admin auth list`
    - `delete`:
      - Options:
        - `--id`: ID of source to be deleted. Required.
      - Examples:
        - `shipyard admin auth delete --id 1`
    - `add-oauth`:
      - Options:
      ...
      - Examples:
        - `shipyard admin auth add-oauth --name external-github --provider github --key OBTAIN_FROM_SOURCE --secret OBTAIN_FROM_SOURCE`
    - `update-oauth`:
      - Options:
        ...
        - `--restricted-group`: Group Claim value for restricted users. (Optional)
      - Examples:
        - `shipyard admin auth update-oauth --id 1 --name external-github-updated`
    - `add-smtp`:
      - Options:
      ...
      - Examples:
        - `shipyard admin auth add-smtp --name ldap --host smtp.mydomain.org --port 587 --skip-verify --active`
    - `update-smtp`:
      - Options:
        - `--id`: ID of source to be updated. Required.
        - other options are shared with `add-smtp`
      - Examples:
        - `shipyard admin auth update-smtp --id 1 --host smtp.mydomain.org --port 587 --skip-verify=false`
        - `shipyard admin auth update-smtp --id 1 --active=false`
    - `add-ldap`: Add new LDAP (via Bind DN) authentication source
      ...
      - Examples:
        - `shipyard admin auth add-ldap --name ldap --security-protocol unencrypted --host mydomain.org --port 389 --user-search-base "ou=Users,dc=mydomain,dc=org" --user-filter "(&(objectClass=posixAccount)(|(uid=%[1]s)(mail=%[1]s)))" --email-attribute mail`
    - `update-ldap`: Update existing LDAP (via Bind DN) authentication source
      - Options:
        ...
        - `--page-size value`: Search page size.
      - Examples:
        - `shipyard admin auth update-ldap --id 1 --name "my ldap auth source"`
        - `shipyard admin auth update-ldap --id 1 --username-attribute uid --firstname-attribute givenName --surname-attribute sn`
    - `add-ldap-simple`: Add new LDAP (simple auth) authentication source
      ...
      - Examples:
        - `shipyard admin auth add-ldap-simple --name ldap --security-protocol unencrypted --host mydomain.org --port 389 --user-dn "cn=%s,ou=Users,dc=mydomain,dc=org" --user-filter "(&(objectClass=posixAccount)(cn=%s))" --email-attribute mail`
    - `update-ldap-simple`: Update existing LDAP (simple auth) authentication source
      ...
      - Examples:
        - `shipyard admin auth update-ldap-simple --id 1 --name "my ldap auth source"`
        - `shipyard admin auth update-ldap-simple --id 1 --username-attribute uid --firstname-attribute givenName --surname-attribute sn`
...
- Examples:
  - `shipyard cert --host git.example.com,example.com,www.example.com --ca`

### dump

Dumps all files and databases into a zip file. Outputs into a file like `shipyard-dump-1482906742.zip`
in the current directory.

- Options:
  - `--file name`, `-f name`: Name of the dump file with will be created. Optional. (default: shipyard-dump-[timestamp].zip).
...
- Commands:
  - `secret`:
    ...
    - Examples:
      - `shipyard generate secret INTERNAL_TOKEN`
      - `shipyard generate secret JWT_SECRET`
      - `shipyard generate secret SECRET_KEY`
...
# username running Shipyard
AuthorizedKeysCommandUser git
AuthorizedKeysCommand /path/to/shipyard keys -e git -u %u -t %t -k %k
```
...
NB: opensshd requires the Shipyard program to be owned by root and not
writable by group or others. The program must be specified by an absolute
path.
NB: Shipyard must be running for this command to succeed.
...
Diagnose and potentially fix problems with the current Shipyard instance.
Several checks are run by default, but additional ones can be run:

- `shipyard doctor check --list` - will list all the available checks
- `shipyard doctor check --all` - will run all available checks
- `shipyard doctor check --default` - will run the default checks
- `shipyard doctor check --run [check(s),]...` - will run the named checks
...
You can cause Shipyard to recreate these tables and copy the old data into the new table
...
- Commands:
    ...
    - Commands:
      - `pause`: Pause logging
        - Notes:
          ...
          - Shipyard will buffer logs up to a certain point and will drop them after that point.
      - `resume`: Resume logging
      - `release-and-reopen`: Cause Shipyard to release and re-open files and connections used for logging (Equivalent to sending SIGUSR1 to Shipyard.)
  ...
  - `processes`: Display Shipyard processes and goroutine information
    - Options:
...
Dump-repo dumps repository data from Git/GitHub/Shipyard/GitLab:

- Options:
  - `--git_service service` : Git service, it could be `git`, `github`, `shipyard`, `gitlab`, If clone_addr could be recognized, this could be ignored.
  - `--repo_dir dir`, `-r dir`: Repository dir path to store the data
  - `--clone_addr addr`: The URL will be clone, currently could be a git/github/shipyard/gitlab http/https URL. i.e. https://github.com/lunny/tango.git
...
```
shipyard actions generate-runner-token
```

To register a runner for a specific organization, in this case `org`:

```
shipyard actions generate-runner-token -s org
```

To register a runner for a specific repo, in this case `username/test-repo`:

```
shipyard actions generate-runner-token -s username/test-repo
```
