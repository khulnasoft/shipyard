...
Shipyard will verify GPG commit signatures in the provided tree by
checking if the commits are signed by a key within the Shipyard database,
...
There are a number of places where Shipyard will generate commits itself:
...
Depending on configuration and server trust you may want Shipyard to
sign these commits.

## Installing and generating a GPG key for Shipyard

It is up to a server administrator to determine how best to install
a signing key. Shipyard generates all its commits using the server `git`
...
Shipyard's configuration for signing can be found with the
...
- `none` - this prevents Shipyard from signing any commits
- `default` - Shipyard will default to the key configured within `git config`
- `KEYID` - Shipyard will sign commits with the gpg key with the ID
...
Please note: by adjusting Git's `config` file within Shipyard's
repositories, `SIGNING_KEY=default` could be used to provide different
signing keys on a per-repository basis. However, this is clearly not an
ideal UI and therefore subject to change.

**Since 1.17**, Shipyard runs git in its own home directory `[git].HOME_PATH` (default to `%(APP_DATA_PATH)/home`)
and uses its own config `{[git].HOME_PATH}/.gitconfig`.
If you have your own customized git config for Shipyard, you should set these configs in system git config (aka `/etc/gitconfig`)
or the Shipyard internal git config `{[git].HOME_PATH}/.gitconfig`.
Related home files for git command (like `.gnupg`) should also be put in Shipyard's git home directory `[git].HOME_PATH`.
...
This option determines whether Shipyard should sign the initial commit
...
This options determines if Shipyard should sign commits to the Wiki.
...
This option determines if Shipyard should sign commits from the web
...
This option determines if Shipyard should sign merge commits from PRs.
...
The public key used to sign Shipyard's commits can be obtained from the API at:
