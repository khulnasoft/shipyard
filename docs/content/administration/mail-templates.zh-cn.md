...
为了定制特定操作的电子邮件主题和内容，可以使用模板来自定义 Shipyard。这些功能的模板位于 [`custom` 目录](administration/customizing-shipyard.md) 下。
如果没有自定义的替代方案，Shipyard 将使用内部模板作为默认模板。

自定义模板在 Shipyard 启动时加载。对它们的更改在 Shipyard 重新启动之前不会被识别。
...
如果最终结果为空字符串，**或者**没有可用的主题模板（即所选模板不包含主题部分），将使用Shipyard的**内部默认值**。
...
即使存在有效的主题模板，Shipyard的默认主题也可以在模板的元数据中作为 `.FallbackSubject` 找到。
...
如果遇到问题，请检查 [Shipyard的日志](administration/logging-config.md) 以获取错误消息。
...
| 函数名            | 参数        | 可用于       | 用法                                                                              |
| ----------------- | ----------- | ------------ | --------------------------------------------------------------------------------- |
| `AppUrl`          | -           | 任何地方     | Shipyard 的 URL                                                                     |
| `AppName`         | -           | 任何地方     | 从 `app.ini` 中设置，通常为 "Shipyard"                                               |
| `AppDomain`       | -           | 任何地方     | Shipyard 的主机名                                                                   |
