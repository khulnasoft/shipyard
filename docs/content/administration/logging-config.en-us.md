...
The logging configuration of Shipyard mainly consists of 3 types of components:
...
Configuration of logging facilities in Shipyard happen in the `[log]` section and its subsections.

In the top level `[log]` section the following configurations can be placed:

- `ROOT_PATH`: (Default: **%(SHIPYARD_WORK_DIR)/log**): Base path for log files
...
```ini
[log]
ROOT_PATH = %(SHIPYARD_WORK_DIR)/log
MODE = console
LEVEL = Info
...
Default logs (>=Warn) goes into `shipyard.log`, while Error logs goes into `file-error.log`:
...
; by default, the "file" mode will record logs to %(log.ROOT_PATH)/shipyard.log, so we don't need to set it
; [log.file]
...
Shipyard provides the following log output writers:
...
stderr streams attached to the Shipyard process.
...
- `FILE_NAME`: The file to write the log events to, relative to `ROOT_PATH`, Default to `%(ROOT_PATH)/shipyard.log`. Exception: access log will default to `%(ROOT_PATH)/access.log`.
...
The exact behavior at the time of v1.17.3 can be seen [here](https://github.com/go-shipyard/shipyard/blob/v1.17.3/modules/setting/log.go#L185).
...
The Router logger logs the following message types when Shipyard's route handlers work:
...
The Access logger is a new logger since Shipyard 1.9. It provides a NCSA
...
logger is that Shipyard can now log accesses in a standard log format so
...
It is possible force Shipyard to release and reopen it's logging files and connections by sending `SIGUSR1` to the
running process, or running `shipyard manager logging release-and-reopen`.

Alternatively, you may wish to pause and resume logging - this can be accomplished through the use of the
`shipyard manager logging pause` and `shipyard manager logging resume` commands. Please note that whilst logging
is paused log events below INFO level will not be stored and only a limited number of events will be stored.
Logging may block, albeit temporarily, slowing Shipyard considerably whilst paused - therefore it is
recommended that pausing only done for a very short period of time.

## Adding and removing logging whilst Shipyard is running

It is possible to add and remove logging whilst Shipyard is running using the `shipyard manager logging add` and `remove` subcommands.
...
Shipyard includes built-in log rotation, which should be enough for most deployments. However, if you instead want to use the `logrotate` utility:
...
- Configure `logrotate` to match your deployment requirements, see `man 8 logrotate` for configuration syntax details.
  In the `postrotate/endscript` block send Shipyard a `USR1` signal via `kill -USR1` or `kill -10` to the `shipyard` process itself,
  or run `shipyard manager logging release-and-reopen` (with the appropriate environment).
  Ensure that your configurations apply to all files emitted by Shipyard loggers as described in the above sections.
- Always do `logrotate /etc/logrotate.conf --debug` to test your configurations.
- If you are using docker and are running from outside the container you can use
  `docker exec -u $OS_USER $CONTAINER_NAME sh -c 'shipyard manager logging release-and-reopen'`
  or `docker exec $CONTAINER_NAME sh -c '/bin/s6-svc -1 /etc/s6/shipyard/'` or send `USR1` directly to the Shipyard process itself.
