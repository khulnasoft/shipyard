...
通过在您的 [`app.ini`](administration/config-cheat-sheet.md) 中启用此功能，Shipyard 可以通过仓库的文件进行搜索：
...
请记住，索引内容可能会消耗大量系统资源，特别是在首次创建索引或全局更新索引时（例如升级 Shipyard 之后）。
...
Shipyard使用 [`gobwas/glob` 库](https://github.com/gobwas/glob) 中的 glob 模式匹配来选择要包含在索引中的文件。
