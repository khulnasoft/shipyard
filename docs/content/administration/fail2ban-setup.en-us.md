...
Shipyard returns an HTTP 200 for bad logins in the web logs, but if you have logging options on in
`app.ini`, then you should be able to go off of `log/shipyard.log`, which gives you something like this
...
Add our filter in `/etc/fail2ban/filter.d/shipyard.conf`:
...
Add our jail in `/etc/fail2ban/jail.d/shipyard.conf`:
...
chain in **iptables**. Configure it in `/etc/fail2ban/jail.d/shipyard-docker.conf`:
...
If you run Shipyard behind a reverse proxy with Nginx (for example with Docker), you need to add
