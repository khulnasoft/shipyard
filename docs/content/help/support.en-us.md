...
- [Paid Commercial Support](https://about.shipyard.khulnasoft.com/)
- [Discord](https://discord.gg/Shipyard)
- [Discourse Forum](https://discourse.shipyard.io/)
- [Matrix](https://matrix.to/#/#shipyard-space:matrix.org)
...
- Chinese Support
  - [Discourse Chinese Category](https://discourse.shipyard.io/c/5-category/5)
  - QQ Group 328432459

# Bug Report

If you found a bug, please [create an issue on GitHub](https://github.com/go-shipyard/shipyard/issues).

**NOTE:** When asking for support, it may be a good idea to have the following available so that the person helping has all the info they need:

1. Your `app.ini` (with any sensitive data scrubbed as necessary).
2. Any error messages you are seeing.
3. The Shipyard logs, and all other related logs for the situation.
   - It's more useful to collect `trace` / `debug` level logs (see the next section).
   - When using systemd, use `journalctl --lines 1000 --unit shipyard` to collect logs.
   - When using docker, use `docker logs --tail 1000 <shipyard-container>` to collect logs.
4. Reproducible steps so that others could reproduce and understand the problem more quickly and easily.
   - [try.shipyard.io](https://try.shipyard.io) could be used to reproduce the problem.
...
then you will find the `*.log` files in Shipyard's log directory (default: `%(SHIPYARD_WORK_DIR)/log`).
...
Shipyard could use Golang's pprof handler and toolchain to collect stacktrace and other runtime information.
