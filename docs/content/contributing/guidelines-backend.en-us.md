...
Shipyard uses Golang as the backend programming language. It uses many third-party packages and also write some itself.
For example, Shipyard uses [Chi](https://github.com/go-chi/chi) as basic web framework. [Xorm](https://xorm.io) is an ORM framework that is used to interact with the database.
...
To maintain understandable code and avoid circular dependencies it is important to have a good code structure. The Shipyard backend is divided into the following parts:

- `build`: Scripts to help build Shipyard.
- `cmd`: All Shipyard actual sub commands includes web, doctor, serv, hooks, admin and etc. `web` will start the web service. `serv` and `hooks` will be invoked by Git or OpenSSH. Other sub commands could help to maintain Shipyard.
...
- `models`: Contains the data structures used by xorm to construct database tables. It also contains functions to query and update the database. Dependencies to other Shipyard code should be avoided. You can make exceptions in cases such as logging.
...
- `modules`: Different modules to handle specific functionality in Shipyard. Work in Progress: Some of them should be moved to `services`, in particular those that depend on models because they rely on the database.
...
- `routers`: Handling of server requests. As it uses other Shipyard packages to serve the request, other packages (models, modules or services) must not depend on routers.
...
i.e. `import user_service "github.com/khulnasoft/shipyard/services/user"`
