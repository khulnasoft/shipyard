...
Shipyard uses [Fomantic-UI](https://fomantic-ui.com/introduction/getting-started.html) (based on [jQuery](https://api.jquery.com)) and [Vue3](https://vuejs.org/) for its frontend.
...
4. CSS styling for classes provided by frameworks should not be overwritten. Always use new class names with 2-3 feature related keywords to overwrite framework styles. Shipyard's helper CSS classes in `helpers.less` could be helpful.
...
12. Shipyard's tailwind-style CSS classes use `gt-` prefix (`gt-relative`), while Shipyard's own private framework-level CSS classes use `g-` prefix (`g-modal-confirm`).

### Accessibility / ARIA

In history, Shipyard heavily uses Fomantic UI which is not an accessibility-friendly framework.
Shipyard uses some patches to make Fomantic UI more accessible (see the `aria.js` and `aria.md`),
...
* Go template code should use Shipyard's `.gt-hidden` and `showElem()/hideElem()/toggleElem()`, see more details in `.gt-hidden`'s comment.
...
Shipyard is using Vue3 now. We decided not to introduce JSX to keep the HTML and the JavaScript code separated.

### UI Examples

Shipyard uses some self-made UI elements and customizes others to integrate them better into the general UI approach. When running Shipyard in development mode (`RUN_MODE=dev`), a page with some standardized UI examples is available under `http(s)://your-shipyard-url:port/devtest`.
