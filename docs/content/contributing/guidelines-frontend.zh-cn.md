...
Shipyard 在其前端中使用[Fomantic-UI](https://fomantic-ui.com/introduction/getting-started.html)（基于[jQuery](https://api.jquery.com)）和 [Vue3](https://vuejs.org/)。
...
4. 不应覆盖框架提供的 class 的 CSS 样式。始终使用具有2-3个与功能相关的关键词的新 class 名称来覆盖框架样式。Shipyard 中的帮助 CSS 类在 `helpers.less` 中。
...
11. 推荐使用自定义事件名称前缀`ce-`。
12. Shipyard 的 tailwind-style CSS 类使用`gt-`前缀（`gt-relative`），而 Shipyard 自身的私有框架级 CSS 类使用`g-`前缀（`g-modal-confirm`）。

### 可访问性 / ARIA

在历史上，Shipyard大量使用了可访问性不友好的框架 Fomantic UI。
Shipyard使用一些补丁使Fomantic UI更具可访问性（参见`aria.js`和`aria.md`），
...
* Go 模板代码应使用 Shipyard 的 `.gt-hidden` 和 `showElem()/hideElem()/toggleElem()` 来显示/隐藏元素，请参阅`.gt-hidden`的注释以获取更多详细信息。
...
Shipyard 现在正在使用 Vue3。我们决定不引入 JSX，以保持 HTML 代码和 JavaScript 代码分离。
