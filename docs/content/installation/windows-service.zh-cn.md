...
在 C:\shipyard\custom\conf\app.ini 中进行了以下更改：

```
RUN_USER = COMPUTERNAME$
```

将 Shipyard 设置为以本地系统用户运行。
...
别忘了将 `C:\shipyard` 替换成你的 Shipyard 安装目录。

之后在控制面板打开 "Windows Services"，搜索 "shipyard"，右键选择 "Run"。在浏览器打开 `http://localhost:3000` 就可以访问了。（如果你修改了端口，请访问对应的端口，3000是默认端口）。

## 添加启动依赖项

要将启动依赖项添加到 Shipyard Windows 服务（例如 Mysql、Mariadb），作为管理员，然后运行以下命令：

```
sc.exe config shipyard depend= mariadb
```

这将确保在 Windows 计算机重新启动时，将延迟自动启动 Shipyard，直到数据库准备就绪，从而减少启动失败的情况。
