...
在使用 Shipyard 前，您需要准备一个数据库。Shipyard 支持 PostgreSQL（>= 12）、MySQL（>= 8.0）、SQLite 和 MSSQL（>= 2012 SP4）这几种数据库。本页将指导您准备数据库。由于 PostgreSQL 和 MySQL 在生产环境中被广泛使用，因此本文档将仅涵盖这两种数据库。如果您计划使用 SQLite，则可以忽略本章内容。

数据库实例可以与 Shipyard 实例在相同机器上（本地数据库），也可以与 Shipyard 实例在不同机器上（远程数据库）。

注意：以下所有步骤要求您的选择的数据库引擎已安装在您的系统上。对于远程数据库设置，请在数据库实例上安装服务器应用程序，在 Shipyard 服务器上安装客户端程序。客户端程序用于测试 Shipyard 服务器与数据库之间的连接，而 Shipyard 本身使用 Go 提供的数据库驱动程序完成相同的任务。此外，请确保服务器和客户端使用相同的引擎版本，以使某些引擎功能正常工作。出于安全原因，请使用安全密码保护 `root`（MySQL）或 `postgres`（PostgreSQL）数据库超级用户。以下步骤假设您在数据库和 Shipyard 服务器上都使用 Linux。
...
3. 创建一个将被 Shipyard 使用的数据库用户，并使用密码进行身份验证。以下示例中使用了 `'shipyard'` 作为密码。请为您的实例使用一个安全密码。
    ...
    其中 `192.0.2.10` 是您的 Shipyard 实例的 IP 地址。
...
7. 在您的 Shipyard 服务器上，测试与数据库的连接：

    ```
    mysql -u shipyard -h 203.0.113.3 -p shipyarddb
    ```

    其中 `shipyard` 是数据库用户名，`shipyarddb` 是数据库名称，`203.0.113.3` 是数据库实例的 IP 地址。对于本地数据库，省略 -h 选项。
...
4. 创建具有登录权限和密码的数据库用户（在 PostgreSQL 术语中称为角色）。请使用安全的、强密码，而不是下面的 `'shipyard'`：
...
6. 通过将以下身份验证规则添加到 `pg_hba.conf`，允许数据库用户访问上面创建的数据库。
    ...
    根据您自己的数据库名称、用户和 Shipyard 实例的 IP 地址进行替换。
...
7. 在您的 Shipyard 服务器上，测试与数据库的连接。
    ...
    其中 `shipyard` 是数据库用户，`shipyarddb` 是数据库名称，`203.0.113.3` 是您的数据库实例的 IP 地址。

    您应该会被提示输入数据库用户的密码，并连接到数据库。

## 使用 TLS 进行数据库连接

如果 Shipyard 和您的数据库实例之间的通信是通过私有网络进行的，或者如果 Shipyard 和数据库运行在同一台服务器上，那么可以省略本节，因为 Shipyard 和数据库实例之间的安全性不会受到严重威胁。但是，如果数据库实例位于公共网络上，请使用 TLS 对数据库连接进行加密，以防止第三方拦截流量数据。

### 先决条件

- 您需要两个有效的 TLS 证书，一个用于数据库实例（数据库服务器），一个用于 Shipyard 实例（数据库客户端）。两个证书都必须由受信任的 CA 签名。
...
- 在数据库服务器证书中，`Subject Alternative Name` 或 `Common Name` 条目之一必须是数据库实例的完全限定域名（FQDN）（例如 `db.example.com`）。在数据库客户端证书中，上述提到的条目之一必须包含 Shipyard 将用于连接的数据库用户名。
- 您需要将 Shipyard 和数据库服务器的域名映射到它们各自的 IP 地址。可以为它们设置 DNS 记录，也可以在每个系统上的 `/etc/hosts`（Windows 中的 `%WINDIR%\System32\drivers\etc\hosts`）中添加本地映射。这样可以通过域名而不是 IP 地址进行数据库连接。有关详细信息，请参阅您系统的文档。

### PostgreSQL TLS

Shipyard 使用的 PostgreSQL 驱动程序支持双向 TLS。在双向 TLS 中，数据库客户端和服务器通过将各自的证书发送给对方进行验证来相互认证。换句话说，服务器验证客户端证书，客户端验证服务器证书。
...
4. 编辑 `pg_hba.conf` 规则，仅允许 Shipyard 数据库用户通过SSL连接，并要求客户端证书验证。
    ...
    根据需要替换数据库名称、用户和 Shipyard 实例的 IP 地址。

5. 重新启动 PostgreSQL 以应用上述配置。

6. 在运行 Shipyard 实例的服务器上，将以下凭据放置在运行 Shipyard 的用户的主目录下（例如 `git`）：
...
虽然 Shipyard 使用的MySQL驱动程序也支持双向 TLS，但目前 Shipyard 仅支持单向 TLS。有关详细信息，请参见工单＃10828。
...
5. Shipyard 的数据库用户可能已经创建过，但只会对运行 Shipyard 的服务器的 IP 地址进行身份验证。要对其域名进行身份验证，请重新创建用户，并设置其需要通过 TLS 连接到数据库：
    ...
    根据需要替换数据库用户名、密码和 Shipyard 实例域名。

6. 确保用于验证数据库服务器证书的CA证书链位于数据库和 Shipyard 服务器的系统证书存储中。请参考系统文档中有关将 CA 证书添加到证书存储的说明。

7. 在运行Shipyard的服务器上，测试与数据库的连接：
