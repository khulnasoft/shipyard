...
class TestContainerConfiguration:
    def test_defaults(
        self, container_factory: ContainerFactory, stream_container_logs, wait_for_shipyard_ready
    ):
        """
        ...
        r = requests.get(f"http://127.0.0.1:{port}/_shipyard/health")
    ...
    def test_gateway_listen_single_value(
        ...
        r = requests.get(f"http://127.0.0.1:{port1}/_shipyard/health")
        assert r.status_code == 200
    ...
    def test_gateway_listen_multiple_values(
        ...
        r = requests.get(f"http://127.0.0.1:{port2}/_shipyard/health")
        assert r.ok
