...
from shipyard.aws.connect import ServiceLevelClientFactory
from shipyard.config import in_docker
from shipyard.testing.pytest import markers
from shipyard.testing.pytest.container import ContainerFactory, LogStreamFactory
from shipyard.testing.scenario.cdk_lambda_helper import load_python_lambda_to_s3
from shipyard.testing.scenario.provisioning import InfraProvisioner
...
@pytest.fixture(scope="class")
def chosen_shipyard_host() -> str:
    """
    Choose a domain name that is guaranteed never to resolve, except by the Shipyard DNS server
...
def container(
    ...
    chosen_shipyard_host,
):
    ls_container = class_container_factory(
        configurators=[
            ContainerConfigurators.mount_shipyard_volume(),
            ...
            ContainerConfigurators.env_vars(
                {
                    "SHIPYARD_HOST": chosen_shipyard_host,
                }
            ),
        ]
    )
...
class TestShipyardHost:
    """
    Scenario test that runs Shipyard in a docker container with `SHIPYARD_HOST` set to a
    non-default value. This ensures that setting the cosmetic "SHIPYARD_HOST" does not affect
    the internal functionality of Shipyard.
    """

    @pytest.fixture(scope="class", autouse=True)
    def infrastructure(
        self, aws_client_factory, infrastructure_setup, port, chosen_shipyard_host
    ):
        aws_client = aws_client_factory(endpoint_url=f"http://localhost:{port}")

        infra: InfraProvisioner = infrastructure_setup(
            namespace="ShipyardHostBootstrap",
            port=port,
        )
        ...
        def create_lambda_function(
            ...
            **kwargs,
            ...
            base_environment = {"CUSTOM_SHIPYARD_HOSTNAME": chosen_shipyard_host}
    ...
    def test_scenario(self, port, infrastructure, aws_client_factory, chosen_shipyard_host):
        """
        Scenario:
        ...
        assert chosen_shipyard_host in stack_outputs["DomainEndpoint"]
        health_url = stack_outputs["DomainEndpoint"].replace(
            chosen_shipyard_host, constants.LOCALHOST_HOSTNAME
        )
        # we only have a route matcher for the domain with shipyard_host in the URL,
        ...
        assert chosen_shipyard_host in stack_outputs["ApiUrl"]
        api_url = (
            stack_outputs["ApiUrl"]
            .rstrip("/")
            .replace(chosen_shipyard_host, constants.LOCALHOST_HOSTNAME)
        )
