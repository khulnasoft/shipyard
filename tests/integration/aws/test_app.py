...
from shipyard.http.websocket import WebSocketRequest
from shipyard.services.edge import ROUTER


class TestExceptionHandlers:
    def test_internal_failure_handler_http_errors(self):
        response = requests.delete(config.internal_service_url() + "/_shipyard/health")
...
class TestWebSocketIntegration:
    ...
    def test_ssl_websockets(self, cleanups):
        ...
        url = (
            config.internal_service_url(host="localhost.shipyard.khulnasoft.com", protocol="wss")
            + "/_ws/world"
        )
        socket = websocket.WebSocket()
        socket.connect(url)
