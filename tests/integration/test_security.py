...
from shipyard.aws.handlers import cors as cors_handler
from shipyard.aws.handlers.cors import _get_allowed_cors_origins
from shipyard.constants import TEST_AWS_ACCESS_KEY_ID, TEST_AWS_REGION_NAME
...
class TestCSRF:
    ...
    def test_default_cors_headers(self):
        headers = {"Origin": "https://app.shipyard.khulnasoft.com"}
        response = requests.get(
            f"{config.internal_service_url()}/2015-03-31/functions/", headers=headers
        )
        assert response.status_code == 200
        assert response.headers["access-control-allow-origin"] == "https://app.shipyard.khulnasoft.com"
        assert "GET" in response.headers["access-control-allow-methods"].split(",")
        assert response.headers["Vary"] == "Origin"
        assert response.headers["Access-Control-Allow-Credentials"] == "true"

    @pytest.mark.parametrize("path", ["/health", "/_shipyard/health"])
    ...
    def test_disable_cors_checks(self, monkeypatch):
        ...
        headers = {"Origin": "https://invalid.shipyard.khulnasoft.com"}
