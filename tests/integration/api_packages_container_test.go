...
import (
	...
	container_module "github.com/khulnasoft/shipyard/modules/packages/container"
...
func TestPackageContainer(t *testing.T) {
	...
	for _, image := range images {
		t.Run(fmt.Sprintf("[Image:%s]", image), func(t *testing.T) {
			...
			t.Run("UploadBlob/Mount", func(t *testing.T) {
				...
				req := NewRequestWithBody(t, "POST", fmt.Sprintf("%sv2/%s/%s/blobs/uploads?digest=%s", setting.AppURL, privateUser.Name, image, privateBlobDigest), strings.NewReader("shipyard")).
	...
	// https://github.com/go-shipyard/shipyard/issues/19586
