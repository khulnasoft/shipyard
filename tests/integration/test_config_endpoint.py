...
from shipyard.http import Resource
from shipyard.services.internal import ConfigResource, get_internal_apis
from shipyard.utils import config_listener
...
def config_endpoint(monkeypatch):
    ...
    # will listen on /_shipyard/config
    rules = router.add(Resource("/_shipyard/config", ConfigResource()))
    yield
    router.remove(rules)
...
def test_config_endpoint(config_endpoint):
    ...
    url = f"{config.internal_service_url()}/_shipyard/config"
