...
import (
	...
	migrate_base "github.com/khulnasoft/shipyard/models/migrations/base"
...
func initMigrationTest(t *testing.T) func() {
	...
	if shipyardRoot == "" {
		tests.Printf("Environment variable $SHIPYARD_ROOT not set\n")
		os.Exit(1)
	}
	setting.AppPath = path.Join(shipyardRoot, "shipyard")
	if _, err := os.Stat(setting.AppPath); err != nil {
		tests.Printf("Could not find shipyard binary at %s\n", setting.AppPath)
		os.Exit(1)
	}

	shipyardConf := os.Getenv("SHIPYARD_CONF")
	if shipyardConf == "" {
		tests.Printf("Environment variable $SHIPYARD_CONF not set\n")
		os.Exit(1)
	} else if !path.IsAbs(shipyardConf) {
...
func availableVersions() ([]string, error) {
	...
	versionRE, err := regexp.Compile("shipyard-v(?P<version>.+)\\." + regexp.QuoteMeta(setting.Database.Type.String()) + "\\.sql.gz")
	if err != nil {
		return nil, err
	}
...
func readSQLFromFile(version string) (string, error) {
	filename := fmt.Sprintf("tests/integration/migration-test/shipyard-v%s.%s.sql.gz", version, setting.Database.Type)
...
func restoreOldDB(t *testing.T, version string) bool {
	...
	case setting.Database.Type.IsMSSQL():
		...
		_, err = db.Exec("DROP DATABASE IF EXISTS [shipyard]")
		assert.NoError(t, err)
