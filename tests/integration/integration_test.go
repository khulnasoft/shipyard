...
func TestMain(m *testing.M) {
	...
	if os.Getenv("SHIPYARD_SLOW_TEST_TIME") != "" {
		duration, err := time.ParseDuration(os.Getenv("SHIPYARD_SLOW_TEST_TIME"))
		if err == nil {
			testlogger.SlowTest = duration
		}
	}

	if os.Getenv("SHIPYARD_SLOW_FLUSH_TIME") != "" {
		duration, err := time.ParseDuration(os.Getenv("SHIPYARD_SLOW_FLUSH_TIME"))
		if err == nil {
...
func getTokenForLoggedInUser(t testing.TB, session *TestSession, scopes ...auth.AccessTokenScope) string {
	...
	if !assert.Equal(t, resp.Result().Header["Location"], []string{"/user/settings/applications"}) {
		for _, cookie := range resp.Result().Cookies() {
			if cookie.Name != shipyard_context.CookieNameFlash {
				continue
			}
