...
func InitTest(requireShipyard bool) {
	log.RegisterEventWriter("test", testlogger.NewTestLoggerWriter)

	shipyardRoot := base.SetupShipyardRoot()
	if shipyardRoot == "" {
		exitf("Environment variable $SHIPYARD_ROOT not set")
	}
	...
	if requireShipyard {
		...
		if _, err := os.Stat(setting.AppPath); err != nil {
			exitf("Could not find shipyard binary at %s", setting.AppPath)
		}
	}
	...
	if shipyardConf == "" {
		...
		// Notice: when doing "ssh push", Shipyard executes sub processes, debugger won't work for the sub processes.
		shipyardConf = "tests/sqlite.ini"
		_ = os.Setenv("SHIPYARD_CONF", shipyardConf)
		fmt.Printf("Environment variable $SHIPYARD_CONF not set, use default: %s\n", shipyardConf)
	...
	if !path.IsAbs(shipyardConf) {
		setting.CustomConf = filepath.Join(shipyardRoot, shipyardConf)
	} else {
...
func PrepareTestEnv(t testing.TB, skip ...int) func() {
	...
	lfsFixtures, err := storage.NewStorage(setting.LocalStorageType, &setting.Storage{
		Path: filepath.Join(filepath.Dir(setting.AppPath), "tests/shipyard-lfs-meta"),
	})
	assert.NoError(t, err)
