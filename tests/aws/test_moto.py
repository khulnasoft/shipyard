...
import shipyard.aws.accounts
from shipyard.aws.api import ServiceException, handler
...
from shipyard.utils.common import short_uid
...
def test_call_s3_with_streaming_trait(payload, monkeypatch):
    monkeypatch.setenv("MOTO_S3_CUSTOM_ENDPOINTS", "s3.localhost.shipyard.khulnasoft.com:4566")

    # In this test we use low-level interface with Moto and skip the standard setup
    # In the absence of below patch, Moto and Shipyard uses difference AWS Account IDs causing the test to fail
    monkeypatch.setattr(shipyard.aws.accounts, "DEFAULT_AWS_ACCOUNT_ID", DEFAULT_MOTO_ACCOUNT_ID)
