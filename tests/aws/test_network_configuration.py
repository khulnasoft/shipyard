...
from shipyard.utils.files import new_tmp_file, save_file
from shipyard.utils.strings import short_uid


class TestOpenSearch:
    """
    OpenSearch does not respect any customisations and just returns a domain with localhost.shipyard.khulnasoft.com in.
    """
    ...
    * SHIPYARD_HOST
    ...
    def test_off_strategy_with_external_port(
        ...
        monkeypatch.setattr(
            config,
            "SHIPYARD_HOST",
            config.HostAndPort(host=shipyard_host().host, port=external_port),
        )
