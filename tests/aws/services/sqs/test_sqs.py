...
from shipyard.services.sqs.models import sqs_stores
from shipyard.services.sqs.provider import MAX_NUMBER_OF_MESSAGES
from shipyard.services.sqs.utils import parse_queue_url
from shipyard.testing.pytest import markers
from shipyard.testing.snapshots.transformer import GenericTransformer
from shipyard.utils.aws import arns
from shipyard.utils.aws.request_context import mock_aws_request_headers
from shipyard.utils.common import poll_condition, retry, short_uid, to_str
...
class TestSqsProvider:
    ...
    def test_get_queue_url_contains_shipyard_host(
    ...
    def test_create_queue_recently_deleted_cache(self, sqs_create_queue, monkeypatch, aws_client):
        ...
        from shipyard.services.sqs import constants
    ...
    def test_send_message_with_delay_0_works_for_fifo(self, sqs_create_queue, aws_client):
        # see issue https://github.com/khulnasoft/shipyard/issues/6612
        queue_name = f"queue-{short_uid()}.fifo"
        attributes = {"FifoQueue": "true"}
        queue_url = sqs_create_queue(QueueName=queue_name, Attributes=attributes)
    ...
    @markers.aws.validated
    @pytest.mark.xfail(reason="see https://github.com/khulnasoft/shipyard/issues/5938")
    ...
    @pytest.mark.skip("Needs AWS fixing and is now failing against Shipyard")
    ...
    def test_external_endpoint(self, monkeypatch, sqs_create_queue, aws_client):
        ...
        monkeypatch.setattr(config, "SQS_ENDPOINT_STRATEGY", "off")
        monkeypatch.setattr(
            config, "SHIPYARD_HOST", config.HostAndPort(host=external_host, port=external_port)
        )
    ...
    def test_fifo_receive_message_visibility_timeout_shared_in_group(
        ...
        assert ordering in (
            ["g3-m1", "g1-m1", "g1-m2", "g1-m3", "g1-m4", "g2-m1"],  # aws and shipyard
            ["g3-m1", "g2-m1", "g1-m1", "g1-m2", "g1-m3", "g1-m4"],  # shipyard
        )
    ...
    def test_fifo_message_group_visibility_after_delete(self, sqs_create_queue, aws_client):
        ...
        assert ordering in (
            ...
            ["g2-m1", "g2-m2", "g3-m1", "g1-m3", "g1-m4"],  # shipyard
        )
    ...
    @markers.aws.validated
    @pytest.mark.xfail(reason="not implemented in shipyard at the moment")
    ...
    def test_fifo_content_based_message_deduplication_arrives_once(
        ...
        # created for https://github.com/khulnasoft/shipyard/issues/6327
        queue_url = sqs_create_queue(
            QueueName=f"test-queue-{short_uid()}.fifo",
            Attributes={"FifoQueue": "true", "ContentBasedDeduplication": "true"},
        )
    ...
    @markers.aws.validated
    @pytest.mark.xfail(
        reason="shipyard allows queue names with slashes, but this should be deprecated"
    )
    ...
    @pytest.mark.xfail(reason="validation currently not implemented in shipyard")
    @markers.aws.validated
...
class TestSqsQueryApi:
    ...
    def test_queue_url_format_path_strategy(self, sqs_create_queue, monkeypatch):
        ...
        queue_url = sqs_create_queue(QueueName=queue_name)
        assert (
            f"localhost.shipyard.khulnasoft.com:4566/queue/{TEST_AWS_REGION_NAME}/{TEST_AWS_ACCOUNT_ID}/{queue_name}"
            in queue_url
        )
...
class TestSQSMultiAccounts:
    ...
    def test_cross_account_access(
        ...
        # but this is not enforced in Shipyard
        # - AddPermission
        # - CreateQueue
        # - DeleteQueue
        # - ListQueues
