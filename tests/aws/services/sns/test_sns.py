...
from shipyard.services.sns.constants import (
...
from shipyard.utils.aws.arns import parse_arn, sqs_queue_arn
from shipyard.utils.net import wait_for_port_closed, wait_for_port_open
from shipyard.utils.strings import short_uid, to_str
from shipyard.utils.sync import poll_condition, retry
...
class TestSNSSubscriptionCrud:
    ...
    def test_subscribe_with_invalid_protocol(self, sns_create_topic, sns_subscription, snapshot):
        ...
        with pytest.raises(ClientError) as e:
            sns_subscription(
                TopicArn=topic_arn, Protocol="test-protocol", Endpoint="shipyard@yopmail.com"
            )
...
class TestSNSSubscriptionSQS:
    ...
    def test_attribute_raw_subscribe(
        ...
        # https://github.com/khulnasoft/shipyard/issues/2432
        aws_client.sns.publish(
            TopicArn=topic_arn,
            Message=message,
...
class TestSNSSubscriptionSQSFifo:
    ...
    def test_fifo_topic_to_regular_sqs(
        ...
        # republish the message, to check deduplication
        # TODO: not implemented in Shipyard yet, only deduplication with FIFO SQS queues
        aws_client.sns.publish(TopicArn=topic_arn, Message=message, **kwargs)
        response = aws_client.sqs.receive_message(
            QueueUrl=queue_url,
            WaitTimeSeconds=3,
...
class TestSNSPlatformEndpoint:
    ...
    @markers.aws.only_shipyard  # needs real credentials for GCM/FCM
...
class TestSNSMultiAccounts:
    ...
    def test_cross_account_publish_to_sqs(
        ...
        Note: we are not setting Queue policies here as it's only in shipyard and IAM is not enforced, for the sake
        of simplicity
        """
...
class TestSNSRetrospectionEndpoints:
    ...
    def test_subscription_tokens_can_retrospect(
        ...
        # we won't confirm the subscription, to simulate an external provider that wouldn't be able to access Shipyard
