...
class TestACM:
    ...
    def test_certificate_for_subdomain_wildcard(
        ...
        domain_name = f"test-domain-{short_uid()}.localhost.shipyard.khulnasoft.com"
        ...
        if is_aws_cloud():
            ...
            prompt = (
                f"Please add the following CNAME entry to the Shipyard DNS server, then hit [ENTER] once "
