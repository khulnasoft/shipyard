...
from shipyard.aws.api.lambda_ import Architecture, Runtime
from shipyard.aws.connect import ServiceLevelClientFactory
from shipyard.services.lambda_.runtimes import RUNTIMES_AGGREGATED
from shipyard.testing.aws.lambda_utils import (
    concurrency_update_done,
    get_invoke_init_type,
    update_done,
)
from shipyard.testing.aws.util import create_client_with_keys, is_aws_cloud
from shipyard.testing.pytest import markers
from shipyard.testing.pytest.snapshot import is_aws
from shipyard.testing.snapshots.transformer import KeyValueBasedTransformer
from shipyard.testing.snapshots.transformer_utility import PATTERN_UUID
from shipyard.utils import files, platform, testutil
from shipyard.utils.aws import arns
from shipyard.utils.aws.arns import lambda_function_name
from shipyard.utils.files import load_file
from shipyard.utils.http import safe_requests
from shipyard.utils.platform import Arch, get_arch, is_arm_compatible, standardized_arch
from shipyard.utils.strings import short_uid, to_bytes, to_str
from shipyard.utils.sync import retry, wait_until
from shipyard.utils.testutil import create_lambda_archive
...
class TestLambdaBaseFeatures:
    ...
    def test_assume_role(
        ...
        due to an invalid role ARN: https://github.com/khulnasoft/shipyard/issues/9016
...
class TestLambdaBehavior:
    ...
    def test_ignore_architecture(self, create_lambda_function, monkeypatch, aws_client):
        ...
        # Assumes that Shipyard runs on native Docker host architecture
...
class TestLambdaErrors:
    ...
    @markers.aws.only_shipyard(
        reason="Can only induce Lambda-internal Docker error in Shipyard"
    )
    def test_lambda_runtime_startup_timeout(
        ...
        monkeypatch.setattr(
            config, "LAMBDA_DOCKER_FLAGS", "-e SHIPYARD_RUNTIME_ENDPOINT=http://somehost.invalid"
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # aws lambda invoke --debug --function-name shipyard-lambda-url-example \
...
class TestLambdaConcurrency:
    ...
    def test_reserved_concurrency_async_queue(self, create_lambda_function, snapshot, aws_client):
        ...
        #  https://github.com/khulnasoft/shipyard/pull/7874
