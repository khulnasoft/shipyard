...
from shipyard.utils.container_networking import get_main_container_network
from shipyard.utils.docker_utils import DOCKER_CLIENT, get_host_path_for_path_in_docker
from shipyard.utils.files import load_file, mkdir, rm_rf
...
class TestLambdaDNS:
    ...
    def test_lambda_localhost_shipyard.khulnasoft.com_connectivity(
        ...
        result = aws_client.lambda_.invoke(
            ...
            Payload=json.dumps(
                {
                    "url": f"http://localhost.shipyard.khulnasoft.com:{config.GATEWAY_LISTEN[0].port}/_shipyard/health"
                }
            ),
        )
