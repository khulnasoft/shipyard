...
from shipyard.constants import SECONDARY_TEST_AWS_REGION_NAME
from shipyard.services.lambda_.api_utils import ARCHITECTURES
...
from shipyard.testing.snapshots.transformer import SortingTransformer
...
from shipyard.utils.strings import long_uid, short_uid, to_str
from shipyard.utils.sync import ShortCircuitWaitException, wait_until
...
class TestLambdaProvisionedConcurrency:
    ...
    def test_provisioned_concurrency_limits(
        ...
        # Adjust limits in Shipyard to avoid creating a Lambda fork-bomb
        monkeypatch.setattr(config, "LAMBDA_LIMITS_CONCURRENT_EXECUTIONS", 5)
        monkeypatch.setattr(config, "LAMBDA_LIMITS_MINIMUM_UNRESERVED_CONCURRENCY", 3)
    ...
    def test_lambda_provisioned_lifecycle(
        ...
        # TODO: implement updates while IN_PROGRESS in Shipyard (currently not supported)
...
class TestLambdaSnapStart:
    ...
    def test_snapstart_lifecycle(self, create_lambda_function, snapshot, aws_client, runtime):
        """Test the API of the SnapStart feature. The optimization behavior is not supported in Shipyard.
