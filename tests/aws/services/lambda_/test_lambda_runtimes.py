...
from shipyard.constants import SHIPYARD_MAVEN_VERSION, MAVEN_REPO_URL
from shipyard.packages import DownloadInstaller, Package, PackageInstaller
...
from shipyard.utils.archives import unzip
from shipyard.utils.files import cp_r, load_file, mkdir, new_tmp_dir, save_file
...
from shipyard.utils.testutil import check_expected_lambda_log_events_length, get_lambda_log_events
...
TEST_LAMBDA_JAR_URL_TEMPLATE = "{url}/cloud/shipyard/{name}/{version}/{name}-{version}-tests.jar"


class LambdaJavaTestlibsPackage(Package):
    def __init__(self):
        super().__init__("JavaLambdaTestlibs", SHIPYARD_MAVEN_VERSION)

    def get_versions(self) -> List[str]:
        return [SHIPYARD_MAVEN_VERSION]
...
class LambdaJavaTestlibsPackageInstaller(DownloadInstaller):
    ...
    def _get_download_url(self) -> str:
        return TEST_LAMBDA_JAR_URL_TEMPLATE.format(
            version=self.version, url=MAVEN_REPO_URL, name="shipyard-utils"
        )
...
class TestNodeJSRuntimes:
    ...
    def test_invoke_nodejs_es6_lambda(self, create_lambda_function, snapshot, runtime, aws_client):
        ...
        assert "response from shipyard lambda" in response["body"]
...
class TestJavaRuntimes:
    ...
    def test_java_runtime_with_lib(self, create_lambda_function, snapshot, aws_client):
        ...
        for archive_desc, archive in [
            ...
            create_result = create_lambda_function(
                ...
                handler="cloud.shipyard.sample.LambdaHandlerWithLib",
            )
    ...
    def test_stream_handler(self, create_lambda_function, java_jar, runtime, snapshot, aws_client):
        ...
        create_lambda_function(
            ...
            zip_file=java_jar,
            runtime=runtime,
            handler="cloud.shipyard.awssdkv1.sample.LambdaStreamHandler",
        )
    ...
    def test_serializable_input_object(
        ...
        create_result = create_lambda_function(
            ...
            zip_file=java_zip,
            runtime=runtime,
            handler="cloud.shipyard.awssdkv1.sample.SerializedInputLambdaHandler",
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                "cloud.shipyard.sample.LambdaHandlerWithInterfaceAndCustom::handleRequestCustom",
                "CUSTOM",
            ),
            ("cloud.shipyard.sample.LambdaHandlerWithInterfaceAndCustom", "INTERFACE"),
            (
                "cloud.shipyard.sample.LambdaHandlerWithInterfaceAndCustom::handleRequest",
                "INTERFACE",
            ),
        ],
    )
    ...
    def test_java_lambda_subscribe_sns_topic(
        ...
        create_lambda_function(
            ...
            zip_file=java_zip,
            runtime=Runtime.java11,
            handler="cloud.shipyard.sample.LambdaHandler",
        )
...
class TestPythonRuntimes:
    ...
    def test_handler_in_submodule(self, create_lambda_function, runtime, aws_client):
        ...
        zip_file = testutil.create_lambda_archive(
            ...
            get_content=True,
            runtime=runtime,
            file_name="shipyard_package/def/main.py",
        )
        create_lambda_function(
            func_name=function_name,
            zip_file=zip_file,
            handler="shipyard_package.def.main.handler",
            runtime=runtime,
        )
