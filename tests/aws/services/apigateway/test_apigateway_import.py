...
from shipyard.aws.api.apigateway import Resources
from shipyard.aws.api.lambda_ import Runtime
from shipyard.constants import TEST_AWS_REGION_NAME
...
def apigateway_placeholder_authorizer_lambda_invocation_arn(aws_client, lambda_su_role):
    ...
    Shipyard does not validate the ARN here, so we can simply return a placeholder
    """
    if not is_aws_cloud():
...
class TestApiGatewayImportRestApi:
    ...
    def test_import_rest_apis_with_base_path_swagger(
        ...
        if is_aws_cloud():
            ...
            # then you realize Shipyard is needed!
            time.sleep(20)
