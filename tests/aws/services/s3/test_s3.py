...
import shipyard.config
from shipyard import config
from shipyard.aws.api.lambda_ import Runtime
from shipyard.aws.api.s3 import StorageClass
from shipyard.config import LEGACY_V2_S3_PROVIDER, S3_VIRTUAL_HOSTNAME
...
from shipyard.services.s3 import constants as s3_constants
from shipyard.services.s3.utils import (
    RFC1123,
...
from shipyard.testing.snapshots.transformer import RegexTransformer
from shipyard.testing.snapshots.transformer_utility import TransformerUtility
from shipyard.utils import testutil
from shipyard.utils.aws.request_context import mock_aws_request_headers
from shipyard.utils.aws.resources import create_s3_bucket
from shipyard.utils.files import load_file
from shipyard.utils.run import run
from shipyard.utils.server import http2_server
from shipyard.utils.strings import (
    checksum_crc32,
    checksum_crc32c,
    hash_sha1,
    hash_sha256,
...
from shipyard.utils.sync import retry
from shipyard.utils.testutil import check_expected_lambda_log_events_length
from shipyard.utils.urls import shipyard_host as get_shipyard_host
...
class TestS3:
    ...
    def test_metadata_header_character_decoding(self, s3_bucket, snapshot, aws_client):
        ...
        # https://github.com/khulnasoft/shipyard/issues/1790
        # put object
    ...
    def test_delete_bucket_no_such_bucket(self, snapshot, aws_client):
        with pytest.raises(ClientError) as e:
            aws_client.s3.delete_bucket(Bucket="does-not-exist-shipyard-test")
    ...
    def test_s3_object_expiry(self, s3_bucket, snapshot, aws_client):
        ...
        # https://github.com/khulnasoft/shipyard/issues/1685
    ...
    def test_range_header_body_length(self, s3_bucket, snapshot, aws_client):
        # Test for https://github.com/khulnasoft/shipyard/issues/1952
    ...
    def test_put_object_chunked_newlines(self, s3_bucket, aws_client):
        ...
        # Test for https://github.com/khulnasoft/shipyard/issues/1571
        object_key = "data"
        body = "Hello\r\n\r\n\r\n\r\n"
        headers = {
    ...
    def test_put_object_chunked_newlines_with_checksum(self, s3_bucket, aws_client):
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/khulnasoft/shipyard/issues/6659
        object_key = "data"
        body = "Hello Blob"
    ...
    def test_upload_part_chunked_newlines_valid_etag(self, s3_bucket, aws_client):
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/khulnasoft/shipyard/issues/8703
        body = "Hello Blob"
    ...
    def test_upload_part_chunked_cancelled_valid_etag(self, s3_bucket, aws_client):
        ...
        This test is to avoid regression for https://github.com/khulnasoft/shipyard/issues/9851
        """
        body = "Hello Blob"
        precalculated_etag = hashlib.md5(body.encode()).hexdigest()
        headers = {
    ...
    def test_put_object_with_md5_and_chunk_signature(self, s3_bucket, aws_client):
        # Boto still does not support chunk encoding, which means we can't test with the client nor
        # aws_http_client_factory. See open issue: https://github.com/boto/boto3/issues/751
        # Test for https://github.com/khulnasoft/shipyard/issues/4987
    ...
    def test_set_external_hostname(
        ...
        expected_url = (
            f"{_bucket_url(bucket_name=s3_bucket, shipyard_host=custom_hostname)}/{key}"
        )
        ...
        download_url = response["Location"].replace(
            f"{get_shipyard_host().host}:", "localhost.shipyard.khulnasoft.com:"
        )
    ...
    def test_s3_hostname_with_subdomain(self, aws_http_client_factory, aws_client):
        """
        This particular test validates the fix for shipyard#7424
    ...
    def test_create_bucket_head_bucket(self, aws_client_factory, snapshot, aws_client):
        ...
        snapshot.add_transformers_list(
            [
                ...
                snapshot.transform.regex(r"s3\.localhost\.shipyard\.cloud:4566", "<host>"),
                snapshot.transform.regex(r"s3\.localhost\.shipyard\.cloud:443", "<host>"),
    ...
    def test_s3_batch_delete_objects_using_requests_with_acl(
        ...
        url = f"{_bucket_url(s3_bucket, shipyard_host=get_shipyard_host().host)}?delete"
    ...
    @pytest.mark.xfail(
        condition=is_v2_provider,
        reason="Behaviour not implemented yet: https://github.com/khulnasoft/shipyard/issues/6882",
    )
    ...
    def test_s3_raw_request_routing(self, s3_bucket, aws_client):
        ...
        """
        default_endpoint = f"http://{get_shipyard_host().host_and_port()}"
...
class TestS3PresignedUrl:
    ...
    def test_presign_with_additional_query_params(
        ...
        """related to issue: https://github.com/khulnasoft/shipyard/issues/4133"""
    ...
    def test_delete_has_empty_content_length_header(self, s3_bucket, aws_client):
        for encoding in None, "gzip":
            ...
            # AWS does not send a content-length header at all, legacy shipyard sends a 0 length header
    ...
    def test_put_url_metadata_with_sig_s3v4(
        ...
        # https://github.com/khulnasoft/shipyard/issues/544
        metadata = {"foo": "bar"}
        object_key = "key-by-hostname"
        ...
        # if we skip validation, it should work for Shipyard
        if not verify_signature and not is_aws_cloud():
            assert response.ok, f"response returned {response.status_code}: {response.text}"
            # response body should be empty, see https://github.com/khulnasoft/shipyard/issues/1317
    ...
    def test_pre_signed_url_forward_slash_bucket(
        ...
        # however the signature will not match afterward (in AWS or with Shipyard)
...
class TestS3DeepArchive:
    ...
    def test_s3_get_deep_archive_object_restore(self, s3_create_bucket, snapshot, aws_client):
        ...
        if 'ongoing-request="false"' in response.get("Restore", ""):
            # if the restoring happens in Shipyard (or was fast in AWS) we can retrieve the object
...
class TestS3StaticWebsiteHosting:
    ...
    def test_routing_rules_conditions(self, s3_bucket, aws_client, allow_bucket_acl):
        # https://github.com/khulnasoft/shipyard/issues/6308
...
# TODO: maybe we can fake the IAM role as it's not needed in Shipyard
...
def _endpoint_url(region: str = "", shipyard_host: str = None) -> str:
    if not region:
    ...
    if region == "us-east-1":
        return f"{config.internal_service_url(host=shipyard_host or S3_VIRTUAL_HOSTNAME)}"
    return config.internal_service_url(host=f"s3.{region}.{LOCALHOST_HOSTNAME}")


def _bucket_url(bucket_name: str, region: str = "", shipyard_host: str = None) -> str:
    return f"{_endpoint_url(region, shipyard_host)}/{bucket_name}"
...
def _website_bucket_url(bucket_name: str):
    ...
    return _bucket_url_vhost(
        bucket_name, shipyard_host=shipyard.config.S3_STATIC_WEBSITE_HOSTNAME
    )


def _bucket_url_vhost(bucket_name: str, region: str = "", shipyard_host: str = None) -> str:
    if not region:
        region = AWS_REGION_US_EAST_1
    if os.environ.get("TEST_TARGET") == "AWS_CLOUD":
        if region == "us-east-1":
    ...
    host_definition = get_shipyard_host()
    if shipyard_host:
        host_and_port = f"{shipyard_host}:{config.GATEWAY_LISTEN[0].port}"
    else:
        host_and_port = (
