...
from shipyard.aws.api.opensearch import AdvancedSecurityOptionsInput, MasterUserOptions
from shipyard.constants import (
    OPENSEARCH_DEFAULT_VERSION,
    OPENSEARCH_PLUGIN_LIST,
    TEST_AWS_ACCOUNT_ID,
)
from shipyard.services.opensearch import provider
from shipyard.services.opensearch.cluster import CustomEndpoint, EdgeProxiedOpensearchCluster
from shipyard.services.opensearch.cluster_manager import (
...
from shipyard.utils.common import call_safe, poll_condition, retry, short_uid, start_worker_thread
from shipyard.utils.common import safe_requests as requests
...
class TestOpensearchProvider:
    """
    Because this test reuses the shipyard instance for each test, all tests are performed with
    ...
    def test_endpoint_strategy_port(self, monkeypatch, opensearch_create_domain, aws_client):
        ...
        assert parts[0] in (shipyard_host().host, "127.0.0.1")
...
class TestSingletonClusterManager:
    ...
    def test_endpoint_strategy_port_singleton_cluster(self, monkeypatch):
        ...
        assert parts[0] == "http"
        # either f"//{the bind host}" is used, or in the case of "//0.0.0.0" the shipyard hostname instead
        assert parts[1][2:] in [config.GATEWAY_LISTEN[0].host, shipyard_host().host]
