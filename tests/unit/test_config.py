...
from shipyard.config import HostAndPort, external_service_url, internal_service_url
...
class TestEdgeVariablesDerivedCorrectly:
    """We are deriving GATEWAY_LISTEN and SHIPYARD_HOST from provided environment variables.
    ...
    # input variable (gateway_listen, shipyard_host) has four unique
    ...
    @pytest.mark.parametrize(
        [
            ...
            "expected_shipyard_host",
        ],
        [
            ###
            (None, None, [f"{ip()}:4566"], "localhost.shipyard.khulnasoft.com:4566"),
            ("1.1.1.1", None, ["1.1.1.1:4566"], "localhost.shipyard.khulnasoft.com:4566"),
            (":5555", None, [f"{ip()}:5555"], "localhost.shipyard.khulnasoft.com:5555"),
            ("1.1.1.1:5555", None, ["1.1.1.1:5555"], "localhost.shipyard.khulnasoft.com:5555"),
            ###
            ...
            ###
            (None, ":7777", [f"{ip()}:4566"], "localhost.shipyard.khulnasoft.com:7777"),
            ("1.1.1.1", ":7777", ["1.1.1.1:4566"], "localhost.shipyard.khulnasoft.com:7777"),
            (":5555", ":7777", [f"{ip()}:5555"], "localhost.shipyard.khulnasoft.com:7777"),
            ("1.1.1.1:5555", ":7777", ["1.1.1.1:5555"], "localhost.shipyard.khulnasoft.com:7777"),
            ###
    ...
    def test_edge_configuration(
        ...
        shipyard_host: str | None,
        expected_gateway_listen: list[str],
        expected_shipyard_host: str,
    ):
        environment = {}
        if gateway_listen is not None:
            environment["GATEWAY_LISTEN"] = gateway_listen
        if shipyard_host is not None:
            environment["SHIPYARD_HOST"] = shipyard_host
        ...
        assert actual_ls_host == expected_shipyard_host
    ...
    def test_legacy_variables_ignored_if_given(self):
        ...
        assert shipyard_host == "localhost.shipyard.khulnasoft.com:4566"
        assert gateway_listen == [
...
class TestServiceUrlHelpers:
    @pytest.mark.parametrize(
        ...
        [
            # Default
            (None, None, None, None, "http://localhost.shipyard.khulnasoft.com:4566"),
            # Customize each part with defaults
            ("https", None, None, None, "https://localhost.shipyard.khulnasoft.com:4566"),
            (None, "s3", None, None, "http://s3.localhost.shipyard.khulnasoft.com:4566"),
            (None, None, "shipyard-container", None, "http://shipyard-container:4566"),
            (None, None, None, 5555, "http://localhost.shipyard.khulnasoft.com:5555"),
            # Multiple subdomains
            (
                None,
                ...
                None,
                None,
                "http://abc123.execute-api.lambda.localhost.shipyard.khulnasoft.com:4566",
            ),
            ...
            (
                ...
                "shipyard-container",
                5555,
                "https://abc.execute-api.shipyard-container:5555",
            ),
        ],
    )
