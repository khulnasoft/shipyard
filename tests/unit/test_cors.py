...
from shipyard.aws.handlers import cors
...
def test_allowed_cors_origins_different_ports_and_protocols(monkeypatch):
    ...
    assert "http://localhost.shipyard.khulnasoft.com:4566" in origins
    assert "http://localhost:433" not in origins
    assert "https://localhost.shipyard.khulnasoft.com:443" not in origins
    ...
    assert "http://localhost.shipyard.khulnasoft.com:4566" in origins
    assert "https://localhost.shipyard.khulnasoft.com:443" in origins


def test_dynamic_allowed_cors_origins(monkeypatch):
    assert _origin_allowed("http://test.s3-website.localhost.shipyard.khulnasoft.com")
    assert _origin_allowed("https://test.s3-website.localhost.shipyard.khulnasoft.com")
    assert _origin_allowed("http://test.cloudfront.localhost.shipyard.khulnasoft.com")

    assert not _origin_allowed("https://test.appsync.localhost.shipyard.khulnasoft.com")
    assert not _origin_allowed("https://testcloudfront.localhost.shipyard.khulnasoft.com")
...
def test_dynamic_allowed_cors_origins_different_ports(monkeypatch):
    ...
    assert _origin_allowed("http://test.s3-website.localhost.shipyard.khulnasoft.com:4566")
    assert _origin_allowed("http://test.s3-website.localhost.shipyard.khulnasoft.com")
    assert _origin_allowed("https://test.s3-website.localhost.shipyard.khulnasoft.com:4566")
    assert _origin_allowed("https://test.s3-website.localhost.shipyard.khulnasoft.com")
    assert _origin_allowed("http://test.cloudfront.localhost.shipyard.khulnasoft.com")

    assert not _origin_allowed("https://test.cloudfront.localhost.shipyard.khulnasoft.com:443")
    assert not _origin_allowed("http://test.cloudfront.localhost.shipyard.khulnasoft.com:123")

    # test allowed origins for extended config (:4566,:443)
    monkeypatch.setattr(config, "GATEWAY_LISTEN", default_gateway_listen_ext)
    monkeypatch.setattr(cors, "_ALLOWED_INTERNAL_PORTS", cors._get_allowed_cors_ports())

    assert _origin_allowed("https://test.cloudfront.localhost.shipyard.khulnasoft.com:443")
