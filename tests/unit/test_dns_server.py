...
from shipyard.dns.models import AliasTarget, RecordType, SOARecord, TargetRecord
from shipyard.dns.server import DnsServer, add_resolv_entry, get_fallback_dns_server
from shipyard.utils.net import get_free_udp_port
...
class TestDNSServer:
    ...
    def test_dns_server_fallback(self, dns_server, query_dns):
        ...
        answer = query_dns("localhost.shipyard.khulnasoft.com", "A")
    ...
    def test_overriding_with_dns_resolve_ip(self, dns_server, query_dns, monkeypatch):
        ...
        dns_server.add_host_pointing_to_shipyard("example.org")
    ...
    def test_redirect_to_shipyard_lifecycle(self, dns_server, query_dns):
        ...
        # delete host pointing to shipyard again
        dns_server.delete_host_pointing_to_shipyard("*.example.org")
        answer = query_dns("subdomain.example.org", "A")
        assert not answer.answer
    ...
    def test_redirect_to_shipyard_with_skip(self, dns_server, query_dns):
        """Test to-shipyard redirects with skip patterns for certain names"""
        # add ipv4 host
        dns_server.add_host_pointing_to_shipyard("*.example.org")
        answer = query_dns("subdomain.example.org", "A")
        assert answer.answer
...
class TestDnsUtils:
    def test_resolv_conf_overwriting(self, tmp_path: Path, monkeypatch):
        from shipyard.dns import server
