...
from shipyard.constants import TEST_AWS_ACCOUNT_ID
from shipyard.services.opensearch.cluster_manager import DomainKey, build_cluster_endpoint
...
class TestBuildClusterEndpoint:
    def test_endpoint_strategy_port(self, monkeypatch):
        ...
        assert parts[0] == "localhost.shipyard.khulnasoft.com"
    ...
    def test_endpoint_strategy_path(self, monkeypatch, engine):
        ...
        assert (
            endpoint == f"localhost.shipyard.khulnasoft.com:4566/{engine_path_prefix}/us-east-1/my-domain"
        )
        ...
        assert (
            endpoint
            == f"localhost.shipyard.khulnasoft.com:4566/{engine_path_prefix}/eu-central-1/my-domain-1"
        )
    ...
    def test_endpoint_strategy_domain(self, monkeypatch, engine):
        ...
        assert (
            endpoint == f"my-domain.us-east-1.{engine_path_prefix}.localhost.shipyard.khulnasoft.com:4566"
        )
        ...
        assert (
            endpoint
            == f"my-domain-1.eu-central-1.{engine_path_prefix}.localhost.shipyard.khulnasoft.com:4566"
        )
