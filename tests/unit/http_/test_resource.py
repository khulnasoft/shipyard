...
from shipyard.http import Request, Resource, Response, Router, resource
...
class TestResource:
    def test_resource_decorator_dispatches_correctly(self):
        ...
        request1 = Request("GET", "/_shipyard/health")
        request2 = Request("POST", "/_shipyard/health")
        request3 = Request("HEAD", "/_shipyard/health")
    ...
    def test_resource_dispatches_correctly(self):
        ...
        router.add(Resource("/_shipyard/health", TestResource()))
    ...
    def test_dispatch_to_non_existing_method_raises_exception(self):
        ...
        with pytest.raises(MethodNotAllowed):
            assert router.dispatch(Request("GET", "/_shipyard/health"))
        assert router.dispatch(Request("POST", "/_shipyard/health")).get_data(True) == "POST/OK"

    def test_resource_with_default_dispatcher(self):
        router = Router()

        @resource("/_shipyard/<path>")
        class TestResource:
        ...
        router.add(TestResource())
        assert router.dispatch(Request("GET", "/_shipyard/health")).json == {
            "message": "GET/OK",
            "path": "health",
        }
        assert router.dispatch(Request("POST", "/_shipyard/foobar")).json == {
            "message": "POST/OK",
            "path": "foobar",
        }
    ...
    def test_resource_overwrite_with_resource_wrapper(self):
        ...
        assert router.dispatch(Request("GET", "/_shipyard/health")).json == {
            "message": "GET/OK",
            "path": "/_shipyard/health",
        }
        assert router.dispatch(Request("POST", "/_shipyard/health")).json == {
            "message": "POST/OK",
            "path": "/_shipyard/health",
        }
