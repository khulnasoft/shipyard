...
from shipyard.http.router import E, RequestArguments, route
from shipyard.utils.common import get_free_tcp_port
...
class TestRouter:
    ...
    def test_dispatch_with_host_matching(self):
        ...
        with pytest.raises(NotFound):
            invoke("/", "shipyard.khulnasoft.com", None)
    ...
    def test_regex_host_dispatcher(self):
        router = Router()
        rgx = r"\.cloudfront.(net|localhost\.shipyard\.cloud)"
        router.add(path="/", endpoint=noop, host=f"<dist_id><regex('{rgx}'):host>:<port>")
        assert router.dispatch(
            Request(
                method="GET",
                headers={"Host": "ad91f538.cloudfront.localhost.shipyard.khulnasoft.com:5446"},
            )
        )
        with pytest.raises(NotFound):
            router.dispatch(
    ...
    def test_port_host_dispatcher(self):
        collector = RequestCollector()
        router = Router(dispatcher=collector)
        router.add(path="/", endpoint=noop, host="localhost.shipyard.khulnasoft.com<port:port>")
        # matches with the port!
        assert router.dispatch(
            Request(
                method="GET",
                headers={"Host": "localhost.shipyard.khulnasoft.com:4566"},
            )
        )
        ...
        assert router.dispatch(
            Request(
                method="GET",
                headers={"Host": "localhost.shipyard.khulnasoft.com"},
            )
        )
        ...
        with pytest.raises(NotFound):
            router.dispatch(
                Request(
                    method="GET",
                    headers={"Host": "localhost.shipyard.khulnasoft.com:544a6"},
                )
            )
        ...
        with pytest.raises(NotFound):
            router.dispatch(
                Request(
                    method="GET",
                    headers={"Host": "shipyard.khulnasoft.com:5446"},
                )
            )
    ...
    def test_path_converter_and_regex_converter_in_host(self):
        ...
        router.add(
            path="/<path:path>",
            host="foobar.us-east-1.opensearch.localhost.shipyard.khulnasoft.com<regex('(?::.*)?'):port>",
        ...
        assert router.dispatch(
            Request(
                ...
                headers={"Host": "foobar.us-east-1.opensearch.localhost.shipyard.khulnasoft.com:4566"},
            )
    ...
    def test_path_converter_and_port_converter_in_host(self):
        ...
        router.add(
            path="/<path:path>",
            host="foobar.us-east-1.opensearch.localhost.shipyard.khulnasoft.com<port:port>",
            endpoint=echo_params_json,
        )
        assert router.dispatch(
            Request(
        ...
        assert router.dispatch(
            Request(
                method="GET",
                path="/_cluster/health",
                headers={"Host": "foobar.us-east-1.opensearch.localhost.shipyard.khulnasoft.com"},
            )
    ...
    def test_path_converter_and_greedy_regex_in_host(self):
        ...
        assert router.dispatch(
            ...
            "path": "_cluster/health",
            "host": "localhost.shipyard.khulnasoft.com:4566",
            "port": None,
        }
