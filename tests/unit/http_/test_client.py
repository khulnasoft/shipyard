...
from shipyard.utils.ssl import create_ssl_cert
...
def test_http_clients_respect_verify(verify, cert_env, ssl_httpserver, monkeypatch):
    ...
    # Send the request to "localhost.shipyard.khulnasoft.com", which is in the SAN and can be verified
    response = client.request(
        request, server=f"localhost.shipyard.khulnasoft.com:{ssl_httpserver.port}"
    ).json
    assert response == expected_response
