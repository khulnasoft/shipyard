...
import shipyard.constants
import shipyard.utils.analytics.cli
from shipyard import config, constants
from shipyard.cli.shipyard import create_with_plugins, is_frozen_bundle
...
from shipyard.utils.common import is_command_available
from shipyard.utils.container_utils.container_client import ContainerException, DockerNotAvailable
...
def test_error_handling_help(runner):
    ...
    assert "Usage: shipyard" in result.output


def test_create_with_plugins(runner):
    shipyard_cli = create_with_plugins()
    result = runner.invoke(shipyard_cli.group, ["--version"])
    assert result.exit_code == 0
...
def test_container_not_runnin_error(runner, command):
    ...
    assert result.exit_code == 1
    assert "Error" in result.output
    assert "Expected a running Shipyard container" in result.output
...
def test_status_services(runner, httpserver, monkeypatch):
    # configure SHIPYARD_HOST because the services endpoint makes a request against the
    # external URL of Shipyard, which may be different to the edge port
    monkeypatch.setattr(
        config,
        "SHIPYARD_HOST",
        HostAndPort(
            host="localhost.shipyard.khulnasoft.com",
            port=httpserver.port,
        ),
    )

    services = {"dynamodb": "starting", "s3": "running"}
    httpserver.expect_request("/_shipyard/health", method="GET").respond_with_json(
        {"services": services}
    )
...
services:
  shipyard:
    ...
    environment:
      ...
      - DEBUG=${DEBUG- }
      - DATA_DIR=${DATA_DIR- }
      - SHIPYARD_API_KEY=${SHIPYARD_API_KEY- }
      - KINESIS_ERROR_PROBABILITY=${KINESIS_ERROR_PROBABILITY- }
      - DOCKER_HOST=unix:///var/run/docker.sock
    volumes:
      - "${TMPDIR:-/tmp/shipyard}:/tmp/shipyard"
...
@pytest.mark.parametrize(
    ...
    [
        ("stop", "shipyard stop", []),
        ("config show", "shipyard config show", ["format_"]),
        ("--debug config show --format plain", "shipyard config show", ["format_"]),
    ],
)
def test_publish_analytics_event_on_command_invocation(
    ...
    monkeypatch.setattr(shipyard.utils.analytics.cli, "ANALYTICS_API_RESPONSE_TIMEOUT_SECS", 3)
    request_data = Queue()
    ...
    with testutil.http_server(handler) as url:
        ...
        monkeypatch.setattr(shipyard.constants, "ANALYTICS_API", url)
...
def test_disable_publish_analytics_event_on_command_invocation(runner, monkeypatch, caplog):
    ...
    caplog.set_level(logging.CRITICAL)
    monkeypatch.setattr(shipyard.utils.analytics.cli, "ANALYTICS_API_RESPONSE_TIMEOUT_SECS", 3)
    monkeypatch.setattr(shipyard.config, "DISABLE_EVENTS", True)
    request_data = []
