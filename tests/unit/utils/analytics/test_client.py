...
from shipyard.utils.analytics.client import AnalyticsClient
from shipyard.utils.analytics.events import Event, EventMetadata
from shipyard.utils.analytics.metadata import get_client_metadata, get_session_id
...
def test_append_events(httpserver: HTTPServer):
    ...
    assert request1.headers["Shipyard-Session-Id"] == get_session_id()
    assert request1.headers["User-Agent"] == f"shipyard/{constants.VERSION}"
