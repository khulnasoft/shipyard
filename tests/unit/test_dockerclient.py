...
from shipyard.utils.bootstrap import extract_port_flags
...
class TestDockerClient:
    ...
    @patch("shipyard.utils.container_utils.docker_cmd_client.run")
    def test_list_containers(self, run_mock):
        mock_container = {
            "ID": "00000000a1",
            "Image": "khulnasoft/shipyard",
            "Names": "shipyard-main",
            "Labels": "authors=Shipyard Contributors",
            "State": "running",
        }
        return_container = {
    ...
    def test_container_status(self, run_mock):
        test_output = "Up 2 minutes - shipyard-main"
        run_mock.return_value = test_output
        docker_client = CmdDockerClient()
        status = docker_client.get_container_status("shipyard-main")
        assert status == DockerContainerStatus.UP
        run_mock.return_value = "Exited (0) 1 minute ago - shipyard-main"
        status = docker_client.get_container_status("shipyard-main")
