...
from shipyard.aws.protocol.service_router import determine_aws_service_name, get_service_catalog
from shipyard.http import Request
from shipyard.utils.run import to_str
...
def _botocore_request_to_shipyard_request(request_object: AWSRequest) -> Request:
...
def test_service_router_works_for_every_service(
    ...
    # The endpoint URL is mandatory here, just set a dummy (doesn't _need_ to be shipyard specific)
    request_dict = client._convert_to_request_dict(
        request_args, operation, "http://localhost.shipyard.khulnasoft.com", request_context
    )
    request_object = create_request_object(request_dict)
    client._request_signer.sign(operation.name, request_object)
    request: Request = _botocore_request_to_shipyard_request(request_object)
...
def test_endpoint_prefix_based_routing():
    # TODO could be generalized using endpoint resolvers and replacing "amazonaws.com" with "localhost.shipyard.khulnasoft.com"
    detected_service_name = determine_aws_service_name(
        Request(method="GET", path="/", headers={"Host": "kms.localhost.shipyard.khulnasoft.com"})
    )
    ...
    detected_service_name = determine_aws_service_name(
        Request(
            method="POST",
            path="/app-instances",
            headers={"Host": "identity-chime.localhost.shipyard.khulnasoft.com"},
        )
    )
...
def test_endpoint_prefix_based_routing_s3_virtual_host():
    detected_service_name = determine_aws_service_name(
        Request(method="GET", path="/", headers={"Host": "pictures.s3.localhost.shipyard.khulnasoft.com"})
    )
    ...
    detected_service_name = determine_aws_service_name(
        Request(
            method="POST",
            path="/app-instances",
            headers={"Host": "kms.s3.localhost.shipyard.khulnasoft.com"},
        )
    )
    assert detected_service_name == "s3"
...
def test_endpoint_prefix_based_not_short_circuit_for_sqs():
    detected_service_name = determine_aws_service_name(
        Request(method="GET", path="/", headers={"Host": "sqs.localhost.shipyard.khulnasoft.com"})
    )
    ...
    detected_service_name = determine_aws_service_name(
        Request(
            ...
            path="/",
            headers={
                "Host": "sqs.localhost.shipyard.khulnasoft.com",
