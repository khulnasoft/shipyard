...
import shipyard.utils.container_utils.docker_cmd_client
from shipyard import config, constants
from shipyard.cli.shipyard import shipyard as cli
from shipyard.config import Directories, in_docker
from shipyard.constants import LOCALHOST_HOSTNAME, LOCALHOST_IP, MODULE_MAIN_PATH, TRUE_STRINGS
from shipyard.utils import bootstrap
from shipyard.utils.bootstrap import in_ci
from shipyard.utils.common import poll_condition
from shipyard.utils.container_utils.container_client import ContainerClient, NoSuchImage
from shipyard.utils.files import mkdir
from shipyard.utils.net import get_free_udp_port, send_dns_query
from shipyard.utils.run import run, to_str
...
def container_client():
    client = shipyard.utils.container_utils.docker_cmd_client.CmdDockerClient()
...
class TestCliContainerLifecycle:
    def test_start_wait_stop(self, runner, container_client):
        ...
        assert "starting Shipyard" in result.output
        ...
        # Note: if `SHIPYARD_HOST` is set to a domain that does not resolve to `127.0.0.1` then
        # this test will fail
        health = requests.get(config.external_service_url() + "/_shipyard/health")
        ...
        with pytest.raises(requests.ConnectionError):
            requests.get(config.external_service_url() + "/_shipyard/health")
    ...
    def test_status_services(self, runner):
        ...
        assert result.exit_code != 0
        assert "could not connect to Shipyard health endpoint" in result.output
    ...
    def test_volume_dir_mounted_correctly(self, runner, tmp_path, monkeypatch, container_client):
        ...
        monkeypatch.setenv("SHIPYARD_VOLUME_DIR", str(volume_dir))
    ...
    def test_container_starts_non_root(self, runner, monkeypatch, container_client):
        ...
        if in_ci() and os.path.exists("/home/runner"):
            volume_dir = "/home/runner/.cache/shipyard/volume/"
            mkdir(volume_dir)
        ...
        assert "shipyard-supervisor" in to_str(output[0])
    ...
    def test_start_cli_within_container(self, runner, container_client, tmp_path):
        output = container_client.run_container(
            ...
            remove=True,
            entrypoint="",
            command=["bin/shipyard", "start", "-d"],
            mount_volumes=[
                ("/var/run/docker.sock", "/var/run/docker.sock"),
                (MODULE_MAIN_PATH, "/opt/code/khulnasoft/shipyard"),
            ],
            env_vars={"SHIPYARD_VOLUME_DIR": f"{tmp_path}/ls-volume"},
        )
        stdout = to_str(output[0])
        assert "starting Shipyard" in stdout
    ...
    def test_prepare_host_hook_called_with_correct_dirs(self, runner, monkeypatch):
        ...
        def _prepare_host(*args, **kwargs):
            ...
            result_configs.append(
                (config.is_in_docker, os.getenv(constants.SHIPYARD_INFRA_PROCESS), config.dirs)
            )
...
class TestImports:
    ...
    def test_import_venv(self):
        from shipyard.utils.venv import VirtualEnvironment
