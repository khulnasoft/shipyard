...
import (
	...
	"github.com/khulnasoft/shipyard/modules/templates/scopedtmpl"
...
func wrapTmplErrMsg(msg string) {
	...
	if setting.IsProd {
		// in prod mode, Shipyard must have correct templates to run
		log.Fatal("Shipyard can't run with template errors: %s", msg)
	} else {
		...
		log.Error("There are template errors but Shipyard continues to run in dev mode: %s", msg)
	}
}
