...
import (
	...
	"github.com/khulnasoft/shipyard/modules/markup/mdstripper"
...
var (
	...
	// e.g. go-shipyard/shipyard@d8a994ef, go-shipyard/shipyard@d8a994ef243349f321568f9e36d5c3f444b99cae (7-40 characters)
	...
	shipyardHostInit         sync.Once
	shipyardHost             string
	shipyardIssuePullPattern *regexp.Regexp
...
// getShipyardHostName returns a normalized string with the local host name, with no scheme or port information
func getShipyardHostName() string {
	shipyardHostInit.Do(func() {
		if uapp, err := url.Parse(setting.AppURL); err == nil {
			shipyardHost = strings.ToLower(uapp.Host)
			shipyardIssuePullPattern = regexp.MustCompile(
				`(\s|^|\(|\[)` +
			...
			shipyardHost = ""
			shipyardIssuePullPattern = nil
		}
	})
	return shipyardHost
}

// getShipyardIssuePullPattern
func getShipyardIssuePullPattern() *regexp.Regexp {
	getShipyardHostName()
	return shipyardIssuePullPattern
}
...
func convertFullHTMLReferencesToShortRefs(re *regexp.Regexp, contentBytes *[]byte) {
	...
	//
	// this is a https://ourshipyard.khulnasoft.com/git/owner/repo/issues/123456789, foo
	...
	pos := 0
	for {
		// re looks for something like: (\s|^|\(|\[)https://ourshipyard.khulnasoft.com/git/(owner/repo)/(issues)/(123456789)(?:\s|$|\)|\]|[:;,.?!]\s|[:;,.?!]$)
...
func FindAllIssueReferences(content string) []IssueReference {
	...
	if re := getShipyardIssuePullPattern(); re != nil {
		convertFullHTMLReferencesToShortRefs(re, &contentBytes)
	} else {
		log.Debug("No ShipyardIssuePullPattern pattern")
	}
