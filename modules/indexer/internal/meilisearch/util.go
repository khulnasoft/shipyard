...
func (i *Indexer) checkOldIndexes() {
	for v := 0; v < i.version; v++ {
		...
		if err == nil {
			log.Warn("Found older meilisearch index named %q, Shipyard will keep the old NOT DELETED. You can delete the old version after the upgrade succeed.", indexName)
		}
	}
}
