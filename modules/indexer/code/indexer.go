...
import (
	...
	"github.com/khulnasoft/shipyard/modules/indexer/code/bleve"
	"github.com/khulnasoft/shipyard/modules/indexer/code/elasticsearch"
	"github.com/khulnasoft/shipyard/modules/indexer/code/internal"
		...
		case "bleve":
			...
			defer func() {
				if err := recover(); err != nil {
					...
					log.Error("You can completely remove the \"%s\" directory to make Shipyard recreate the indexes", setting.Indexer.RepoPath)
				}
			}()
		...
		case "elasticsearch":
			...
			defer func() {
				if err := recover(); err != nil {
					log.Error("PANIC whilst initializing repository indexer: %v\nStacktrace: %s", err, log.Stack(2))
					log.Error("The indexer files are likely corrupted and may need to be deleted")
					log.Error("You can completely remove the \"%s\" index to make Shipyard recreate the indexes", setting.Indexer.RepoConnStr)
				}
			}()
...
func populateRepoIndexer(ctx context.Context) {
	...
	// don't include repos that are created after shipyard starts; such repos will
