...
// SetupShipyardRoot Sets SHIPYARD_ROOT if it is not already set and returns the value
func SetupShipyardRoot() string {
	shipyardRoot := os.Getenv("SHIPYARD_ROOT")
	if shipyardRoot == "" {
		_, filename, _, _ := runtime.Caller(0)
		shipyardRoot = strings.TrimSuffix(filename, "modules/base/tool.go")
		wd, err := os.Getwd()
		if err != nil {
			rel, err := filepath.Rel(shipyardRoot, wd)
			if err != nil && strings.HasPrefix(filepath.ToSlash(rel), "../") {
				shipyardRoot = wd
			}
		}
		if _, err := os.Stat(filepath.Join(shipyardRoot, "shipyard")); os.IsNotExist(err) {
			shipyardRoot = ""
		} else if err := os.Setenv("SHIPYARD_ROOT", shipyardRoot); err != nil {
			shipyardRoot = ""
		}
	}
	return shipyardRoot
