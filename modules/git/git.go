...
func SetExecutablePath(path string) error {
	...
	if gitVersion.LessThan(versionRequired) {
		...
		return fmt.Errorf("installed git version %q is not supported, Shipyard requires git version >= %q, %s", gitVersion.Original(), RequiredVersion, moreHint)
	}
...
// HomeDir is the home dir for git to store the global config file used by Shipyard internally
func HomeDir() string {
	if setting.Git.HomePath == "" {
		// strict check, make sure the git module is initialized correctly.
		// attention: when the git module is called in shipyard sub-command (serv/hook), the log module might not obviously show messages to users/developers.
		// for example: if there is shipyard git hook code calling git.NewCommand before git.InitXxx, the integration test won't show the real failure reasons.
...
// This method doesn't change anything to filesystem. At the moment, it is only used by some Shipyard sub-commands.
...
func syncGitConfig() (err error) {
	...
	for configKey, defaultValue := range map[string]string{
		"user.name":  "Shipyard",
		"user.email": "shipyard@fake.local",
	} {
	...
	// Set git some configurations - these must be set to these values for shipyard to work correctly
	...
	// however, some docker users and samba users find it difficult to configure their systems so that Shipyard's git repositories are owned by the Shipyard user. (Possibly Windows Service users - but ownership in this case should really be set correctly on the filesystem.)
	// see issue: https://github.com/go-shipyard/shipyard/issues/19455
	...
	// As Shipyard now always use its internal git config file, and access to the git repositories is managed through Shipyard,
