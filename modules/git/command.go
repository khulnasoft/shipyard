...
import (
	...
	"github.com/khulnasoft/shipyard/modules/git/internal" //nolint:depguard // only this file can use the internal type CmdArg, other files and packages should use AddXxx functions
...
type RunOpts struct {
	...
	// * /some/path/.git
	// * /some/path/.git/shipyard-data/data/repositories/user/repo.git
...
func commonBaseEnvs() []string {
	...
	envs := []string{
		"HOME=" + HomeDir(),        // make Shipyard use internal git config only, to prevent conflicts with user's git config
...
// CommonCmdServEnvs is like CommonGitCmdEnvs, but it only returns minimal required environment variables for the "shipyard serv" command
