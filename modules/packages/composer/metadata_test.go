...
const (
	name        = "shipyard.khulnasoft.composer-package"
	description = "Package Description"
	packageType = "composer-plugin"
	author      = "Shipyard Authors"
	email       = "no.reply@shipyard.io"
...
const composerContent = `{
    ...
    "homepage": "` + homepage + `",
    "autoload": {
        "psr-4": {"Shipyard\\ComposerPackage\\": "src/"}
    },
    "require": {
        "php": ">=7.2 || ^8.0"
    }
...
func TestParseComposerFile(t *testing.T) {
	...
	t.Run("InvalidPackageVersion", func(t *testing.T) {
		cp, err := ParseComposerFile(strings.NewReader(`{"name": "shipyard.khulnasoft.composer-package", "version": "1.a.3"}`))
		assert.Nil(t, cp)
