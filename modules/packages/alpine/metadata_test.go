...
const (
	...
	packageProjectURL  = "https://shipyard.io"
	packageMaintainer  = "KN4CK3R <dummy@shipyard.io>"
)
...
func TestParsePackageInfo(t *testing.T) {
	...
	t.Run("Valid", func(t *testing.T) {
		...
		assert.Equal(t, "Shipyard <pack@ag.er>", p.FileMetadata.Packager)
		...
		assert.ElementsMatch(t, []string{"common", "shipyard"}, p.FileMetadata.Provides)
		assert.ElementsMatch(t, []string{"common", "shipyard"}, p.FileMetadata.Dependencies)
	})
}
