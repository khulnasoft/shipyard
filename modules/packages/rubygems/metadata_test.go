...
4ftn7/yUxd4YGcglvTmmY8aGY3ZwRn4CqcWcidUGAAA=`)
	...
	assert.Equal(t, "shipyard", rp.Name)
	assert.Equal(t, "1.0.5", rp.Version)
	assert.Equal(t, "ruby", rp.Metadata.Platform)
	assert.Equal(t, "Shipyard package", rp.Metadata.Summary)
	assert.Equal(t, "RubyGems package test", rp.Metadata.Description)
	assert.Equal(t, []string{"Shipyard"}, rp.Metadata.Authors)
	assert.Equal(t, "https://shipyard.io/", rp.Metadata.ProjectURL)
