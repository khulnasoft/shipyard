...
var (
	// AppPath represents the path to the shipyard binary
	AppPath string

	// AppWorkPath is the "working directory" of Shipyard. It maps to the: WORK_PATH in app.ini, "--work-path" flag, environment variable SHIPYARD_WORK_DIR.
	// If that is not set it is the default set here by the linker or failing that the directory of AppPath.
	// It is used as the base path for several other paths.
	AppWorkPath string
	CustomPath  string // Custom directory path. Env: SHIPYARD_CUSTOM
	CustomConf  string
...
func InitWorkPathAndCfgProvider(getEnvFn func(name string) string, args ArgWorkPathAndCustomConf) {
	...
	readFromEnv := func() {
		envWorkPath := getEnvFn("SHIPYARD_WORK_DIR")
		if envWorkPath != "" {
			tmpWorkPath.Set(envWorkPath)
			if !filepath.IsAbs(tmpWorkPath.Value) {
				log.Fatal("SHIPYARD_WORK_DIR (work path) must be absolute path")
			}
		}

		envCustomPath := getEnvFn("SHIPYARD_CUSTOM")
		if envCustomPath != "" {
			tmpCustomPath.Set(envCustomPath)
			if !filepath.IsAbs(tmpCustomPath.Value) {
				log.Fatal("SHIPYARD_CUSTOM (custom path) must be absolute path")
			}
		}
	}
	...
	if configWorkPath != "" {
		...
		if tmpWorkPath.Value != "" && (getEnvFn("SHIPYARD_WORK_DIR") != "" || args.WorkPath != "") {
