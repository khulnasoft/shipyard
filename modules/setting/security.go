...
var (
	...
	OnlyAllowPushIfShipyardEnvironmentSet bool
...
func loadSecurityFrom(rootCfg ConfigProvider) {
	...
	if SecretKey == "" {
		// FIXME: https://github.com/go-shipyard/shipyard/issues/16832
		// Until it supports rotating an existing secret key, we shouldn't move users off of the widely used default value
		SecretKey = "!#@FDEWREWR&*(" //nolint:gosec
	}

	CookieRememberName = sec.Key("COOKIE_REMEMBER_NAME").MustString("shipyard_incredible")
	...
	OnlyAllowPushIfShipyardEnvironmentSet = sec.Key("ONLY_ALLOW_PUSH_IF_SHIPYARD_ENVIRONMENT_SET").MustBool(true)
	...
	if InstallLock && InternalToken == "" {
		// if Shipyard has been installed but the InternalToken hasn't been generated (upgrade from an old release), we should generate
	...
	if !DisableQueryAuthToken {
		log.Warn("Enabling Query API Auth tokens is not recommended. DISABLE_QUERY_AUTH_TOKEN will default to true in shipyard 1.23 and will be removed in shipyard 1.24.")
	}
}
