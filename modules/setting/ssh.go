...
var SSH = struct {
	...
	ServerHostKeys:                []string{"ssh/shipyard.rsa", "ssh/gogs.rsa"},
...
func loadSSHFrom(rootCfg ConfigProvider) {
	...
	SSH.TrustedUserCAKeysFile = sec.Key("SSH_TRUSTED_USER_CA_KEYS_FILENAME").MustString(filepath.Join(SSH.RootPath, "shipyard-trusted-user-ca-keys.pem"))
