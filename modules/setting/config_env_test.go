...
func TestDecodeEnvironmentKey(t *testing.T) {
	...
	ok, section, key, file = decodeEnvironmentKey(prefix, suffix, "SHIPYARD__SEC")
	assert.False(t, ok)
	assert.Equal(t, "", section)
	assert.Equal(t, "", key)
	assert.False(t, file)

	ok, section, key, file = decodeEnvironmentKey(prefix, suffix, "SHIPYARD____KEY")
	assert.True(t, ok)
	assert.Equal(t, "", section)
	assert.Equal(t, "KEY", key)
	assert.False(t, file)

	ok, section, key, file = decodeEnvironmentKey(prefix, suffix, "SHIPYARD__SEC__KEY")
	assert.True(t, ok)
	assert.Equal(t, "sec", section)
	assert.Equal(t, "KEY", key)
	assert.False(t, file)

	// with "__FILE" suffix, it doesn't support to write "[sec].FILE" to config (no such key FILE is used in Shipyard)
	// but it could be fixed in the future by adding a new suffix like "__VALUE" (no such key VALUE is used in Shipyard either)
	ok, section, key, file = decodeEnvironmentKey(prefix, suffix, "SHIPYARD__SEC__FILE")
	assert.False(t, ok)
	assert.Equal(t, "", section)
	assert.Equal(t, "", key)
	assert.True(t, file)

	ok, section, key, file = decodeEnvironmentKey(prefix, suffix, "SHIPYARD__SEC__KEY__FILE")
	assert.True(t, ok)
	assert.Equal(t, "sec", section)
	assert.Equal(t, "KEY", key)
	assert.True(t, file)
...
`)
	...
	changed = EnvironmentToConfig(cfg, []string{"SHIPYARD__sec__key=new"})
	...
	changed = EnvironmentToConfig(cfg, []string{"SHIPYARD__sec__key__FILE=" + tmpFile})
	assert.True(t, changed)
	assert.Equal(t, "value-from-file", cfg.Section("sec").Key("key").String())

	cfg, _ = NewConfigProviderFromData("")
	_ = os.WriteFile(tmpFile, []byte("value-from-file\n"), 0o644)
	EnvironmentToConfig(cfg, []string{"SHIPYARD__sec__key__FILE=" + tmpFile})
	assert.Equal(t, "value-from-file", cfg.Section("sec").Key("key").String())
...
`)
	...
	changed := EnvironmentToConfig(cfg, []string{"SHIPYARD__sec_0X2E_sub__key=some"})
	assert.True(t, changed)
