...
const (
	EnvConfigKeyPrefixShipyard = "SHIPYARD__"
...
func CollectEnvConfigKeys() (keys []string) {
	for _, env := range os.Environ() {
		if strings.HasPrefix(env, EnvConfigKeyPrefixShipyard) {
...
// The environment key is in the form of SHIPYARD__SECTION__KEY or SHIPYARD__SECTION__KEY__FILE
func decodeEnvironmentKey(prefixShipyard, suffixFile, envKey string) (ok bool, section, key string, useFileValue bool) {
	if !strings.HasPrefix(envKey, prefixShipyard) {
		return false, "", "", false
	}
	...
	ok, section, key = decodeEnvSectionKey(envKey[len(prefixShipyard):])
...
func EnvironmentToConfig(cfg ConfigProvider, envs []string) (changed bool) {
	for _, kv := range envs {
		...
		envKey := kv[:idx]
		envValue := kv[idx+1:]
		ok, sectionName, keyName, useFileValue := decodeEnvironmentKey(EnvConfigKeyPrefixShipyard, EnvConfigKeySuffixFile, envKey)
		if !ok {
			continue
		}
