...
// settings
var (
	// AppVer is the version of the current build of Shipyard. It is set in main.go from main.Version.
	AppVer string
	// AppBuiltWith represents a human-readable version go runtime build version and build tags. (See main.go formatBuiltWith().)
	AppBuiltWith string
	// AppStartTime store time shipyard has started
...
func PrepareAppDataPath() error {
	...
	// For example, if someDir=/mnt/vol1/shipyard-home/data, if the mount point /mnt/vol1 is not mounted when Shipyard runs,
	// then shipyard will make new empty directories in /mnt/vol1, all are stored in the root filesystem.
...
func MustInstalled() {
	if !InstallLock {
		log.Fatal(`Unable to load config file for a installed Shipyard instance, you should either use "--config" to set your config file (app.ini), or run "shipyard web" command to install Shipyard.`)
	}
}
...
func loadRunModeFrom(rootCfg ConfigProvider) {
	...
	// The following is a purposefully undocumented option. Please do not run Shipyard as root. It will only cause future headaches.
	// Please don't use root as a bandaid to "fix" something that is broken, instead the broken thing should instead be fixed properly.
	unsafeAllowRunAsRoot := ConfigSectionKeyBool(rootSec, "I_AM_BEING_UNSAFE_RUNNING_AS_ROOT")
	RunMode = os.Getenv("SHIPYARD_RUN_MODE")
	if RunMode == "" {
	...
	if os.Getuid() == 0 {
		if !unsafeAllowRunAsRoot {
			...
			log.Fatal("Shipyard is not supposed to be run as root. Sorry. If you need to use privileged TCP ports please instead use setcap and the `cap_net_bind_service` permission")
		}
		log.Critical("You are running Shipyard using the root user, and have purposely chosen to skip built-in protections around this. You have been warned against this.")
	}
}
