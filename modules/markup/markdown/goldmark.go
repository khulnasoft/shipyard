...
import (
	...
	shipyardutil "github.com/khulnasoft/shipyard/modules/util"
	...
	_ = ast.Walk(node, func(n ast.Node, entering bool) (ast.WalkStatus, error) {
		...
		case *ast.Image:
			...
			if len(link) > 0 && !markup.IsLink(link) {
				...
				if pc.Get(isWikiKey).(bool) {
					prefix = shipyardutil.URLJoin(prefix, "wiki", "raw")
				}
				prefix = strings.Replace(prefix, "/src/", "/media/", 1)

				lnk := strings.TrimLeft(string(link), "/")

				lnk = shipyardutil.URLJoin(prefix, lnk)
				link = []byte(lnk)
			}
			v.Destination = link
		...
		case *ast.Link:
			...
			if len(link) > 0 && !markup.IsLink(link) &&
				...
				lnk := string(link)
				if pc.Get(isWikiKey).(bool) {
					lnk = shipyardutil.URLJoin("wiki", lnk)
				}
				link = []byte(shipyardutil.URLJoin(pc.Get(urlPrefixKey).(string), lnk))
			}
...
// renders shipyard specific features.
