...
func (p *Renderer) Render(ctx *markup.RenderContext, input io.Reader, output io.Writer) error {
	var (
		...
		command      = strings.NewReplacer(envMark("SHIPYARD_PREFIX_SRC"), ctx.URLPrefix,
			envMark("SHIPYARD_PREFIX_RAW"), urlRawPrefix).Replace(p.Command)
	...
	if p.IsInputFile {
		// write to temp file
		f, err := os.CreateTemp("", "shipyard_input")
		if err != nil {
	...
	cmd.Env = append(
		os.Environ(),
		"SHIPYARD_PREFIX_SRC="+ctx.URLPrefix,
		"SHIPYARD_PREFIX_RAW="+urlRawPrefix,
	)
	if !p.IsInputFile {
		cmd.Stdin = input
	}
