...
func checkDBConsistency(ctx context.Context, logger log.Logger, autofix bool) error {
	...
	if err := db.InitEngineWithMigration(ctx, migrations.EnsureUpToDate); err != nil {
		logger.Critical("Model version on the database does not match the current Shipyard version. Model consistency will not be checked until the database is upgraded")
		return err
	}
