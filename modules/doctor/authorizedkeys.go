...
const tplCommentPrefix = `# shipyard public key`
	...
	for scanner.Scan() {
		...
		if !autofix {
			logger.Critical(
				...
				"shipyard doctor --run authorized-keys --fix")
			return fmt.Errorf(`authorized_keys is out of date and should be regenerated with "shipyard admin regenerate keys" or "shipyard doctor --run authorized-keys --fix"`)
		}
