...
// requestJSONResp sends a request to the shipyard server and then parses the response.
...
func requestJSONResp[T any](req *httplib.Request, res *T) (ret *T, extra ResponseExtra) {
	...
	if err != nil {
		...
		extra.Error = fmt.Errorf("unable to contact shipyard %q: %w", req.GoString(), err)
		return nil, extra
	}
...
// requestJSONClientMsg sends a request to the shipyard server, server only responds text message status=200 with "success" body
