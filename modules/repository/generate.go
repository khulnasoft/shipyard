...
// ShipyardTemplate holds information about a .shipyard/template file
type ShipyardTemplate struct {
	Path    string
	Content []byte
...
// Globs parses the .shipyard/template globs or returns them if they were already parsed
func (gt ShipyardTemplate) Globs() []glob.Glob {
	if gt.globs != nil {
		return gt.globs
	}
...
func checkShipyardTemplate(tmpDir string) (*ShipyardTemplate, error) {
	gtPath := filepath.Join(tmpDir, ".shipyard", "template")
...
func generateRepoCommit(ctx context.Context, repo, templateRepo, generateRepo *repo_model.Repository, tmpDir string) error {
	...
	// Variable expansion
	gt, err := checkShipyardTemplate(tmpDir)
	if err != nil {
		return fmt.Errorf("checkShipyardTemplate: %w", err)
	}

	if gt != nil {
		if err := util.Remove(gt.Path); err != nil {
			return fmt.Errorf("remove .shipyardtemplate: %w", err)
		}
...
func generateGitContent(ctx context.Context, repo, templateRepo, generateRepo *repo_model.Repository) (err error) {
	tmpDir, err := os.MkdirTemp(os.TempDir(), "shipyard-"+repo.Name)
	if err != nil {
