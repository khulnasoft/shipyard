...
func getHookTemplates() (hookNames, hookTpls, shipyardHookTpls []string) {
	hookNames = []string{"pre-receive", "update", "post-receive"}
	hookTpls = []string{
		// for pre-receive
		fmt.Sprintf(`#!/usr/bin/env %s
# AUTO GENERATED BY SHIPYARD, DO NOT MODIFY
data=$(cat)
exitcodes=""
hookname=$(basename $0)
...
`, setting.ScriptType, util.ShellEscape(setting.AppPath), util.ShellEscape(setting.CustomConf)))
		shipyardHookTpls = append(shipyardHookTpls, "")
	}

	return hookNames, hookTpls, shipyardHookTpls
}

// CreateDelegateHooks creates all the hooks scripts for the repo
func CreateDelegateHooks(repoPath string) (err error) {
	hookNames, hookTpls, shipyardHookTpls := getHookTemplates()
	hookDir := filepath.Join(repoPath, "hooks")

	for i, hookName := range hookNames {
		oldHookPath := filepath.Join(hookDir, hookName)
		newHookPath := filepath.Join(hookDir, hookName+".d", "shipyard")
		...
		if err = os.WriteFile(newHookPath, []byte(shipyardHookTpls[i]), 0o777); err != nil {
...
func CheckDelegateHooks(repoPath string) ([]string, error) {
	...
	for i, hookName := range hookNames {
		...
		if string(contents) != shipyardHookTpls[i] {
