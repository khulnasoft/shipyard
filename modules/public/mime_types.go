...
// If the user's OS has incorrect mime-type config, it would make Shipyard can not respond a correct Content-Type to browsers.
// For example, if Shipyard returns `text/plain` for a `.js` file, the browser couldn't run the JS due to security reasons.
