...
func main() {
	...
	app.Description = `Backport will look-up the PR in Shipyard's git log and attempt to cherry-pick it on the current version`
	...
	app.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			Name:  "upstream",
			Value: "origin",
			Usage: "Upstream remote for the Shipyard upstream",
		},
		&cli.StringFlag{
			Name:  "release-branch",
			Value: "",
		...
		&cli.StringFlag{
			Name:  "remote",
			Value: "",
			Usage: "Remote for your fork of the Shipyard upstream",
		},
		&cli.StringFlag{
			Name:  "fork-user",
			Value: "",
...
func runBackport(c *cli.Context) error {
	...
	if !c.Bool("no-push") {
		url := "https://github.com/go-shipyard/shipyard.khulnasoft.compare/" + upstreamReleaseBranch + "..." + forkUser + ":" + backportBranch
...
func determineRemote(ctx context.Context, forkUser string) (string, string, error) {
	...
	for _, line := range lines {
		...
		// ignore go-shipyard/shipyard
		if strings.Contains(remote, "go-shipyard/shipyard") {
			continue
		}
...
func determineSHAforPR(ctx context.Context, prStr string) (string, error) {
	...
	pr, _, err := client.PullRequests.Get(ctx, "go-shipyard", "shipyard", prNum)
	if err != nil {
		return "", err
	}
