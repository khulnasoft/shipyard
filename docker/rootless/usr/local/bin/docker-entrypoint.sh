...
if [ ! -x /bin/sh ]; then
  echo "Executable test for /bin/sh failed. Your Docker version is too old to run Alpine 3.14+ and Shipyard. You must upgrade Docker.";
  exit 1;
fi
...
if [ $# -gt 0 ]; then
    ...
    exec /usr/local/bin/shipyard -c ${SHIPYARD_APP_INI} web
fi
