...
from shipyard.packages import InstallTarget, Package
from shipyard.packages.api import NoSuchPackageException, PackagesPluginManager
...
def cli():
    """
    The Shipyard Package Manager (lpm) CLI is a set of commands to install third-party packages used by shipyard
    service providers.
    ...
    List all packages
        ...
        python -m shipyard.cli.lpm list

    Install DynamoDB Local:

        python -m shipyard.cli.install dynamodb-local

    Install all community packages, four in parallel:

        python -m shipyard.cli.lpm list | grep "/community" | cut -d'/' -f1 | xargs python -m shipyard.cli.lpm install --parallel 4
    """
    setup_logging()
