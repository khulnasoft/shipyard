...
from shipyard.testing.aws.cloudformation_utils import load_template_file, render_template
from shipyard.testing.aws.util import get_lambda_logs, is_aws_cloud
from shipyard.utils import testutil
from shipyard.utils.aws.client import SigningHttpClient
from shipyard.utils.aws.resources import create_dynamodb_table
from shipyard.utils.bootstrap import is_api_enabled
from shipyard.utils.collections import ensure_list
from shipyard.utils.functions import run_safe
from shipyard.utils.http import safe_requests as requests
from shipyard.utils.json import CustomEncoder, json_safe
from shipyard.utils.net import wait_for_port_open
from shipyard.utils.strings import short_uid, to_str
from shipyard.utils.sync import ShortCircuitWaitException, poll_condition, retry, wait_until
...
def kms_create_grant(kms_create_key, aws_client):
    ...
    def _create_grant(**kwargs):
        # Just a random ARN, since KMS in Shipyard currently doesn't validate GranteePrincipal,
    ...
    def _deploy(
        *,
        ...
        def _destroy_stack():
            ...
            # TODO: fix in shipyard. stack should only be in DELETE_COMPLETE state after all resources have been deleted
            time.sleep(2)
...
def create_lambda_function_aws(aws_client):
    ...
    def _create_lambda_function(**kwargs):
        ...
        # shipyard should normally not require the retries and will just continue here
...
#
# We would like to have shipyard tests to pass not just against shipyard, but also against AWS to make sure
...
def acm_request_certificate(aws_client_factory):
    ...
    def factory(**kwargs) -> str:
        if "DomainName" not in kwargs:
            kwargs["DomainName"] = f"test-domain-{short_uid()}.localhost.shipyard.khulnasoft.com"
...
def pytest_collection_modifyitems(config: Config, items: list[Item]):
    only_shipyard = pytest.mark.skipif(
        os.environ.get("TEST_TARGET") == "AWS_CLOUD",
        reason="test only applicable if run against shipyard",
    )
    for item in items:
        for mark in item.iter_markers():
            if mark.name.endswith("only_shipyard"):
                item.add_marker(only_shipyard)
...
def assert_host_customisation(monkeypatch):
    shipyard_host = "foo.bar"
    monkeypatch.setattr(
        config, "SHIPYARD_HOST", config.HostAndPort(host=shipyard_host, port=8888)
    )
    ...
    def asserter(
        ...
        *,
        ...
        if custom_host is not None:
            ...
            assert shipyard_host not in url
        else:
            assert shipyard_host in url, f"Could not find `{shipyard_host}` in `{url}`"
