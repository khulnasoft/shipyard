...
from shipyard.testing.pytest.fixtures import StackDeployError
...
from shipyard.aws.api.cloudformation import Capability
...
class InfraProvisioner:
    ...
    def get_asset_bucket(self):
        ...
        return f"shipyard-testing-{account}-{region}"
    ...
    def provision(self, skip_deployment: Optional[bool] = False):
        ...
        if all(
            ...
            # TODO: in shipyard we might want to do a delete/create
            #  but generally this won't be a common use case when developing against Shipyard
            is_update = True
    ...
    def teardown(self):
        ...
        if not is_aws_cloud():
            # TODO: also clean up s3 bucket on shipyard?
    ...
    def _template_bucket_name(self):
        ...
        region = self.aws_client.sts.meta.region_name
        return f"shipyard-testing-assets-{account_id}-{region}"
    ...
    def get_asset_bucket_cdk(stack: cdk.Stack):
        return cdk.Fn.join("-", ["shipyard", "testing", stack.account, stack.region])
