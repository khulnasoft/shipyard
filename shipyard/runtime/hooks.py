...
HOOKS_CONFIGURE_SHIPYARD_CONTAINER = "shipyard.hooks.configure_shipyard_container"
HOOKS_ON_INFRA_READY = "shipyard.hooks.on_infra_ready"
HOOKS_ON_INFRA_START = "shipyard.hooks.on_infra_start"
HOOKS_ON_PRO_INFRA_START = "shipyard.hooks.on_pro_infra_start"
HOOKS_ON_INFRA_SHUTDOWN = "shipyard.hooks.on_infra_shutdown"
HOOKS_PREPARE_HOST = "shipyard.hooks.prepare_host"
...
def hook_spec(namespace: str):
    ...
    on_infra_start = hook_spec("shipyard.hooks.on_infra_start")
...
configure_shipyard_container = hook_spec(HOOKS_CONFIGURE_SHIPYARD_CONTAINER)
"""Hooks to configure the Shipyard container before it starts. Executed on the host when invoking the CLI."""

prepare_host = hook_spec(HOOKS_PREPARE_HOST)
"""Hooks to prepare the host that's starting Shipyard. Executed on the host when invoking the CLI."""

on_infra_start = hook_spec(HOOKS_ON_INFRA_START)
"""Hooks that are executed right before starting the Shipyard infrastructure."""

on_pro_infra_start = hook_spec(HOOKS_ON_PRO_INFRA_START)
"""Hooks that are executed after on_infra_start hooks, and only if Shipyard pro has been activated."""

on_infra_ready = hook_spec(HOOKS_ON_INFRA_READY)
"""Hooks that are execute after all startup hooks have been executed, and the Shipyard infrastructure has become
available."""

on_infra_shutdown = hook_spec(HOOKS_ON_INFRA_SHUTDOWN)
"""Hooks that are execute when shipyard shuts down."""
