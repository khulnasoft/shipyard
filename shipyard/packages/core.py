...
from ..constants import SHIPYARD_VENV_FOLDER
...
class ArchiveDownloadAndExtractInstaller(ExecutableInstaller):
    ...
    def get_installed_dir(self) -> str | None:
        ...
        # f.e. /var/lib/shipyard/lib/amazon-mq/5.16.5/apache-activemq-5.16.5/
...
class NodePackageInstaller(ExecutableInstaller):
    ...
    def _install(self, target: InstallTarget) -> None:
        ...
        # - https://github.com/khulnasoft/shipyard/issues/7620
        if is_root():
...
SHIPYARD_VENV = VirtualEnvironment(SHIPYARD_VENV_FOLDER)
...
class PythonPackageInstaller(PackageInstaller):
    ...
    def _prepare_installation(self, target: InstallTarget) -> None:
        ...
        if not venv.exists:
            ...
            venv.create()
            LOG.info("adding shipyard venv path %s", venv.venv_dir)
            venv.add_pth("shipyard-venv", SHIPYARD_VENV)
