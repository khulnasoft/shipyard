...
class DnsServerProtocol(Protocol):
    ...
    def add_host_pointing_to_shipyard(self, name: str) -> None:
        """
        Add a dns name which should be pointing to Shipyard when resolved.

        :param name: Name which should be pointing to Shipyard when resolved
        """
        pass

    def delete_host_pointing_to_shipyard(self, name: str) -> None:
        """
        Removes a dns name from pointing to Shipyard

        :param name: Name to be removed
        :raises ValueError: If the host pointing to Shipyard was not previously registered using `add_host_pointing_to_shipyard`
        """
        pass
    ...
    # has standard list of skips: shipyard.services.dns_server.SKIP_PATTERNS
