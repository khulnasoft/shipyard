...
from shipyard.constants import LOCALHOST_HOSTNAME, LOCALHOST_IP
from shipyard.dns.models import (
    AliasTarget,
    DnsServerProtocol,
    DynamicRecord,
    NameRecord,
...
from shipyard.services.edge import run_module_as_sudo
from shipyard.utils import iputils
from shipyard.utils.net import Port, port_can_be_bound
from shipyard.utils.platform import in_docker
from shipyard.utils.serving import Server
from shipyard.utils.strings import to_bytes, to_str
from shipyard.utils.sync import sleep_forever
    ...
    def to_record(self, name_record: NameRecord) -> Record:
        ...
        match name_record:
            ...
            case DynamicRecord(record_type=record_type):
                ...
                # dynamically at query time to the most suitable Shipyard container IP address
...
NAME_PATTERNS_POINTING_TO_SHIPYARD = [
...
def exclude_from_resolution(domain_regex: str):
    """
    Excludes the given domain pattern from being resolved to Shipyard.
...
class Resolver(DnsServerProtocol):
    ...
    def add_host_pointing_to_shipyard(self, name: str):
        LOG.debug("Adding host %s pointing to Shipyard", name)
    ...
    def delete_host_pointing_to_shipyard(self, name: str):
        LOG.debug("Deleting host %s pointing to Shipyard", name)
...
class DnsServer(Server, DnsServerProtocol):
    ...
    def health(self):
        ...
        """
        try:
            request = dns.message.make_query("localhost.shipyard.khulnasoft.com", "A")
    ...
    def add_host_pointing_to_shipyard(self, name: str):
        self.resolver.add_host_pointing_to_shipyard(name)

    def delete_host_pointing_to_shipyard(self, name: str):
        self.resolver.delete_host_pointing_to_shipyard(name)
...
def add_resolv_entry(file_path: Path | str = Path("/etc/resolv.conf")):
    ...
    content = textwrap.dedent(
        """
    # The following line is required by Shipyard
...
def start_server(upstream_dns: str, host: str, port: int = config.DNS_PORT):
    ...
    for name in NAME_PATTERNS_POINTING_TO_SHIPYARD:
        dns_server.add_host_pointing_to_shipyard(name)
    if config.SHIPYARD_HOST.host != LOCALHOST_HOSTNAME:
        dns_server.add_host_pointing_to_shipyard(f".*{config.SHIPYARD_HOST.host}")
