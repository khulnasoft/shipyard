...
class Directories:
    """
    Holds the different directories available to shipyard. Some directories are shared between the host and the
    shipyard container, some live only on the host and some only in the container.
    ...
    Attributes:
        ...
        cache:       shared; ephemeral data that has to persist across shipyard runs and reboots
        tmp:         container only; ephemeral data that has to persist across shipyard runs but not reboots
        ...
        data:        shared; holds shipyard state, pods, ...
        ...
        logs:        shared; log files produced by shipyard
    """
    ...
    def defaults() -> "Directories":
        """Returns Shipyard directory paths based on the shipyard filesystem hierarchy."""
        return Directories(
            static_libs="/usr/lib/shipyard",
            var_libs=f"{DEFAULT_VOLUME_DIR}/lib",
            cache=f"{DEFAULT_VOLUME_DIR}/cache",
            tmp=os.path.join(tempfile.gettempdir(), "shipyard"),
            ...
            config="/etc/shipyard/conf.d",  # for future use
            init="/etc/shipyard/init",
        )

    @staticmethod
    def for_container() -> "Directories":
        """
        Returns Shipyard directory paths as they are defined within the container. Everything shared and writable
        lives in /var/lib/shipyard or {tempfile.gettempdir()}/shipyard.
    ...
    def for_host() -> "Directories":
        """Return directories used for running shipyard in host mode. Note that these are *not* the directories
        that are mounted into the container when the user starts shipyard."""
        root = os.environ.get("FILESYSTEM_ROOT") or os.path.join(
            SHIPYARD_ROOT_FOLDER, ".filesystem"
        )
    ...
    def for_cli() -> "Directories":
        """Returns directories used for when running shipyard CLI commands from the host system. Unlike
        ``for_container``, these here need to be cross-platform. Ideally, this should not be needed at all,
        because the shipyard runtime and CLI do not share any control paths. There are a handful of
        ...
        from shipyard.utils import files

        tmp_dir = os.path.join(tempfile.gettempdir(), "shipyard-cli")
        cache_dir = (files.get_user_cache_dir()).absolute() / "shipyard-cli"
        ...
        return Directories(
            ...
            mounted_tmp=tmp_dir,
            functions=None,
            data=os.path.join(tmp_dir, "state"),  # used by shipyard_ext config TODO: remove
...
def load_environment(profiles: str = None, env=os.environ) -> List[str]:
    """Loads the environment variables from ~/.shipyard/{profile}.env, for each profile listed in the profiles.
...
def in_docker():
    ...
    for path in [
        "/usr/lib/shipyard/.community-version",
        "/usr/lib/shipyard/.pro-version",
        "/tmp/shipyard/.marker",
    ]:
        if os.path.isfile(path):
            return True

    # details: https://github.com/khulnasoft/shipyard/pull/4352
...
CONFIG_DIR = os.environ.get("CONFIG_DIR", os.path.expanduser("~/.shipyard"))
...
# whether shipyard should persist service state across shipyard runs
...
TMP_FOLDER = os.path.join(tempfile.gettempdir(), "shipyard")

# this is exclusively for the CLI to configure the container mount into /var/lib/shipyard
VOLUME_DIR = os.environ.get("SHIPYARD_VOLUME_DIR", "").strip() or TMP_FOLDER
...
if DEBUG:
    ...
    logging.getLogger("shipyard").setLevel(logging.DEBUG)
...
def populate_edge_configuration(
    ...
    """Populate the Shipyard edge configuration from environment variables."""
    shipyard_host_raw = environment.get("SHIPYARD_HOST")
    ...
    # the actual value of the SHIPYARD_HOST port now depends on what gateway listen actually listens to.
    if shipyard_host_raw is None:
        shipyard_host = HostAndPort(
            host=constants.LOCALHOST_HOSTNAME, port=gateway_listen[0].port
        )
    else:
        shipyard_host = HostAndPort.parse(
            shipyard_host_raw,
    ...
    assert shipyard_host is not None
...
MAIN_CONTAINER_NAME = os.environ.get("MAIN_CONTAINER_NAME", "").strip() or "shipyard-main"

# the latest commit id of the repository when the docker image was created
SHIPYARD_BUILD_GIT_HASH = os.environ.get("SHIPYARD_BUILD_GIT_HASH", "").strip() or None

# the date on which the docker image was created
SHIPYARD_BUILD_DATE = os.environ.get("SHIPYARD_BUILD_DATE", "").strip() or None
...
# PUBLIC: Endpoint host under which Shipyard APIs are accessible from Lambda Docker containers.
...
# PUBLIC v1: Shipyard DNS (default)
...
# shipyard/services/lambda_/invocation/lambda_models.py:IMAGE_MAPPING
...
# DEV: Release version of https://github.com/khulnasoft/lambda-runtime-init overriding the current default
...
# URL to a custom OpenSearch/Elasticsearch backend cluster. If this is set to a valid URL, then shipyard will not
...
# How shipyard will react to encountering unsupported resource types.
...
# hostnames resolve to 127.0.0.1, which allows to use the Shipyard APIs transparently
...
DNS_VERIFICATION_DOMAIN = os.environ.get("DNS_VERIFICATION_DOMAIN") or "shipyard.khulnasoft.com"
...
S3_VIRTUAL_HOSTNAME = "s3.%s" % SHIPYARD_HOST.host
S3_STATIC_WEBSITE_HOSTNAME = "s3-website.%s" % SHIPYARD_HOST.host
...
# List of environment variable names used for configuration that are passed from the host into the Shipyard container.
# => Synchronize this list with the above and the configuration docs:
# https://docs.shipyard.khulnasoft.com/references/configuration/
...
CONFIG_ENV_VARS = [
    ...
    "SHIPYARD_HOSTNAME",  # deprecated since 2.0.0
...
def collect_config_items() -> List[Tuple[str, Any]]:
    """Returns a list of key-value tuples of Shipyard configuration values."""
...
def populate_config_env_var_names():
    ...
    CONFIG_ENV_VARS += [
        ...
        if (key.startswith("SHIPYARD_") or key.startswith("PROVIDER_OVERRIDE_"))
        # explicitly exclude SHIPYARD_CLI (it's prefixed with "SHIPYARD_",
        # but is only used in the CLI (should not be forwarded to the container)
        and key != "SHIPYARD_CLI"
    ]

    # create variable aliases prefixed with SHIPYARD_ (except SHIPYARD_HOST)
    CONFIG_ENV_VARS += [
        "SHIPYARD_" + v for v in CONFIG_ENV_VARS if not v.startswith("SHIPYARD_")
    ]
...
def external_service_url(
    ...
    machine than Shipyard. The configurations SHIPYARD_HOST and USE_SSL can customize these returned URLs.
    The optional parameters can be used to customize the defaults.
    Examples with default configuration:
    * external_service_url() == http://localhost.shipyard.khulnasoft.com:4566
    * external_service_url(subdomains="s3") == http://s3.localhost.shipyard.khulnasoft.com:4566
    """
    protocol = protocol or get_protocol()
    subdomains = f"{subdomains}." if subdomains else ""
    host = host or SHIPYARD_HOST.host
    port = port or SHIPYARD_HOST.port
...
def internal_service_url(
    ...
    """Returns a service URL for internal use within Shipyard (i.e., same host).
    The configuration USE_SSL can customize these returned URLs but SHIPYARD_HOST has no effect.
    The optional parameters can be used to customize the defaults.
    Examples with default configuration:
...
def service_port(service_key: str, external: bool = False) -> int:
    """@deprecated: Use `shipyard_host().port` for external and `GATEWAY_LISTEN[0].port` for
    internal use."""
    warnings.warn(
        "Deprecated: use `shipyard_host().port` for external and `GATEWAY_LISTEN[0].port` for "
        "internal use.",
...
def get_edge_port_http():
    ...
    and HTTP ports anymore since Shipyard listens to both."""
    warnings.warn(
        """@deprecated: Use `shipyard_host().port` for external and `GATEWAY_LISTEN[0].port`
        for internal use. This function is also not needed anymore because we don't separate
        between HTTP and HTTP ports anymore since Shipyard listens to both.""",
        DeprecationWarning,
    )
    return GATEWAY_LISTEN[0].port


def get_edge_url(shipyard_hostname=None, protocol=None):
    ...
    return internal_service_url(host=shipyard_hostname, protocol=protocol)
