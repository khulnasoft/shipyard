...
HEADER_SHIPYARD_EDGE_URL = "x-shipyard-edge"
HEADER_SHIPYARD_REQUEST_URL = "x-shipyard-request-url"
# xXx custom shipyard authorization header only used in ext
HEADER_SHIPYARD_AUTHORIZATION = "x-shipyard-authorization"
HEADER_SHIPYARD_TARGET = "x-shipyard-target"
...
LOCALHOST_HOSTNAME = "localhost.shipyard.khulnasoft.com"

# version of the Maven dependency with Java utility code
SHIPYARD_MAVEN_VERSION = "0.2.21"
MAVEN_REPO_URL = "https://repo1.maven.org/maven2"

# URL of shipyard's artifacts repository on GitHub
ARTIFACTS_REPO = "https://github.com/khulnasoft/shipyard-artifacts"

# Download URLs
SSL_CERT_URL = f"{ARTIFACTS_REPO}/raw/master/local-certs/server.key"
SSL_CERT_URL_FALLBACK = "{api_endpoint}/proxy/shipyard.cert.key"
...
SHIPYARD_ROOT_FOLDER = os.path.realpath(os.path.join(MODULE_MAIN_PATH, ".."))

# virtualenv folder
SHIPYARD_VENV_FOLDER = os.environ.get("VIRTUAL_ENV")
if not SHIPYARD_VENV_FOLDER:
    # fallback to the previous logic
    SHIPYARD_VENV_FOLDER = os.path.join(SHIPYARD_ROOT_FOLDER, ".venv")
    if not os.path.isdir(SHIPYARD_VENV_FOLDER):
        # assuming this package lives here: <python>/lib/pythonX.X/site-packages/shipyard/
        SHIPYARD_VENV_FOLDER = os.path.realpath(
            os.path.join(SHIPYARD_ROOT_FOLDER, "..", "..", "..")
        )

# default volume directory containing shared data
DEFAULT_VOLUME_DIR = "/var/lib/shipyard"

# API Gateway path to indicate a user request sent to the gateway
PATH_USER_REQUEST = "_user_request_"

# name of Shipyard Docker image
DOCKER_IMAGE_NAME = "khulnasoft/shipyard"
DOCKER_IMAGE_NAME_PRO = "khulnasoft/shipyard-pro"
DOCKER_IMAGE_NAME_FULL = "khulnasoft/shipyard-full"

# backdoor API path used to retrieve or update config variables
CONFIG_UPDATE_PATH = "/?_config_"

# API path for shipyard internal resources
INTERNAL_RESOURCE_PATH = "/_shipyard"

# environment variable name to tag local test runs
ENV_INTERNAL_TEST_RUN = "SHIPYARD_INTERNAL_TEST_RUN"

# environment variable name to tag collect metrics during a test run
ENV_INTERNAL_TEST_COLLECT_METRIC = "SHIPYARD_INTERNAL_TEST_COLLECT_METRIC"
...
API_ENDPOINT = os.environ.get("API_ENDPOINT") or "https://api.shipyard.khulnasoft.com/v1"
# new analytics API endpoint
ANALYTICS_API = os.environ.get("ANALYTICS_API") or "https://analytics.shipyard.khulnasoft.com/v1"

# environment variable to indicates this process should run the shipyard infrastructure
SHIPYARD_INFRA_PROCESS = "SHIPYARD_INFRA_PROCESS"
