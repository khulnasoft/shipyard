...
from shipyard.aws.handlers.proxy import ProxyHandler
from shipyard.aws.serving.asgi import AsgiGateway
from shipyard.config import HostAndPort
from shipyard.logging.setup import setup_hypercorn_logger
...
class GatewayServer(HypercornServer):
    ...
    def __init__(
        ...
        :param use_ssl: True if the Shipyard cert should be loaded and HTTP/HTTPS multiplexing should be enabled.
        """
        # build server config
        config = Config()
...
class ProxyServer(GatewayServer):
    """
    Proxy server implementation which uses the shipyard.http.proxy module.
