"""
Several ContainerConfigurator implementations to set up a development version of a shipyard container.
"""
import gzip
import os
...
from shipyard.utils.files import get_user_cache_dir
from shipyard.utils.run import run
from shipyard.utils.strings import md5

from .paths import CommunityContainerPaths, ContainerPaths, HostPaths, ProContainerPaths


class ConfigEnvironmentConfigurator:
    """Configures the environment variables from the shipyard and shipyard_ext config."""
    ...
    def __call__(self, cfg: ContainerConfiguration):
        ...
        if self.pro:
            # import shipyard_ext config extends the list of config vars
            from shipyard_ext import config as config_ext  # noqa
...
class PortConfigurator:
    """
    Configures the port mappings. Can be randomized to run multiple shipyard instances.
    """
...
class ImageConfigurator:
    """
    Sets the container image to use for the container (by default either khulnasoft/shipyard or
...
class SourceVolumeMountConfigurator:
    """
    Mounts source code of shipyard, localsack_ext, and moto into the container. It does this by assuming
    that there is a "workspace" directory in which the source repositories are checked out into.
    Depending on whether we want to start the pro container, the source paths for shipyard are different.
    """
    ...
    def __call__(self, cfg: ContainerConfiguration):
        # shipyard source code if available
        source = self.host_paths.shipyard_project_dir / "shipyard"
        if source.exists():
            cfg.volumes.add(
                VolumeBind(str(source), self.container_paths.shipyard_source_dir, read_only=True)
            )

        # ext source code if available
        if self.pro:
            source = self.host_paths.shipyard_ext_project_dir / "shipyard_ext"
            if source.exists():
                cfg.volumes.add(
                    VolumeBind(
                        str(source), self.container_paths.shipyard_ext_source_dir, read_only=True
                    )
                )
        ...
        # docker entrypoint
        if self.pro:
            source = self.host_paths.shipyard_ext_project_dir / "bin" / "docker-entrypoint.sh"
        else:
            source = self.host_paths.shipyard_project_dir / "bin" / "docker-entrypoint.sh"
        if source.exists():
            cfg.volumes.add(
...
class CoverageRunScriptConfigurator:
    """
    Adds the coverage-run.py script as read-only volume mount into /opt/code/shipyard/bin/coverage-run.py
    """
    ...
    def __call__(self, cfg: ContainerConfiguration):
        # coverage script
        source = self.host_paths.shipyard_ext_project_dir / "bin" / "coverage-run.py"
        ...
        source = self.host_paths.shipyard_ext_project_dir / "pyproject.toml"
...
class EntryPointMountConfigurator:
    """
    Mounts ``entry_points.txt`` files of shipyard and dependencies into the venv in the container.

    For example, when starting the pro container, the entrypoints of shipyard-ext on the host would be in
    ``~/workspace/shipyard-ext/shipyard_ext.egg-info/entry_points.txt``
    which needs to be mounted into the distribution info of the installed dependency within the container:
    ``/opt/code/shipyard/.venv/.../site-packages/shipyard_ext-2.1.0.dev0.dist-info/entry_points.txt``.
    """

    entry_point_glob = (
        "/opt/code/shipyard/.venv/lib/python3.*/site-packages/*.dist-info/entry_points.txt"
    )
    shipyard_community_entry_points = (
        "/opt/code/khulnasoft/shipyard_core.egg-info/entry_points.txt"
    )
    ...
    def __call__(self, cfg: ContainerConfiguration):
        ...
        if not self.pro:
            host_path = (
                self.host_paths.shipyard_project_dir
            ...
            if host_path.exists():
                cfg.volumes.append(
                    VolumeBind(
                        str(host_path), self.shipyard_community_entry_points, read_only=True
                    )
                )
...
class DependencyMountConfigurator:
    ...
    dependency_glob = "/opt/code/shipyard/.venv/lib/python3.*/site-packages/*"
...
def _list_files_in_container_image(container_client: ContainerClient, image_name: str) -> list[str]:
    ...
    the image ID into a gziped file into ``~/.cache/shipyard-dev-cli`` to (significantly) speed up
    subsequent calls.
    ...
    cache_dir = get_user_cache_dir() / "shipyard-dev-cli"
