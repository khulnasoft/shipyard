...
from shipyard.constants import (
    ...
    ENV_DEV,
    SHIPYARD_INFRA_PROCESS,
)
from shipyard.http.duplex_socket import enable_duplex_socket
from shipyard.runtime import events, hooks
from shipyard.runtime.exceptions import ShipyardExit
from shipyard.services.plugins import SERVICE_PLUGINS, ServiceDisabled, wait_for_infra_shutdown
from shipyard.utils import files, objects
from shipyard.utils.analytics import usage
from shipyard.utils.aws.request_context import patch_moto_request_handling
...
from shipyard.utils.container_networking import get_main_container_id
from shipyard.utils.files import cleanup_tmp_files
from shipyard.utils.net import is_port_open
from shipyard.utils.patch import patch
from shipyard.utils.platform import in_docker
from shipyard.utils.sync import poll_condition
from shipyard.utils.threads import (
...
def exit_infra(code: int):
    """
    Triggers an orderly shutdown of the shipyard infrastructure and sets the code the main process should
...
def check_aws_credentials():
    # Setup AWS environment vars, these are used by Boto when Shipyard makes internal cross-service calls
...
def signal_supervisor_restart():
    if pid := os.environ.get("SUPERVISOR_PID"):
        ...
        LOG.warning("could not signal supervisor to restart shipyard")
...
def print_runtime_information(in_docker=False):
    ...
    print()
    print("Shipyard version: %s" % constants.VERSION)
    if in_docker:
        id = get_main_container_id()
        if id:
            print("Shipyard Docker container id: %s" % id[:12])

    if config.SHIPYARD_BUILD_DATE:
        print("Shipyard build date: %s" % config.SHIPYARD_BUILD_DATE)

    if config.SHIPYARD_BUILD_GIT_HASH:
        print("Shipyard build git hash: %s" % config.SHIPYARD_BUILD_GIT_HASH)
...
def start_infra(asynchronous=False, apis=None):
    ...
    try:
        os.environ[SHIPYARD_INFRA_PROCESS] = "1"
        ...
        print(f"Shipyard returning with exit code {e.code}. Reason: {e}")
        raise
...
def do_start_infra(asynchronous, apis, is_in_docker):
    if config.DEVELOP:
        from shipyard.packages.debugpy import debugpy_package
    ...
    def start_runtime_components():
        from shipyard.services.edge import start_edge

        # TODO: we want a composable Shipyard runtime (edge proxy, service manager, dns, ...)
