...
from shipyard.aws.api import HttpResponse
from shipyard.aws.api.lambda_ import InvocationType
from shipyard.aws.protocol.serializer import gen_amzn_requestid
from shipyard.http import Request, Router
from shipyard.http.dispatcher import Handler
from shipyard.services.lambda_.api_utils import FULL_FN_ARN_PATTERN
...
from shipyard.utils.aws.request_context import AWS_REGION_REGEX
from shipyard.utils.strings import long_uid, to_bytes, to_str
from shipyard.utils.time import TIMESTAMP_READABLE_FORMAT, mktime, timestamp
...
def event_for_lambda_url(api_id: str, path: str, data, headers, method: str) -> dict:
    ...
    ignored_headers = ["connection", "x-shipyard-tgt-api", "x-shipyard-request-url"]
    ...
    event_headers.update(
        {
            ...
            "x-forwarded-port": str(shipyard_host().port),
        }
    )
