...
from shipyard.aws.api.lambda_ import Architecture, PackageType, Runtime
from shipyard.dns import server as dns_server
from shipyard.services.lambda_ import hooks as lambda_hooks
from shipyard.services.lambda_.invocation.executor_endpoint import (
    INVOCATION_PORT,
    ExecutorEndpoint,
)
from shipyard.services.lambda_.invocation.lambda_models import FunctionVersion
from shipyard.services.lambda_.invocation.runtime_executor import (
    LambdaRuntimeException,
    RuntimeExecutor,
)
from shipyard.services.lambda_.lambda_utils import HINT_LOG
from shipyard.services.lambda_.networking import (
...
from shipyard.utils.docker_utils import DOCKER_CLIENT as CONTAINER_CLIENT
from shipyard.utils.net import get_free_tcp_port
from shipyard.utils.strings import truncate
...
HOT_RELOADING_ENV_VARIABLE = "SHIPYARD_HOT_RELOADING_PATHS"
...
def get_image_name_for_function(function_version: FunctionVersion) -> str:
    return f"shipyard/lambda-{function_version.id.qualified_arn().replace(':', '_').replace('$', '_').lower()}"
...
class DockerRuntimeExecutor(RuntimeExecutor):
    ...
    def _generate_container_name(self):
        ...
        container_name = "-".join(
            [
                get_main_container_name() or "shipyard",
                "lambda",
    ...
    def start(self, env_vars: dict[str, str]) -> None:
        ...
        # set the dns server of the lambda container to the Shipyard container IP
        ...
        if config.LAMBDA_DOCKER_DNS:
            ...
            LOG.warning(
                "Container DNS overriden to %s, connection to names pointing to Shipyard, like 'localhost.shipyard.khulnasoft.com' will need additional configuration.",
        ...
        if function_version.config.code:
            ...
            if (image_name, platform) not in PULLED_IMAGES:
                try:
                    ...
                    HINT_LOG.error(
                        "Failed to pull Docker image because Docker is not available in the Shipyard container "
                        "but required to run Lambda functions. Please add the Docker volume mount "
                        '"/var/run/docker.sock:/var/run/docker.sock" to your Shipyard startup. '
                        "https://docs.shipyard.khulnasoft.com/user-guide/aws/lambda/#docker-not-available"
                    )
                    raise e
            if config.LAMBDA_PREBUILD_IMAGES:
    ...
    def validate_environment(cls) -> bool:
        if not CONTAINER_CLIENT.has_docker():
            LOG.warning(
                "WARNING: Docker not available in the Shipyard container but required to run Lambda "
                'functions. Please add the Docker volume mount "/var/run/docker.sock:/var/run/docker.sock" to your '
                "Shipyard startup. https://docs.shipyard.khulnasoft.com/user-guide/aws/lambda/#docker-not-available"
            )
            return False
        return True
