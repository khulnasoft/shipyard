...
from shipyard.aws.api.lambda_ import TracingMode
...
class ExecutionEnvironment:
    ...
    def get_environment_variables(self) -> Dict[str, str]:
        ...
        env_vars = {
            ...
            # 3) Public Shipyard endpoint
            "SHIPYARD_HOSTNAME": self.runtime_executor.get_endpoint_from_executor(),
            "EDGE_PORT": str(config.GATEWAY_LISTEN[0].port),
            # AWS_ENDPOINT_URL conditionally added below
            # 4) Internal Shipyard runtime API
            "SHIPYARD_RUNTIME_ID": self.id,
            "SHIPYARD_RUNTIME_ENDPOINT": self.runtime_executor.get_runtime_endpoint(),
            # used by the init to spawn the x-ray daemon
            # SHIPYARD_USER conditionally added below
        }
        ...
        if config.LAMBDA_INIT_DEBUG:
            ...
            env_vars["SHIPYARD_USER"] = "root"
        # Forcefully overwrite the user might break debugging!
        if config.LAMBDA_INIT_USER is not None:
            env_vars["SHIPYARD_USER"] = config.LAMBDA_INIT_USER
        if config.DEBUG:
            env_vars["SHIPYARD_INIT_LOG_LEVEL"] = "debug"
        if config.LAMBDA_INIT_POST_INVOKE_WAIT_MS:
            env_vars["SHIPYARD_POST_INVOKE_WAIT_MS"] = int(config.LAMBDA_INIT_POST_INVOKE_WAIT_MS)
        return env_vars
