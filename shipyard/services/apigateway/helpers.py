...
from shipyard.constants import (
    ...
    HEADER_SHIPYARD_EDGE_URL,
    PATH_USER_REQUEST,
)
from shipyard.services.apigateway.context import ApiInvocationContext
from shipyard.services.apigateway.models import (
    ApiGatewayStore,
    RestApiContainer,
    apigateway_stores,
)
from shipyard.utils import common
from shipyard.utils.aws import queries
from shipyard.utils.aws import resources as resource_utils
from shipyard.utils.aws.arns import parse_arn
from shipyard.utils.aws.aws_responses import requests_error_response_json, requests_response
from shipyard.utils.aws.request_context import MARKER_APIGW_REQUEST_REGION, THREAD_LOCAL
from shipyard.utils.json import try_json
from shipyard.utils.strings import long_uid, short_uid, to_bytes, to_str
...
def set_api_id_stage_invocation_path(
    ...
    host_header = headers.get(HEADER_SHIPYARD_EDGE_URL, "") or headers.get("Host") or ""
...
def extract_api_id_from_hostname_in_url(hostname: str) -> str:
    """Extract API ID 'id123' from URLs like https://id123.execute-api.localhost.shipyard.khulnasoft.com:4566"""
...
def get_regional_domain_name(domain_name: str) -> str:
    ...
    In Shipyard, we're returning this format: "d-<domain_hash>.execute-api.localhost.shipyard.khulnasoft.com"
    """
    domain_name_hash = get_domain_name_hash(domain_name)
    host = shipyard_host().host
