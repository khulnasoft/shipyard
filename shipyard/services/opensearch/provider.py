...
from shipyard.aws.api import RequestContext, handler
...
from shipyard.constants import OPENSEARCH_DEFAULT_VERSION
from shipyard.services.opensearch import versions
from shipyard.services.opensearch.cluster import SecurityOptions
from shipyard.services.opensearch.cluster_manager import (
    ClusterManager,
    DomainKey,
    create_cluster_manager,
)
from shipyard.services.opensearch.models import OpenSearchStore, opensearch_stores
...
from shipyard.utils.collections import PaginatedList, remove_none_values_from_dict
...
def _run_cluster_startup_monitor(cluster: Server, domain_name: str, region: str):
    ...
    # NOTE: does not work when DNS rebind protection is active for localhost.shipyard.khulnasoft.com
...
def create_cluster(
    ...
    status["Endpoint"] = cluster.url.split("://")[-1].replace("0.0.0.0", shipyard_host().host)
