...
from shipyard.utils.archives import download_and_extract_with_retry
from shipyard.utils.files import chmod_r, load_file, mkdir, rm_rf, save_file
from shipyard.utils.run import run
from shipyard.utils.ssl import create_ssl_cert, install_predefined_cert_if_available
from shipyard.utils.sync import SynchronizedDefaultDict, retry
...
class OpensearchPackageInstaller(PackageInstaller):
    ...
    def _install(self, target: InstallTarget):
        ...
        from shipyard.aws.api.opensearch import EngineType
        from shipyard.services.opensearch import versions
        ...
        with _OPENSEARCH_INSTALL_LOCKS[version]:
            if not os.path.exists(install_dir):
                ...
                tmp_archive = os.path.join(
                    config.dirs.cache, f"shipyard.{os.path.basename(opensearch_url)}"
                )
    ...
    def _setup_security(self, install_dir: str, parsed_version: semver.VersionInfo):
        ...
        # create the internal user which allows shipyard to manage the running instance
...
class ElasticsearchPackageInstaller(PackageInstaller):
    ...
    def _install(self, target: InstallTarget):
        ...
        if not os.path.exists(installed_executable):
            ...
            mkdir(install_dir_parent)
            # download and extract archive
            tmp_archive = os.path.join(config.dirs.cache, f"shipyard.{os.path.basename(es_url)}")
