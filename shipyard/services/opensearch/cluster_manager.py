...
from shipyard.aws.api.opensearch import DomainEndpointOptions, EngineType
from shipyard.constants import LOCALHOST
from shipyard.services.opensearch import versions
from shipyard.services.opensearch.cluster import (
    CustomEndpoint,
...
def create_cluster_manager() -> "ClusterManager":
    ...
    # If we are using a shipyard-managed multi-cluster-setup, we use the MultiClusterManager.
...
def build_cluster_endpoint(
    ...
    """
    Builds the cluster endpoint from and optional custom_endpoint and the shipyard opensearch config. Example
    values:

    - my-domain.us-east-1.opensearch.localhost.shipyard.khulnasoft.com:4566 (endpoint strategy = domain (default))
    ...
    - my.domain:443/foo (arbitrary endpoints (technically not allowed by AWS, but there are no rules in shipyard))
    ...
    if config.OPENSEARCH_ENDPOINT_STRATEGY == "port":
        ...
        host_definition = shipyard_host(custom_port=assigned_port)
    ...
    # or through a subdomain (domain-name.region.opensearch.localhost.shipyard.khulnasoft.com)
    host_definition = shipyard_host()
...
class MultiplexingClusterManager(ClusterManager):
    """
    Multiplexes multiple endpoints to a single backend cluster (not managed by Shipyard).
