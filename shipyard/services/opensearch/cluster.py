...
from shipyard.aws.api.opensearch import (
    AdvancedSecurityOptionsInput,
    EngineType,
    ValidationException,
)
from shipyard.http.client import SimpleRequestsClient
from shipyard.http.proxy import ProxyHandler
from shipyard.services.edge import ROUTER
from shipyard.services.infra import DEFAULT_BACKEND_HOST
from shipyard.services.opensearch import versions
from shipyard.services.opensearch.packages import elasticsearch_package, opensearch_package
from shipyard.utils.aws.arns import parse_arn
from shipyard.utils.common import (
    ShellCommandThread,
    chmod_r,
    get_free_tcp_port,
    is_root,
...
INTERNAL_USER_AUTH = ("shipyard-internal", "shipyard-internal")
...
def register_cluster(
    ...
    if custom_endpoint and custom_endpoint.enabled:
        ...
        assert not (
            host == shipyard_host().host and (not path or path == "/")
        ...
        assert not host == shipyard_host().host, "trying to register an illegal catch all route"
        rules.append(
            ROUTER.add(
                "/",
                endpoint=endpoint,
...
class OpensearchCluster(Server):
    """Manages an OpenSearch cluster which is installed and operated by Shipyard."""
...
class EdgeProxiedOpensearchCluster(Server):
    ...
    def health(self):
        ...
        # The user may have customised `SHIPYARD_HOST`, so we need to rewrite the health
        # check endpoint to always make a request against localhost.shipyard.khulnasoft.com (since we
        ...
        url = self.url.replace(config.SHIPYARD_HOST.host, constants.LOCALHOST_HOSTNAME)
        url = url.replace(str(config.SHIPYARD_HOST.port), str(config.GATEWAY_LISTEN[0].port))
        host = self._url.hostname
