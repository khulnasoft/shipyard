...
from shipyard.aws.api.dynamodb import (
...
from shipyard.services.dynamodb.models import DynamoDBStore, dynamodb_stores
from shipyard.services.dynamodb.server import DynamodbServer
from shipyard.services.dynamodb.utils import (
    ItemFinder,
    ItemSet,
    SchemaExtractor,
    extract_table_name_from_partiql_update,
)
from shipyard.services.dynamodbstreams import dynamodbstreams_api
from shipyard.services.dynamodbstreams.dynamodbstreams_api import (
    get_and_increment_sequence_number_counter,
)
from shipyard.services.dynamodbstreams.models import dynamodbstreams_stores
from shipyard.services.edge import ROUTER
from shipyard.services.plugins import ServiceLifecycleHook
from shipyard.state import AssetDirectory, StateVisitor
from shipyard.utils.aws import arns
from shipyard.utils.aws.arns import extract_account_id_from_arn, extract_region_from_arn
from shipyard.utils.aws.aws_stack import get_valid_regions_for_service
from shipyard.utils.aws.request_context import (
    extract_account_id_from_headers,
    extract_region_from_headers,
)
from shipyard.utils.collections import select_attributes, select_from_typed_dict
from shipyard.utils.common import short_uid, to_bytes
from shipyard.utils.json import BytesEncoder, canonical_json
from shipyard.utils.strings import long_uid, to_str
from shipyard.utils.threads import start_worker_thread
...
class SSEUtils:
    ...
    def get_sse_kms_managed_key(cls, account_id: str, region_name: str):
        from shipyard.services.kms import provider
...
class DynamoDBProvider(DynamodbApi, ServiceLifecycleHook):
    ...
    def _forward_request(self, context: RequestContext, region: str | None) -> ServiceResponse:
        ...
        This is used for operations impacted by global tables. In Shipyard, a single copy of global table
    ...
    def delete_table(self, context: RequestContext, table_name: TableName) -> DeleteTableOutput:
        ...
        # This is not the case for Shipyard, where all replicated tables will also be removed if source is deleted.
    ...
    def describe_table(self, context: RequestContext, table_name: TableName) -> DescribeTableOutput:
        ...
        # Update table properties from Shipyard stores
    ...
    def update_table(
        ...
        try:
            ...
            # But we still need to update this info in Shipyard stores
    ...
    def get_global_table_region(context: RequestContext, table_name: str) -> str:
        ...
        Replication in Shipyard works by keeping a single copy of a table and forwarding
