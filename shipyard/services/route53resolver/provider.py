...
import shipyard.services.route53resolver.utils
from shipyard.aws.api import RequestContext
from shipyard.aws.api.route53resolver import (
    Action,
...
from shipyard.services.ec2.models import get_ec2_backend
from shipyard.services.moto import call_moto
from shipyard.services.route53resolver.models import Route53ResolverStore, route53resolver_stores
from shipyard.services.route53resolver.utils import (
...
from shipyard.utils.collections import select_from_typed_dict
                ...
                for domain in domains:
                    if domain in firewall_domains:
                        ...
                        raise ValidationException(
                            f"[RSLVR-02502] The following domains don't exist in the DNS Firewall domain list '{firewall_domain_list_id}'. You can't delete a domain that isn't in a domain list. Example unknown domain: '{domain}'. Trace Id: '{shipyard.services.route53resolver.utils.get_trace_id()}'"
                        )
    ...
    def list_firewall_rules(
        ...
        if len(firewall_rules) == 0:
            raise ResourceNotFoundException(
                f"Can't find the resource with ID '{firewall_rule_group_id}'. Trace Id: '{shipyard.services.route53resolver.utils.get_trace_id()}'"
            )
    ...
    def associate_firewall_rule_group(
        ...
        for firewall_rule_group_association in store.firewall_rule_group_associations.values():
            if (
                ...
                raise ValidationException(
                    f"[RSLVR-02302] This DNS Firewall rule group can't be associated to a VPC: '{vpc_id}'. It is already associated to VPC '{firewall_rule_group_id}'. Try again with another VPC or DNS Firewall rule group. Trace Id: '{shipyard.services.route53resolver.utils.get_trace_id()}'"
                )
