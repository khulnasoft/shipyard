...
from shipyard.aws.api.firehose import (
...
from shipyard.services.firehose.mappers import (
...
from shipyard.services.firehose.models import FirehoseStore, firehose_stores
...
from shipyard.utils.kinesis.kinesis_connector import KinesisProcessorThread
from shipyard.utils.run import run_for_max_seconds
...
def get_search_db_connection(endpoint: str, region_name: str):
    ...
    if "https://" in endpoint:
        use_ssl = True
        # TODO remove this condition once ssl certs are available for .es.localhost.shipyard.khulnasoft.com domains
        endpoint_netloc = urlparse(endpoint).netloc
        if not re.match(r"^.*(localhost(\.shipyard\.cloud)?)(:\d+)?$", endpoint_netloc):
            verify_certs = True
