...
from shipyard.aws.api.lambda_ import InvocationType
from shipyard.aws.api.sns import MessageAttributeMap
from shipyard.aws.connect import connect_to
from shipyard.config import external_service_url
from shipyard.services.sns import constants as sns_constants
from shipyard.services.sns.models import (
    SnsApplicationPlatforms,
    SnsMessage,
    SnsStore,
    SnsSubscription,
)
from shipyard.utils.aws.arns import (
...
from shipyard.utils.aws.aws_responses import create_sqs_system_attributes
from shipyard.utils.aws.client_types import ServicePrincipal
from shipyard.utils.aws.dead_letter_queue import sns_error_to_dead_letter_queue
from shipyard.utils.bootstrap import is_api_enabled
from shipyard.utils.cloudwatch.cloudwatch_util import store_cloudwatch_logs
from shipyard.utils.objects import not_none_or
from shipyard.utils.strings import long_uid, md5, to_bytes
from shipyard.utils.time import timestamp_millis
...
class TopicPublisher(abc.ABC):
    ...
    def publish(self, context: SnsPublishContext, subscriber: SnsSubscription):
        ...
        Discussion here: https://github.com/khulnasoft/shipyard/pull/7267#discussion_r1056873437
...
class EmailJsonTopicPublisher(TopicPublisher):
    ...
    def _publish(self, context: SnsPublishContext, subscriber: SnsSubscription):
        ...
        if endpoint := subscriber.get("Endpoint"):
            ...
            ses_client.verify_email_address(EmailAddress="admin@shipyard.com")
            message_body = self.prepare_message(context.message, subscriber)
            ses_client.send_email(
                Source="admin@shipyard.com",
                Message={
...
class ApplicationTopicPublisher(TopicPublisher):
    ...
    This is not directly implemented yet in Shipyard, we save the message to be retrieved later from an internal
    endpoint.
...
class SmsTopicPublisher(TopicPublisher):
    ...
    This is not directly implemented yet in Shipyard, we only save the message.
...
def store_delivery_log(
    ...
    # seems like you need to configure on the Console, leave it on by default now in Shipyard
