...
from shipyard.aws.api.kms import (
    AlgorithmSpec,
...
from shipyard.services.kms.exceptions import ValidationException
from shipyard.services.kms.models import (
...
from shipyard.services.kms.utils import is_valid_key_arn, parse_key_arn, validate_alias_name
from shipyard.services.plugins import ServiceLifecycleHook
from shipyard.utils.aws.arns import kms_alias_arn, parse_arn
from shipyard.utils.collections import PaginatedList
from shipyard.utils.common import select_attributes
...
class KmsProvider(KmsApi, ServiceLifecycleHook):
    """
    The Shipyard Key Management Service (KMS) provider.
    ...
    # in Shipyard, so this function is limited to them.
    #
    ...
    # Currently Shipyard only calculates SHA256 digests no matter what the signing algorithm is.
    ...
    def encrypt(
        ...
        # For compatibility, we return EncryptionAlgorithm values expected from AWS. But Shipyard currently always
    ...
    def decrypt(
        ...
        # Since Shipyard doesn't currently do asymmetrical encryption, there is a question of modeling here: we
        ...
        try:
            ...
            raise InvalidCiphertextException(
                "Shipyard is unable to deserialize the ciphertext blob. Perhaps the "
                "blob didn't come from Shipyard"
            )
    ...
    def delete_alias(self, context: RequestContext, request: DeleteAliasRequest) -> None:
        ...
        # Shipyard, are supported by this operation.
        store = self._get_store(context.account_id, context.region)
        alias_name = request["AliasName"]
        if alias_name not in store.aliases:
...
# APIs to do that. Functions here are supposed to be used by other Shipyard services to have similar integrations
# with KMS in Shipyard. As such, they are supposed to be proper APIs (as in error and security handling),
