...
from shipyard.utils.aws import aws_stack
from shipyard.utils.common import select_attributes, short_uid
from shipyard.utils.functions import run_safe
from shipyard.utils.json import json_safe
from shipyard.utils.objects import recurse_object
from shipyard.utils.strings import is_string
    ...
    def replace(account_id: str, region_name: str, params, logical_resource_id, *args, **kwargs):
        ...
        for name in param_names:
            if isinstance(result.get(name), (dict, list)):
                # Fix for https://github.com/khulnasoft/shipyard/issues/2022
...
def log_not_available_message(resource_type: str, message: str):
    LOG.warning(
        f"{message}. To find out if {resource_type} is supported in Shipyard Pro, "
        "please check out our docs at https://docs.shipyard.khulnasoft.com/user-guide/aws/cloudformation/#resources-pro--enterprise-edition"
    )
