...
from shipyard.services.stores import AccountRegionBundle
from shipyard.state.core import StateVisitor
...
class ServiceBackend(TypedDict, total=False):
    ...
    shipyard: AccountRegionBundle | None
...
class ServiceBackendCollectorVisitor(StateVisitor):
    ...
    def collect(self) -> ServiceBackend:
        ...
        if self.store:
            service_backend.update({"shipyard": self.store})
...
class ReflectionStateLocator:
    ...
    def accept_state_visitor(self, visitor: StateVisitor):
        ...
        module_name = f"shipyard_ext.services.{service_name}.models"
        ...
        if attribute is None:
            module_name = f"shipyard.services.{service_name}.models"
            attribute = _load_attribute_from_module(module_name, attribute_name)
