...
from shipyard.aws.connect import connect_externally_to, connect_to
...
from shipyard.constants import (
    ...
    SHIPYARD_VENV_FOLDER,
    TEST_AWS_ACCESS_KEY_ID,
    TEST_AWS_ACCOUNT_ID,
    TEST_AWS_REGION_NAME,
)
from shipyard.services.lambda_.lambda_utils import (
    get_handler_file_from_name,
)
from shipyard.utils.archives import create_zip_file_cli, create_zip_file_python
from shipyard.utils.collections import ensure_list
from shipyard.utils.files import (
    TMP_FILES,
    chmod_r,
    cp_r,
    is_empty_dir,
...
from shipyard.utils.net import get_free_tcp_port, is_port_open
from shipyard.utils.platform import is_debian
    ...
    with tempfile.TemporaryDirectory(prefix=ARCHIVE_DIR_PREFIX) as tmp_dir:
        ...
        for lib in libs:
            ...
            try:
                ...
                pass
            target_dir = tmp_dir
            root_folder = os.path.join(SHIPYARD_VENV_FOLDER, "lib/python*/site-packages")
            if lib == "shipyard":
                paths = ["shipyard/*.py", "shipyard/utils"]
                root_folder = SHIPYARD_ROOT_FOLDER
...
def http_server(handler, host="127.0.0.1", port=None) -> str:
    ...
    """
    from shipyard.utils.server.http2_server import run_server
...
def upload_file_to_bucket(s3_client, bucket_name, file_path, file_name=None):
    ...
    domain = "amazonaws.com" if is_aws_cloud() else shipyard_host().host_and_port()
