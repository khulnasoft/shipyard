...
from shipyard.constants import DEFAULT_VOLUME_DIR, DOCKER_IMAGE_NAME
...
from shipyard.utils.net import IntOrPort, Port, PortNotAvailableException, PortRange
from shipyard.utils.objects import singleton_factory
from shipyard.utils.strings import to_str
...
def get_default_volume_dir_mount() -> Optional[VolumeInfo]:
    """
    Returns the volume information of Shipyard's DEFAULT_VOLUME_DIR (/var/lib/shipyard), if mounted,
...
def get_host_path_for_path_in_docker(path):
    """
    Returns the calculated host location for a given subpath of DEFAULT_VOLUME_DIR inside the shipyard container.
    ...
    if config.is_in_docker:
        ...
        if volume:
            ...
            if not path.startswith(f"{DEFAULT_VOLUME_DIR}/") and path != DEFAULT_VOLUME_DIR:
                ...
                # Please check if the target path is indeed prefixed by /var/lib/shipyard
...
def _get_ports_check_docker_image() -> str:
    ...
    if not config.is_in_docker:
        ...
        from shipyard.utils.bootstrap import get_docker_image_to_start

        # Use whatever image the user is trying to run Shipyard with, since they either have
        # it already, or need it by definition to start Shipyard.
