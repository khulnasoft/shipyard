...
from .urls import shipyard_host
...
def generate_ssl_cert(
    ...
    cert.set_pubkey(k)
    alt_names = (
        f"DNS:localhost,DNS:test.localhost.atlassian.io,DNS:localhost.shipyard.khulnasoft.com,DNS:{host_definition.host}IP:127.0.0.1"
    ).encode("utf8")
    cert.add_extensions(
        [
    ...
    if target_file:
        ...
        # https://github.com/khulnasoft/shipyard/issues/1607
        if not all_exist(target_file, key_file_name, cert_file_name):
            for i in range(2):
                try:
                    ...
                    # Fix for https://github.com/khulnasoft/shipyard/issues/1743
