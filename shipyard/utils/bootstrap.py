...
from shipyard.config import HostAndPort, default_ip, is_env_not_false, is_env_true
...
from shipyard.utils.container_utils.docker_cmd_client import CmdDockerClient
from shipyard.utils.docker_utils import DOCKER_CLIENT, container_ports_can_be_bound
from shipyard.utils.files import cache_dir, mkdir
from shipyard.utils.functions import call_safe
from shipyard.utils.net import Port, get_free_tcp_port, get_free_tcp_port_range
from shipyard.utils.run import is_command_available, run, to_str
from shipyard.utils.serving import Server
from shipyard.utils.strings import short_uid
from shipyard.utils.sync import poll_condition
...
def get_server_version_from_running_container() -> str:
    try:
        ...
        version, _ = DOCKER_CLIENT.exec_in_container(
            container_name, interactive=True, command=["bin/shipyard", "--version"]
        )
...
def get_server_version() -> str:
    ...
    version_cache = cache_dir() / "image_metadata" / image_hash / "shipyard_version"
    if version_cache.exists():
        cached_version = version_cache.read_text()
        return cached_version.strip()

    env_version = get_image_environment_variable("SHIPYARD_BUILD_VERSION")
...
def setup_logging():
    ...
    from shipyard.logging.setup import setup_logging_from_config
...
def validate_shipyard_config(name: str):
    ...
    ls_service_name = [
        name for name, svc in services_config.items() if "shipyard" in svc.get("image", "")
    ]
    if not ls_service_name:
        raise Exception(
            'No Shipyard service found in config (looking for image names containing "shipyard")'
        )
    if len(ls_service_name) > 1:
        warns.append(f"Multiple candidates found for Shipyard service: {ls_service_name}")
...
class ContainerConfigurators:
    """
    A set of useful container configurators that are typical for starting the shipyard container.
    """
    ...
    @staticmethod
    def mount_shipyard_volume(host_path: str | os.PathLike = None):
...
def get_gateway_port(container: Container) -> int:
    ...
    :param container: the shipyard container
...
def get_gateway_url(
    ...
    """
    Returns the shipyard container's gateway URL reachable from the host. In most cases this will be
    ``http://localhost.shipyard.khulnasoft.com:4566``.

    :param container: the container
    :param hostname: the hostname to use (default localhost.shipyard.khulnasoft.com)
...
class Container:
    ...
    def start(self, attach: bool = False) -> RunningContainer:
        # FIXME: this is pretty awkward, but additional_flags in the ShipyardContainer API was
        ...
        try:
            ...
            LOG.error(
                "Error while starting Shipyard container: %s\n%s",
                e.message,
                to_str(e.stderr),
...
class RunningContainer:
    """
    Represents a Shipyard container that is running.
    """
...
class ShipyardContainerServer(Server):
    ...
    def do_run(self):
        if self.is_container_running():
            raise ContainerExists(
                'Shipyard container named "%s" is already running' % self.container.name
            )
        ...
        LOG.debug("starting Shipyard container")
    ...
    def do_shutdown(self):
        try:
            ...
            LOG.info("error cleaning up shipyard container %s: %s", self.container.name, e)
...
def prepare_docker_start():
    ...
    if DOCKER_CLIENT.is_container_running(container_name):
        raise ContainerExists('Shipyard container named "%s" is already running' % container_name)
...
def configure_container(container: Container):
    """
    Configuration routine for the ShipyardContainer.
    """
    port_configuration = PortMappings(bind_host=config.GATEWAY_LISTEN[0].host)
    ...
    hooks.configure_shipyard_container.run(container)
    ...
    container.configure(
        [
            ...
            ContainerConfigurators.mount_shipyard_volume(config.VOLUME_DIR),
...
def prepare_host(console):
    """
    Prepare the host environment for running Shipyard, this should be called before start_infra_*.
    """
    if os.environ.get(constants.SHIPYARD_INFRA_PROCESS) in constants.TRUE_STRINGS:
        return
...
def start_infra_in_docker(console, cli_params: Dict[str, Any] = None):
    ...
    status = console.status("Starting Shipyard container")
    status.start()
    ...
    def _init_log_printer(line):
        ...
        to print."""
        status.stop()
        console.rule("Shipyard Runtime Log (press [bold][yellow]CTRL-C[/yellow][/bold] to quit)")
        print(line)
    ...
    #  being received (e.g., when running the shipyard CLI as part of a "npm run .." script).
    ...
    # start the Shipyard container as a Server
    server = ShipyardContainerServer(container)
...
def start_infra_in_docker_detached(console, cli_params: Dict[str, Any] = None):
    ...
    server = ShipyardContainerServer(container_config)
    server.start()
...
def wait_container_is_ready(timeout: Optional[float] = None):
    """Blocks until the shipyard main container is running and the ready marker has been printed."""
    container_name = config.MAIN_CONTAINER_NAME
    started = time.time()
...
def is_api_key_configured() -> bool:
    ...
    return (
        True
        if os.environ.get("SHIPYARD_API_KEY") and os.environ.get("SHIPYARD_API_KEY").strip()
        else False
    )
