...
from shipyard.utils.net import get_docker_host_from_container
...
def get_main_container_network() -> Optional[str]:
    """
    Gets the main network of the Shipyard container (if we run in one, bridge otherwise)
    If there are multiple networks connected to the Shipyard container, we choose the first as "main" network
    ...
    if config.MAIN_DOCKER_NETWORK:
        if config.is_in_docker:
            ...
            if config.MAIN_DOCKER_NETWORK not in networks:
                LOG.warning(
                    "The specified 'MAIN_DOCKER_NETWORK' is not connected to the Shipyard container! Falling back to %s",
                    networks[0],
                )
                return networks[0]
...
def get_endpoint_for_network(network: Optional[str] = None) -> str:
    """
    Get the Shipyard endpoint (= IP address) on the given network.
    If a network is given, it will return the IP address/hostname of Shipyard on that network
...
def get_main_container_id():
    """
    Return the container ID of the Shipyard container
...
def get_main_container_name():
    """
    Returns the container name of the Shipyard container

    :return: Shipyard container name
    """
