...
from shipyard.utils.bootstrap import Container
from shipyard.utils.files import rm_rf
from shipyard.utils.functions import call_safe
from shipyard.utils.json import FileMappedDocument
from shipyard.utils.objects import singleton_factory
from shipyard.utils.strings import long_uid, md5
...
def get_version_string() -> str:
    gh = config.SHIPYARD_BUILD_GIT_HASH
    if gh:
...
def get_session_id() -> str:
    """
    Returns the unique ID for this Shipyard session.
    :return: a UUID
    """
...
def get_shipyard_edition() -> Optional[Literal["enterprise", "pro", "community"]]:
    if os.path.exists("/usr/lib/shipyard/.enterprise-version"):
        return "enterprise"
    elif os.path.exists("/usr/lib/shipyard/.pro-version"):
        return "pro"
    elif os.path.exists("/usr/lib/shipyard/.community-version"):
        return "community"
...
def is_license_activated() -> bool:
    try:
        from shipyard_ext import config  # noqa
    except ImportError:
        return False

    try:
        from shipyard_ext.bootstrap import licensingv2
...
def get_api_key_or_auth_token() -> Optional[str]:
    # TODO: this is duplicated code from ext, but should probably migrate that to shipyard
    auth_token = os.environ.get("SHIPYARD_AUTH_TOKEN", "").strip("'\" ")
    if auth_token:
        return auth_token

    api_key = os.environ.get("SHIPYARD_API_KEY", "").strip("'\" ")
    if api_key:
        return api_key
...
@hooks.configure_shipyard_container()
