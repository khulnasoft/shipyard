...
from shipyard.aws.api import RequestContext, ServiceException
from shipyard.aws.chain import ExceptionHandler, HandlerChain
from shipyard.http import Response
from shipyard.http.request import restore_payload
from shipyard.logging.format import AwsTraceLoggingFormatter, TraceLoggingFormatter
from shipyard.logging.setup import create_default_handler
...
class ResponseLogger:
    def __call__(self, _: HandlerChain, context: RequestContext, response: Response):
        if context.request.path == "/health" or context.request.path == "/_shipyard/health":
    ...
    def aws_logger(self):
        return self._prepare_logger(
            logging.getLogger("shipyard.request.aws"), formatter=AwsTraceLoggingFormatter
        )

    @cached_property
    def http_logger(self):
        return self._prepare_logger(
            logging.getLogger("shipyard.request.http"), formatter=TraceLoggingFormatter
        )

    @cached_property
    def internal_aws_logger(self):
        return self._prepare_logger(
            logging.getLogger("shipyard.request.internal.aws"), formatter=AwsTraceLoggingFormatter
        )

    @cached_property
    def internal_http_logger(self):
        return self._prepare_logger(
            logging.getLogger("shipyard.request.internal.http"), formatter=TraceLoggingFormatter
        )
