...
from shipyard.utils.common import camel_to_snake_case, snake_to_camel_case
...
def generate_service_types(output, service: ServiceModel, doc=True):
    ...
    output.write("\n")
    output.write(
        "from shipyard.aws.api import handler, RequestContext, ServiceException, ServiceRequest"
    )
    output.write("\n")
...
@click.option(
    "--path", default="./shipyard/aws/api", help="the path where the api should be saved"
)
