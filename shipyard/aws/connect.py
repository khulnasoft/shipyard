...
from shipyard.aws.spec import SHIPYARD_BUILTIN_DATA_PATH
...
from shipyard.utils.aws.aws_stack import get_local_service_url, get_s3_hostname
from shipyard.utils.aws.client_types import ServicePrincipal, TypedServiceClientFactory
...
def my_patch(fn, self, **kwargs):
    ...
    if shipyard_config.DISABLE_CUSTOM_BOTO_WAITER_CONFIG:
        return fn(self, **kwargs)
    else:
        patched_kwargs = {
            **kwargs,
            "WaiterConfig": {
                "Delay": shipyard_config.BOTO_WAITER_DELAY,
                "MaxAttempts": shipyard_config.BOTO_WAITER_MAX_ATTEMPTS,
                **kwargs.get(
                    "WaiterConfig", {}
...
class ClientFactory(ABC):
    ...
    def __init__(
        ...
        if SHIPYARD_BUILTIN_DATA_PATH not in self._session._loader.search_paths:
            self._session._loader.search_paths.append(SHIPYARD_BUILTIN_DATA_PATH)
    ...
    def __call__(
        ...
        *,
        ...
        :param endpoint_url: Full endpoint URL to be used by the client.
            Defaults to appropriate Shipyard endpoint.
    ...
    def _get_client(
        ...
        with self._create_client_lock:
            default_config = (
                ...
                if shipyard_config.DISABLE_BOTO_RETRIES
                else Config()
            )
...
class InternalClientFactory(ClientFactory):
    ...
    def get_client(
        ...
        """
        Build and return client for connections originating within Shipyard.
        ...
        additional information to Shipyard server during internal calls.

        :param service_name: Service to build the client for, eg. `s3`
        :param region_name: Region name. See note above.
            If set to None, loads from botocore session.
        :param aws_access_key_id: Access key to use for the client.
            Defaults to Shipyard internal credentials.
        :param aws_secret_access_key: Secret key to use for the client.
            Defaults to Shipyard internal credentials.
        :param aws_session_token: Session token to use for the client.
            Not being used if not set.
...
class ExternalClientFactory(ClientFactory):
    def get_client(
        ...
        """
        Build and return client for connections originating outside Shipyard and targeting Shipyard.
...
class ExternalAwsClientFactory(ClientFactory):
    def get_client(
        ...
        """
        Build and return client for connections originating outside Shipyard and targeting AWS.
...
def _handler_inject_dto_header(params: dict[str, Any], context: dict[str, Any], **kwargs):
    ...
    """
    if (dto := context.pop("_shipyard", None)) is not None:
