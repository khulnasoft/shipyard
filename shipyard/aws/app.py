...
from shipyard.aws.handlers.metric_handler import MetricHandler
from shipyard.aws.handlers.service_plugin import ServiceLoader
from shipyard.aws.trace import TracingHandlerChain
from shipyard.services.plugins import SERVICE_PLUGINS, ServiceManager, ServicePluginManager
...
class ShipyardAwsGateway(Gateway):
    def __init__(self, service_manager: ServiceManager = None) -> None:
        ...
        self.request_handlers.extend(
            [
                ...
                handlers.serve_shipyard_resources,  # try to serve internal resources in /_shipyard first
...
def main():
    """
    Serve the ShipyardGateway with the default configuration directly through hypercorn. This is mostly for
    ...
    # serve the ShipyardAwsGateway in a dev app
    from shipyard.utils.bootstrap import setup_logging
    ...
    gw = ShipyardAwsGateway(SERVICE_PLUGINS)
